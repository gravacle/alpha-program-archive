#!/usr/bin/env python3
"""
SEALED GENERATOR — STAGE8_DERIVED_GRAV_SCALE_FEASIBILITY_HUNT_DARIO_V001.md   (relay 1097, DARIO lane)
...
ART = WS / "STAGE8_DERIVED_GRAV_SCALE_FEASIBILITY_HUNT_DARIO_V001.md"; SIDE = Path(str(ART)+".seal.sha256")
SELF = Path(__file__)
...
def main():
    ...
    # R10 DERIVABLE is the maximum-pressure answer and needs an EXHIBITED seed; the conservative
    # verdicts need their named ground, and any near-proof must declare its quantifier.
    if "FEASIBILITY = DERIVABLE-FROM-RECORD" in t:
        need={"the exhibited seed":"dimensionful seed:","the non-circularity":"NON_CIRCULAR = YES"}
        missing=[k for k,v in need.items() if v not in t]
        if missing:
            return fail(10,f"DERIVABLE may not seal without {missing}")
        print("DERIVABLE_GATE = CLEAN (seed exhibited; non-circularity stated)")
    if "FEASIBILITY = NEEDS-NEW-STRUCTURE" in t:
        need={"the named primitive":"THE MISSING PRIMITIVE, NAMED",
              "the declared quantifier":"NOT AN IMPOSSIBILITY THEOREM",
              "the not-refuted limit":"IS NOT REFUTED",
              "the emergent route pressed":"LATENT_ROUTE = EXHAUSTED"}
        missing=[k for k,v in need.items() if v not in t]
        if missing:
            return fail(10,f"NEEDS-NEW-STRUCTURE may not seal without {missing}")
        print("NEEDS_STRUCTURE_GATE = CLEAN (primitive named; quantifier declared; thesis not "
              "overstated as refuted; emergent route pressed)")
    # R11 the assignment's named flattening must be carried explicitly
    if "DO NOT flatten" not in t and "DO NOT FLATTEN" not in t:
        return fail(11,"the assignment's named flattening must be carried explicitly")
    print("FLATTENING_GATE = CLEAN")
    # R12 a search that found nothing may not be published as a proof of absence
    if "SEARCHED AND NOT FOUND" in t and "not a proof" not in t:
        return fail(12,"a failed search must state that absence of a find is not a proof")
    print("SEARCH_SILENCE_GATE = CLEAN")
    # R13 the two consumers must be confirmed at bytes, not assumed from the hand-off
    if "ONE_OBJECT_TWO_CONSUMERS = CONFIRMED" in t and "at bytes" not in t.lower():
        return fail(13,"the one-object claim must be confirmed at bytes rather than carried")
    print("ONE_OBJECT_GATE = CLEAN")
