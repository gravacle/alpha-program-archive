#!/usr/bin/env python3
"""
SEALED GENERATOR — STAGE8_COUPLING_ASSEMBLY_DARIO_V001.md   (relay 1105, DARIO lane)
...
ART = WS / "STAGE8_COUPLING_ASSEMBLY_DARIO_V001.md"; SIDE = Path(str(ART)+".seal.sha256")
SELF = Path(__file__)
...
AUTHORING_MARKERS = ["PROPOSED_NOT_ADOPTED", "PENDING PRINCIPAL ENTRY"]
...
def main():
    ...
    # R10 THE PROMOTION GATE — the exact import member 02 caught in relay 1101. An
    # IMPORTED_NOTHING certification is not credible unless it names the promotion explicitly.
    if "IMPORTED_NOTHING = CERTIFIED" in t:
        if "PROMOTION" not in t.upper():
            return fail(10,"an imported-nothing certification must address the operator-to-scalar "
                           "PROMOTION, which is the import caught in the predecessor relay")
        if "NO_LOG_REPROMOTED = CERTIFIED" not in t:
            return fail(10,"the no-logarithm ruling must be carried as a certification")
        print("PROMOTION_GATE = CLEAN (the promotion is named and disclaimed; no log re-promoted)")
    # R11 every MACHINERY-INDUCED tag needs BOTH the imported assumption AND the dissolving wall
    tags=t.count("MACHINERY-INDUCED")
    dissolves=t.upper().count("WALL DISSOLVES") + t.upper().count("DISSOLVE")
    if tags and not dissolves:
        return fail(11,"a machinery-induced obstruction must show its wall dissolving")
    print(f"CLASSIFICATION_GATE = CLEAN ({tags} machinery tags; dissolution shown)")
    # R12 a surface-real ingredient must be named with its type, and not built
    if "SURFACE_REAL_INGREDIENT =" in t and "NONE" not in t.split("SURFACE_REAL_INGREDIENT =")[1][:40]:
        seg=t.split("SURFACE_REAL_INGREDIENT =")[1][:600]
        if "type" not in seg.lower() and "DIMENSIONLESS" not in seg.upper():
            return fail(12,"a surface-real ingredient must be named WITH ITS TYPE")
        if "not built" not in seg.lower() and "Not built" not in seg:
            return fail(12,"a surface-real ingredient must be declared not-built")
        print("SURFACE_REAL_GATE = CLEAN (ingredient named with type; declared not built)")
    # R13 the corrections against this lane must be carried, not footnoted away
    if "AUTHORED_RECORD_NATIVE_INTERACTING" in t or "UNLICENSED" in t.upper():
        if "against me" not in t.lower():
            return fail(13,"a correction against this lane must be carried explicitly")
        print("SELF_CORRECTION_GATE = CLEAN")
    # R14 the three assignment rules
    for k in ("RETRACED_NOTHING = CERTIFIED","NO_ORDER_IMPOSED = CERTIFIED"):
        if k not in t: return fail(14,f"missing required certification: {k}")
    print("THREE_RULES_GATE = CLEAN")
