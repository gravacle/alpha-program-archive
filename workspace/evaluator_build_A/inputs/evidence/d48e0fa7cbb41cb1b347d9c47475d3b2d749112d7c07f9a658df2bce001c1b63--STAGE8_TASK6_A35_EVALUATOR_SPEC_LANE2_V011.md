# STAGE8 TASK 6 / A35–RD-22 — evaluator specification V011

**Artifact:** `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V011.md`  
**Lane:** Codex Lane 2  
**Date:** 2026-08-08  
**Custody:** V011 carries the single Q-615 citation-key amendment; all 66 V010 descriptor rows remain byte-identical; Builder B keys the independent R9 resolver separately  
**Status:** V011 FINITE-DELTA SPECIFICATION ONLY — condition 2 resolves a ground atom only through its sealed source-and-span citation; no criterion, physical check, fixture, physical quantity, fixed point, end test, or authorization is executed here

```text
V011 = one citation-key amendment statement
M2_CORPUS_RULE = executable and schema-bound
M2_REGISTRY = 17 rows (SPEC-INCOMPLETE pending sealed corpus definitions)
DESCRIPTOR_ROWS = 66 unchanged from V010
...
[PROVABLE] The commissioned live preflight identifies register head `Q-615`.
That entry confirms the V010 envelope and resolver controls, records condition
2's missing mapping, and commissions one citation-key amendment. The Q-604
guard remains: one lawful row is one row and is not evaluator closure.

[PROVABLE] Before V011 was written, neither its requested filename nor the
commissioned Codex 2 report filename existed in the cleanroom or archive
workspace. V010 remains intact. V011 is a byte-copy-plus-named-replacements
...
[PROVABLE] The three structural fixture source rows migrate to these V011
zero-based half-open byte spans; each generated fixture-manifest row must use
the matching V011 path and final V011 SHA-256. The rows below are generated
from the final source bytes and include each row's terminating newline:

| Fixture ID | V011 source byte span |
|---|---|
| source row for `FX-A35-03-C-FAMILY` | `[141708,141897)` |
| source row for `FX-A35-04-TAU-FAMILY` | `[141897,142085)` |
| source row for `FX-A35-05-PRIMITIVE-THOMSON-CONFLATION` | `[142085,142338)` |
