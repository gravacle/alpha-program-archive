# STAGE8 TASK 6 / A35–RD-22 — evaluator specification V012

**Artifact:** `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V012.md`  
**Lane:** Codex Lane 2  
**Date:** 2026-08-08  
**Custody:** V012 carries the Q-616 row-citation linkage completion and source-supply obligation; exactly one V011 descriptor row changes and 65 remain byte-identical; Builder B confirms independently  
**Status:** V012 FINITE-DELTA SPECIFICATION ONLY — `C-B-V009-06` binds its existing path/span to the sealed source digest in the row itself; its criterion identity, execution class, procedure, and predicate are unchanged; no criterion, physical check, fixture, physical quantity, fixed point, end test, or authorization is executed here

```text
V012 = row citation + source supply only
M2_CORPUS_RULE = executable and schema-bound
M2_REGISTRY = 17 rows (SPEC-INCOMPLETE pending sealed corpus definitions)
DESCRIPTOR_ROWS = 65 unchanged + C-B-V009-06 linkage-only amendment
...
[PROVABLE] The commissioned live preflight identifies register head `Q-616`.
That entry verifies the V011 carrier rule and citation resolver, records the
row's absent source digest and unsupplied source bytes, and commissions the two
parts of one cure: row-citation linkage completion and source supply. The
Q-604 guard remains: one lawful row is one row and is not evaluator closure.

[PROVABLE] Before V012 was written, neither its requested filename nor the
commissioned Codex 2 report filename existed in the cleanroom or archive
workspace. V011 remains intact. V012 is a byte-copy-plus-named-replacements
...
|---|---|---|
| `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V011.md` | `d48e0fa7cbb41cb1b347d9c47475d3b2d749112d7c07f9a658df2bce001c1b63` | immutable V012 carriage base; 65 unchanged descriptor rows and the complete before row |
| `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V010.md` | `31ccee9c45c885e4e379fa1750f38695e293151e74bd8f8c15a5f0ccf23bfc19` | immutable V011 carriage base; all 66 descriptor rows |
| `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V009.md` | `900a240df2bfdee5867eb589ae88c7f282810a8c7718999ad5cdf2bfb3f80698` | immutable V010 carriage base; all 66 descriptor rows |
| `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V008.md` | `3b24fc39f47d4502b01c1f06faf9f02b7828f5ba0cf4b6a882a3b331ae9c0986` | immutable V009 carriage base; all 66 descriptor rows |
| `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V007.md` | `d38d31719b64839744a98da5ee005fb50119f9a26b2b98b0e1a1de445b5d4973` | immutable V008 carriage base; all 66 descriptor rows |
...
**V008-R9-1 — P0 is independently computed.** R9 itself computes every one of
the six §2.1 `P0` conjuncts from the supplied, hash-verified `BASE` carriers
before replaying a criterion. A producer-emitted P0 result object is a contract
fault and is never an input: BR-1 forbids a producer-declared object from
carrying the direction of any criterion, including a precondition. R9 has no
P0 exemption and may not accept a producer status, receipt, or prior verdict as
a substitute for this computation.

**V008-R9-2 — the two missing manifests are closed launch inputs.** The R9
launch contract adds the subject and evidence manifest path/digest pairs on the
existing ledger path/digest pattern. The exact ordered `argv` schema is:
...
The verifier-manifest instance is now an exact closed twelve-field object,
now with the following exact nested inventories:

```json
{
  "type":"object",
  "additionalProperties":false,
  "required":["argv","entry_point","exit_contract","input_roots","optimize","output_path","receipt_authoritative","receipt_path","schema","stdout_discipline","verifier_root_members","verifier_root_sha256"],
  "properties":{
    "argv":{"$ref":"#/$defs/argv_v008"},
    "entry_point":{"type":"string"},
    "exit_contract":{"type":"object"},
    "input_roots":{
      "type":"object",
      "additionalProperties":false,
      "required":["evidence_manifest_sha256","evidence_root_sha256","ledger_sha256","runtime_gate_sha256","runtime_snapshot_sha256","spec_sha256","subject_manifest_sha256"],
      "properties":{
        "evidence_manifest_sha256":{"pattern":"[0-9a-f]{64}","type":"string"},
        "evidence_root_sha256":{"pattern":"[0-9a-f]{64}","type":"string"},
        "ledger_sha256":{"pattern":"[0-9a-f]{64}","type":"string"},
        "runtime_gate_sha256":{"pattern":"[0-9a-f]{64}","type":"string"},
        "runtime_snapshot_sha256":{"pattern":"[0-9a-f]{64}","type":"string"},
        "spec_sha256":{"pattern":"[0-9a-f]{64}","type":"string"},
        "subject_manifest_sha256":{"pattern":"[0-9a-f]{64}","type":"string"}
      }
    },
    ...
    "schema":{"const":"rd22.verifier-manifest.v001","type":"string"},
    "stdout_discipline":{"type":"object"},
    ...
    "verifier_root_sha256":{"pattern":"[0-9a-f]{64}","type":"string"}
  }
}
```

The parent binds all six path/value launch pairs, and R9 verifies both new
manifest files at the corresponding `input_roots` digests before computing
their declared content roots. With those supplied inputs, every §2.1 P0
conjunct is replayable by R9; there is no missing-manifest exception.

**V009-J3 — every declared subject byte object resolves through the existing
evidence carrier.** No new argv or `input_roots` slot is added: the existing
`--evidence-dir`, `--evidence-manifest`, and
`evidence_manifest_sha256`/`evidence_root_sha256` pair already provide the
smallest content-addressed carrier. R9 constructs and validates one closed
resolution record for every row of `subject_manifest.files`:

```json
{
  "type":"object",
  "additionalProperties":false,
  "required":["evidence_payload_path","evidence_payload_sha256","subject_byte_length","subject_relative_path","subject_sha256"],
  "properties":{
    "evidence_payload_path":{"minLength":1,"type":"string"},
    "evidence_payload_sha256":{"pattern":"[0-9a-f]{64}","type":"string"},
    "subject_byte_length":{"minimum":0,"type":"integer"},
    "subject_relative_path":{"minLength":1,"type":"string"},
    "subject_sha256":{"pattern":"[0-9a-f]{64}","type":"string"}
  }
}
```

For each record, `evidence_payload_sha256 = subject_sha256`, the payload bytes
rehash to that value, and their length equals `subject_byte_length`. Zero or
multiple matching payloads produces `PRECONDITION_NOT_REPLAYABLE` naming the
subject carrier. Consequently the integration addendum declared by the subject
manifest must be a payload member of the evidence manifest and evidence
directory; a manifest-only citation is not supplied bytes.

**V009-J4 — verifier-root membership is instance data, not a Builder A
constant.** `verifier_root_members` is sorted by `relative_path`, paths are
unique and package-relative, every row is byte-length/digest verified inside
the verifier package, and

```text
verifier_root_sha256 = SHA256(concat(row.sha256 for row in verifier_root_members))
```

in that sorted order. The parent consumes this sealed array and has no private
membership census. The current census is carried only by the sealed manifest
instance; future membership changes require a reissued instance, not a
specification or parent-code edit.

**V010-M1 — R9 alone resolves closed ground atoms.** A descriptor atom is a
ground atom if and only if its normalized record validates against the
following closed schema and all source-binding conditions below hold:

```json
{
  "type":"object",
  "additionalProperties":false,
  "required":["schema","atom_class","opcode","result_name","evidence_operand","constant_operand","operand_order","mask"],
  "properties":{
    "schema":{"const":"rd22.r9-ground-atom.v001","type":"string"},
    "atom_class":{"const":"P0_EVIDENCE_SHA256_EQ_SEALED_SPEC_SHA256","type":"string"},
    "opcode":{"const":"COMPARE","type":"string"},
    "result_name":{"pattern":"r_[A-Za-z0-9_]+","type":"string"},
    "evidence_operand":{
      "type":"object",
      "additionalProperties":false,
      "required":["source","member_key","field"],
      "properties":{
        "source":{"const":"P0.evidence_files","type":"string"},
        "member_key":{"pattern":"[A-Za-z_][A-Za-z0-9_]*","type":"string"},
        "field":{"const":"sha256","type":"string"}
      }
    },
    "constant_operand":{
      "type":"object",
      "additionalProperties":false,
      "required":["source","constant_name","value"],
      "properties":{
        "source":{"const":"SEALED_DESCRIPTOR_CONSTANT","type":"string"},
        "constant_name":{"pattern":"[A-Z][A-Z0-9_]*_SHA256","type":"string"},
        "value":{"pattern":"[0-9a-f]{64}","type":"string"}
      }
    },
    "operand_order":{"enum":["evidence_left_constant_right","constant_left_evidence_right"],"type":"string"},
    "mask":{"type":"array","maxItems":0,"items":false}
  }
}
```

Qualification is exact: `result_name` and the complete atom occur in the same
sealed descriptor row; `member_key` resolves exactly one row in R9's own
P0-verified evidence table; R9 obtains the evidence operand by rehashing that
row's supplied bytes; `constant_name=value` occurs literally in that same
descriptor row; and neither operand nor the comparison truth depends on a
producer output, receipt, result, or choice. Of the closed descriptor opcodes,
only a `COMPARE` atom satisfying every field and source-binding condition above
qualifies. `STRICT`, `SCHEMA`, `TYPE`, `EXACT`, `KERNEL`, `ENUM`, `DOMAIN`,
`UNITS`, `DAG`, `M2`, `SYMBOLIC`, `SPECTRAL`, and `RUNTIME`, and every other
`COMPARE` atom, are not ground atoms under this rule; no class is admitted by
analogy.

R9 constructs the normalized record from the sealed descriptor and P0 table,
evaluates that atom from those sources alone, and records the independently
computed result under `result_name`. No producer carrier exists for a ground
atom. A producer-emitted invocation or result object purporting to supply such
an atom is a contract fault under BR-1, never replay evidence. The singular
producer `checks[].invocation` carrier remains reserved for the row's
structured-evidence-consuming atom, with its seven-field byte-span linkage; it
does not carry a ground atom. Thus `C-B-V009-06` carries only its structured
`r_dag` invocation, while R9 resolves `r_ground` under this schema.

**V011-O1 — condition-2 citation-key amendment to `rd22.r9-ground-atom.v001`.**

```json
{
  "$schema":"https://json-schema.org/draft/2020-12/schema",
  "$id":"rd22.r9-ground-atom-citation-key-amendment.v001",
  "type":"object",
  "additionalProperties":false,
  "required":["schema","amends","condition","atom_class","result_name","member_key","descriptor_citation","p0_index","forbidden_mappings"],
  "properties":{
    "schema":{"const":"rd22.r9-ground-atom-citation-key-amendment.v001","type":"string"},
    "amends":{"const":"rd22.r9-ground-atom.v001","type":"string"},
    "condition":{"const":2,"type":"integer"},
    "atom_class":{"const":"P0_EVIDENCE_SHA256_EQ_SEALED_SPEC_SHA256","type":"string"},
    "result_name":{"pattern":"r_[A-Za-z0-9_]+","type":"string"},
    "member_key":{"pattern":"[A-Za-z_][A-Za-z0-9_]*","type":"string"},
    "descriptor_citation":{
      "type":"object",
      "additionalProperties":false,
      "required":["carrier","source_sha256","span"],
      "properties":{
        "carrier":{"const":"SEALED_DESCRIPTOR_ROW.atom[result_name].source_and_span","type":"string"},
        "source_sha256":{"pattern":"[0-9a-f]{64}","type":"string"},
        "span":{
          "type":"array",
          "prefixItems":[{"minimum":0,"type":"integer"},{"minimum":0,"type":"integer"}],
          "items":false,
          "minItems":2,
          "maxItems":2
        }
    ...
    "p0_index":{
      "type":"object",
      "additionalProperties":false,
      "required":["table","key_fields","interval","match","cardinality","member_binding"],
      "properties":{
        "table":{"const":"R9.P0.evidence_files_by_citation","type":"string"},
        "key_fields":{"const":["source_sha256","span"],"type":"array"},
        "interval":{"const":"ZERO_BASED_HALF_OPEN","type":"string"},
        "match":{"const":"EXACT_TUPLE_EQUALITY","type":"string"},
        "cardinality":{"const":"EXACTLY_ONE","type":"string"},
        "member_binding":{"const":"MEMBER_KEY_BINDS_EXACT_ROW_MATCHING_DESCRIPTOR_ATOM_CITATION","type":"string"}
      }
    },
    "forbidden_mappings":{
      "const":["PAYLOAD_FILENAME","CONSTANT_DIGEST_SELF_REFERENCE","PRODUCER_SUPPLIED"],
      "type":"array"
    }
  }
}
...
**V008-R9-3 — precondition failure has its own refusal value.** If a carrier
needed to compute P0 is absent or cannot be replayed, R9 emits the following
closed value, naming the missing carrier:

```json
{
  "schema":"rd22.r9-precondition-refusal.v001",
  "status":"PRECONDITION_NOT_REPLAYABLE",
  "criterion_evaluated":false,
  "missing_carrier":"<nonempty exact carrier name>"
}
```

`PRECONDITION_NOT_REPLAYABLE` is a fail-closed refusal and is distinct from a
criterion result of `FAIL`. A criterion `FAIL` may be recorded only after P0
was independently computed and the criterion was actually evaluated; a FAIL
that was never evaluated is not a verdict.

Subject to those three statements, R9 independently:
...
[PROVABLE] The three structural fixture source rows migrate to these V012
zero-based half-open byte spans; each generated fixture-manifest row must use
the matching V012 path and final V012 SHA-256. The rows below are generated
from the final source bytes and include each row's terminating newline:

| Fixture ID | V012 source byte span |
|---|---|
| source row for `FX-A35-03-C-FAMILY` | `[142226,142415)` |
| source row for `FX-A35-04-TAU-FAMILY` | `[142415,142603)` |
| source row for `FX-A35-05-PRIMITIVE-THOMSON-CONFLATION` | `[142603,142856)` |
...
### 12.7E V008 three-statement complete finite-delta certificate

[PROVABLE] V008 was constructed by byte-copy of the pinned V007 base followed
only by the three Q-611 R9 law statements and the mechanically consequent
header, authority-pin, fixture-span, carriage, and terminal-scope updates.
Counts use `/usr/bin/diff -U 3`; file headers are excluded and every other `+`
or `-` line, including a bare blank-line marker, is counted.

```text
CARRIAGE_BASE = STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V007.md
CARRIAGE_BASE_SHA256 = d38d31719b64839744a98da5ee005fb50119f9a26b2b98b0e1a1de445b5d4973
CARRIAGE_OUTPUT = STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V008.md
COUNT_CONVENTION = diff -U 3; exclude only ---/+++ headers; include blank lines
FINAL_HUNKS = 6
FINAL_INSERTIONS = 155
FINAL_DELETIONS = 21
UNASSIGNED_HUNKS = 0
MULTIPLY_ASSIGNED_HUNKS = 0
...
| Delta item | Exact authorization and content |
|---|---|
| V8-01 | H1(a): R9 computes all six P0 conjuncts itself and faults any producer-emitted P0 result object. |
| V8-02 | H1(b): closed 22-item argv schema plus closed eleven-field manifest and seven-field input-roots schemas; subject/evidence manifest path+digest pairs supplied. |
| V8-03 | H1(c): closed `PRECONDITION_NOT_REPLAYABLE` refusal value, distinct from criterion-FAIL and naming the missing carrier. |
| V8-04 | H1/output: advance header, preflight, and immutable V007 base pin. |
| V8-05 | Whole-state carriage: recompute the three structural-fixture row spans after the R9 insertion; the row bytes themselves remain unchanged. |
| V8-06 | H1/output: add this complete finite-delta certificate and replace the scoped terminal verdict. |

[PROVABLE] A regenerated diff assigns every physical hunk and every changed
line exactly once to V8-01–V8-06. V8-01–V8-03 are the only new law statements.
All 66 descriptor row bodies are byte-identical to V007. The protected runtime
pin (§9.1), authority firewall, and aggregate rule (§9.5) are byte-identical to
V007.

Correction of record: the V008 certificate's prior `160/26` figures were
produced by Python `difflib.SequenceMatcher`, whose alignment emitted five
additional canceling replacement pairs. The declared `/usr/bin/diff -U 3`
method emits `155/21`; four independent command-line recomputations agree.
Both erroneous figures were +5, so their net line delta happened to balance.

### 12.7F V009 Q-612 complete finite-delta certificate

[PROVABLE] V009 was constructed by byte-copy of the pinned V008 base followed
only by the J1/J2 corrections, the two J3/J4 schema-boundary repairs, and their
mechanically consequent header, pin, fixture-span, carriage, and terminal-scope
updates. Counts use `/usr/bin/diff -U 3`, excluding only the `---`/`+++` file
headers and including every content line.

```text
CARRIAGE_BASE = STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V008.md
CARRIAGE_BASE_SHA256 = 3b24fc39f47d4502b01c1f06faf9f02b7828f5ba0cf4b6a882a3b331ae9c0986
CARRIAGE_OUTPUT = STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V009.md
COUNT_CONVENTION = /usr/bin/diff -U 3; exclude only ---/+++ file headers
FINAL_HUNKS = 9
FINAL_INSERTIONS = 120
FINAL_DELETIONS = 23
UNASSIGNED_HUNKS = 0
MULTIPLY_ASSIGNED_HUNKS = 0
DESCRIPTOR_ROWS_CHANGED = 0
...
| Delta item | Exact authorization and content |
|---|---|
| V9-01 | J1: replace the three newline-exclusive V008 table ends; generate all current V009 spans from source bytes. |
| V9-02 | J2: correct V008's recorded diff counts to `155/21` and name the `difflib` +5/+5 cause. |
| V9-03 | J3: schema-bind every subject row to exactly one content-addressed evidence payload; addendum uses the existing evidence carrier. |
| V9-04 | J4: add closed `verifier_root_members` rows to the manifest instance and delete parent-private membership transcription. |
| V9-05 | Whole-state carriage: advance header/pins and add this finite-delta certificate plus scoped terminal lines. |

[PROVABLE] All 66 descriptor row bodies are byte-identical to V008. V009 adds
no criterion, expected predicate, opcode, physical value, or authorization.

### 12.7G V010 ground-atom complete finite-delta certificate

[PROVABLE] V010 was constructed by byte-copy of the pinned V009 base followed
only by the single M1 ground-atom statement and its mechanically consequent
header, base-pin, fixture-span, carriage, and terminal-scope updates. Counts
use `/usr/bin/diff -U 3`, excluding only the `---`/`+++` file headers and
including every content line.

```text
CARRIAGE_BASE = STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V009.md
CARRIAGE_BASE_SHA256 = 900a240df2bfdee5867eb589ae88c7f282810a8c7718999ad5cdf2bfb3f80698
CARRIAGE_OUTPUT = STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V010.md
COUNT_CONVENTION = /usr/bin/diff -U 3; exclude only ---/+++ file headers
FINAL_HUNKS = 6
...
| Delta item | Exact authorization and content |
|---|---|
| V10-01 | M1: install the one closed R9 ground-atom resolution statement, including exact opcode/source qualification and BR-1 producer-carrier prohibition. |
| V10-02 | M1/output: advance header and immutable V009 base pin. |
| V10-03 | Whole-state carriage: regenerate three structural-fixture spans after the M1 insertion; fixture row bodies remain unchanged. |
| V10-04 | M1/output: add this complete finite-delta certificate and scoped terminal lines. |

[PROVABLE] V10-01 is the only new law statement. All 66 descriptor row bodies
are byte-identical to V009. The protected runtime pin (§9.1), authority
firewall, and aggregate rule (§9.5) are byte-identical to V009. V010 adds no
criterion, expected predicate, opcode, physical value, or authorization.

### 12.7H V011 citation-key complete finite-delta certificate

[PROVABLE] V011 was constructed by byte-copy of the pinned V010 base followed
only by the single O1 closed-schema amendment and its mechanically consequent
header, base-pin, instance-neutral membership sentence, fixture-span,
carriage, and terminal-scope updates. Counts use `/usr/bin/diff -U 3`,
excluding only the `---`/`+++` file headers and including every content line.

```text
CARRIAGE_BASE = STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V010.md
CARRIAGE_BASE_SHA256 = 31ccee9c45c885e4e379fa1750f38695e293151e74bd8f8c15a5f0ccf23bfc19
CARRIAGE_OUTPUT = STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V011.md
COUNT_CONVENTION = /usr/bin/diff -U 3; exclude only ---/+++ file headers
FINAL_HUNKS = 7
...
| Delta item | Exact authorization and content |
|---|---|
| V11-01 | O1: install the one closed citation-key amendment, including exact tuple key, cardinality, member binding, and three forbidden mappings. |
| V11-02 | O1/output: advance header and immutable V010 base pin. |
| V11-03 | Instance-data carriage: delete the stale prose census while retaining manifest-instance membership law. |
| V11-04 | Whole-state carriage: regenerate three structural-fixture spans after the O1 insertion; fixture row bodies remain unchanged. |
| V11-05 | O1/output: add this complete finite-delta certificate and scoped terminal lines. |

[PROVABLE] V11-01 is the only new law statement. All 66 descriptor row bodies
are byte-identical to V010. The protected runtime pin (§9.1), authority
firewall, and aggregate rule (§9.5) are byte-identical to V010. V011 adds no
criterion, expected predicate, opcode, physical value, or authorization.

### 12.7I V012 row-citation complete finite-delta certificate

[PROVABLE] V012 was constructed by byte-copy of the pinned V011 base followed
only by the `C-B-V009-06` source-digest insertion and its mechanically
consequent header, base-pin, fixture-span, carriage, and terminal-scope
updates. The cited full source is a required content-addressed structural-
evidence payload; R9 verifies its digest and rehashes the cited span itself.

The complete descriptor row before and after is displayed below. Removing the
leading diff marker yields the exact UTF-8 row bytes, excluding only the line
terminator:
...
[PROVABLE] Column-byte comparison is exact: criterion ID, execution class,
deterministic procedure, and expected predicate are byte-identical. Only the
sealed-input/citation column changes, by the single 86-byte insertion above.
This is linkage completion, not a criterion, procedure, or predicate change.

Counts use `/usr/bin/diff -U 3`, excluding only the `---`/`+++` file headers
and including every content line:

```text
CARRIAGE_BASE = STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V011.md
CARRIAGE_BASE_SHA256 = d48e0fa7cbb41cb1b347d9c47475d3b2d749112d7c07f9a658df2bce001c1b63
CARRIAGE_OUTPUT = STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V012.md
COUNT_CONVENTION = /usr/bin/diff -U 3; exclude only ---/+++ file headers
FINAL_HUNKS = 6
FINAL_INSERTIONS = 89
FINAL_DELETIONS = 21
...
DESCRIPTOR_ROW = C-B-V009-06
DESCRIPTOR_CHANGE_CLASS = LINKAGE_ONLY
```

| Delta item | Exact authorization and content |
|---|---|
| V12-01 | Q1: insert the sealed source SHA-256 into the existing path/span citation in `C-B-V009-06`; display the complete before/after rows and byte delta. |
| V12-02 | Q2: require the cited full source as a content-addressed structural-evidence payload for R9 digest and span replay. |
| V12-03 | Q1/output: advance header and immutable V011 base pin. |
| V12-04 | Whole-state carriage: regenerate three structural-fixture spans; fixture row bodies remain unchanged. |
| V12-05 | Q1/output: add this complete finite-delta certificate and scoped terminal lines. |

[PROVABLE] Exactly one descriptor row changes, linkage only; the other 65 are
byte-identical to V011. The protected runtime pin (§9.1), authority firewall,
and aggregate rule (§9.5) are byte-identical to V011. V012 adds no criterion,
expected predicate, opcode, physical value, or authorization.
