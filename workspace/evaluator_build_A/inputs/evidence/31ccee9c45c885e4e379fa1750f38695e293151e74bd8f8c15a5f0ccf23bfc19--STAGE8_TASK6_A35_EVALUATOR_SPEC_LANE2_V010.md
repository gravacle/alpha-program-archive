# STAGE8 TASK 6 / A35–RD-22 — evaluator specification V010

**Artifact:** `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V010.md`  
**Lane:** Codex Lane 2  
**Date:** 2026-08-08  
**Custody:** V010 carries the single Q-614 ground-atom resolution statement; all 66 V009 descriptor rows remain byte-identical; Builder B implements the independent R9 resolver separately  
**Status:** V010 FINITE-DELTA SPECIFICATION ONLY — qualifying ground atoms are reconstructed only by R9 from P0-verified evidence and sealed spec constants; no criterion, physical check, fixture, physical quantity, fixed point, end test, or authorization is executed here

```text
V010 = one ground-atom resolution statement
M2_CORPUS_RULE = executable and schema-bound
M2_REGISTRY = 17 rows (SPEC-INCOMPLETE pending sealed corpus definitions)
DESCRIPTOR_ROWS = 66 unchanged from V009
...
[PROVABLE] The commissioned live preflight identifies register head `Q-614`.
That entry records run 031's single substantive replay fault, adjudicates the
producer's omitted `r_ground` result carrier as conformant, and commissions
one R9 ground-atom resolution statement. The Q-604 guard remains: one lawful
row is one row and is not evaluator closure.

[PROVABLE] Before V010 was written, neither its requested filename nor the
commissioned Codex 2 report filename existed in the cleanroom or archive
workspace. V009 remains intact. V010 is a byte-copy-plus-named-replacements
...
membership census. The currently sealed Builder B package generates thirteen
rows, including `verifier/preconditions.py`; future membership changes require
a reissued manifest instance, not a parent-code edit.
...
[PROVABLE] The three structural fixture source rows migrate to these V010
zero-based half-open byte spans; each generated fixture-manifest row must use
the matching V010 path and final V010 SHA-256. The rows below are generated
from the final source bytes and include each row's terminating newline:

| Fixture ID | V010 source byte span |
|---|---|
| source row for `FX-A35-03-C-FAMILY` | `[139421,139610)` |
| source row for `FX-A35-04-TAU-FAMILY` | `[139610,139798)` |
| source row for `FX-A35-05-PRIMITIVE-THOMSON-CONFLATION` | `[139798,140051)` |
