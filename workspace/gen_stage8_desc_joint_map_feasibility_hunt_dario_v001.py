#!/usr/bin/env python3
"""
SEALED GENERATOR — STAGE8_DESC_JOINT_MAP_FEASIBILITY_HUNT_DARIO_V001.md   (relay 1085, DARIO lane)
...
ART = WS / "STAGE8_DESC_JOINT_MAP_FEASIBILITY_HUNT_DARIO_V001.md"; SIDE = Path(str(ART)+".seal.sha256")
SELF = Path(__file__)
...
def main():
    ...
    # R9 the latent-first burden: the SURPRISING verdict needs a proof with a DECLARED quantifier
    if "FEASIBILITY = NEEDS-SECOND-RULE" in t:
        need={"the collapse proof":"Y-DIAGONAL COLLAPSE",
              "the declared quantifier":"QUANTIFIER, DECLARED FIRST",
              "the escape census":"WHAT WOULD OVERTURN SECTION 4",
              "the SM-6 legitimacy ruling":"SM-6-legitimate",
              "the second rule named":"RULE 2"}
        missing=[k for k,v in need.items() if v not in t]
        if missing:
            return fail(9,"NEEDS-SECOND-RULE is the verdict that ADDS authorship and may not seal "
                          f"without {missing}")
        # a proof must not be silently widened past the family it covers
        if "IT DOES NOT prove that no joint map exists in any sense" not in t:
            return fail(9,"the collapse proof must disclaim the wider claim it does not make")
        print("PROOF_GATE = CLEAN (collapse proved, quantifier declared, escapes censused, wider "
              "claim disclaimed, SM-6 legitimacy ruled, second rule named)")
    if "FEASIBILITY = FEASIBLE-FROM-SEALED" in t:
        need={"the named operation":"JOINT MAP:","the receiver":"RECEIVER:"}
        missing=[k for k,v in need.items() if v not in t]
        if missing: return fail(9,f"FEASIBLE-FROM-SEALED sealed without {missing}")
        print("FEASIBILITY_GATE = CLEAN")
    # R10 the latent route must be worked before any verdict, and the product must stay excluded
    if "LATENT_JOINT_MAP = EXHAUSTED" not in t:
        return fail(10,"the latent route must be exhausted and reported before any verdict")
    if "PRODUCT_STILL_EXCLUDED = YES" not in t:
        return fail(10,"the product exclusion must be carried explicitly")
    print("LATENT_FIRST_GATE = CLEAN (routes pressed and reported; product exclusion carried)")
    # R10b the naming collision the assignment makes mandatory
    if "NAMING-COLLISION GUARD LIVE" not in t:
        return fail(10,"the d_state / W_N naming-collision guard must be declared")
    print("COLLISION_GUARD = CLEAN")
