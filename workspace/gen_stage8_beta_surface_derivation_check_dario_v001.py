#!/usr/bin/env python3
"""
SEALED GENERATOR — STAGE8_BETA_SURFACE_DERIVATION_CHECK_DARIO_V001.md   (relay 1109, DARIO lane)
...
ART = WS / "STAGE8_BETA_SURFACE_DERIVATION_CHECK_DARIO_V001.md"; SIDE = Path(str(ART)+".seal.sha256")
SELF = Path(__file__)
...
def main():
    ...
    # R10 CHECKER GATES. A check must recompute rather than defer, must not privilege its own
    # ruling, and — where it declines to confirm — must say plainly that it also did not refute.
    if "TESTIMONIAL WEIGHT OF THE SUBJECT = ZERO" not in t:
        return fail(10,"a check must declare the subject's zero testimonial weight")
    if "CLAIMED" not in t or "checker's ruling is not privileged" not in t.lower():
        return fail(10,"a check must not privilege its own ruling")
    print("CHECKER_GATE = CLEAN (subject zero-weighted; own ruling not privileged)")
    if "VERDICT = UNDETERMINED" in t:
        for k,v in {"the not-refuted statement":"NOT REFUTED",
                    "the repair path":"WHAT WOULD CONVERT THIS TO RAIL-CONFIRMED"}.items():
            if v not in t: return fail(10,f"an UNDETERMINED check verdict needs {k}")
        print("UNDETERMINED_GATE = CLEAN (not-refuted stated as plainly as not-established; repair "
              "path given)")
    # R11 a declared conflict of interest must be carried where the checker's own work is cited
    if "DECLARED CONFLICT" not in t:
        return fail(11,"a check of an artifact standing on this lane's own ground must declare it")
    print("CONFLICT_GATE = CLEAN")
    # R12 custody must be recomputed, not accepted from the subject
    if "rehashed" not in t.lower() or "rather than accepted" not in t.lower():
        return fail(12,"the subject's custody chain must be recomputed, not accepted")
    print("CUSTODY_RECOMPUTE_GATE = CLEAN")
    # R13 an anomaly or self-error found in passing must be disclosed, not buried
    if "SUBJECT_SIDECAR = ABSENT" not in t:
        return fail(13,"the missing subject sidecar must be flagged in the final lines")
    if "SHELL TRAP" not in t.upper():
        return fail(13,"a checker's own procedural error must be disclosed")
    print("DISCLOSURE_GATE = CLEAN (subject sidecar flagged; checker's own error disclosed)")
