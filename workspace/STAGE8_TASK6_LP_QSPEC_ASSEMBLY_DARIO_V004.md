# STAGE 8 / TASK 6 / SUBGATE — LP-QSPEC: THE EIGHTEEN-SLOT ASSEMBLY — DARIO V004

Lane: Dario (Claude Opus 5), assembly lane
Task: PASTE 639 / Task 6, the `(M5a-V002)` conjunct #18 long pole
Authority: DoR-020-A8. **THIS ARTIFACT ADOPTS NOTHING, FREEZES NOTHING, SEALS
NOTHING, AND EVALUATES NOTHING.**
Supersedes: my V003 (`c205602d…`), convicted at `85a7a9c3…` (Q-572) — which
superseded my V002 (`11e4e6e4…`, Q-566) and my V001 (`a2fdd7c0…`, Q-556).
Custody: V004 implements the Q-572 §5.3 bounded repair **and nothing else**. That
repair is confined to **certificate prose**: a miscited hunk, a false empty
multiply-assignment claim, two mis-classified fidelity assertions, and a residual
miscount. The conviction states expressly that *"the operative assembly, rows
2/6/7, all 18 grades, appeals, rulings, and freeze text require no change,"* and
this artifact changes none of them.
**It relitigates no grade, no appeal disposition, and no ruling.**

```text
REGISTER_HEAD = Q-574
FIXES = 5/5 (Q-572 §5.3 certificate prose only)
OPERATIVE_CONTENT = byte-identical to V003
...
VERB_AUDIT_SELF = CLEAN (+3 build defects self-caught; +1 finding disclosed)
...
| Check | Result |
|---|---|
| Register head Q-574 *(live appends beyond it are verify-and-proceed)* | verified |
| Base: my V003 = `c205602d4db6f30df3999deb4f2a1af425e14e38120cc8904af2480e04911aa4` | **verified before reading**; sidecar matches |
| Conviction of record (Q-572) = `85a7a9c32e19d0713845dd1bd7ef50d96c0b27190878e433b55de24559444d56` | **verified before reading** |
...
UNASSIGNED_HUNKS      = 0
MULTIPLY_ASSIGNED_HUNKS = 7        (displayed and justified below)
ROWS_WITHOUT_A_HUNK   = 0
CARRIAGE = complete_finite_delta
```

[PROVABLE] **The multiply-assigned set is NOT empty, and V003 was wrong to imply
it was.** C-V5's `PRESEAL_CHECK` is the pair *"unassigned hunks empty and
multiply-assigned hunks empty"*; V003 displayed only the first count and asserted
*"both are displayed above as counts."* That was false — the second count was
never displayed, and it is **seven**, not zero. The seven are:

| Hunk | Rows sharing it | Why the sharing is lawful |
|---|---|---|
| H05 | V2, V3 | one contiguous lead-board block; V2 restores `THOMSON`/`UNUSED_PREDICTION`, V3 corrects `DELTA`/`CARRIAGE` and the head — adjacent lines, one hunk |
| H10 | V5, V6 | the §0.1 addition ends and the old §0.2 heading is deleted at the same boundary |
| H11 | V5, V6 | the same boundary continues: V5's admission paragraph and V6's replacement heading are one insertion run |
| H13 | V7, V8, V9, V10, V11 | the five new sections (§0.3–§0.7) were inserted at **one point** in the file, so one insertion hunk carries all five rows |
| H19 | V12, V15, V16 | the S17 note (V15), the new §1.1 heading (V12) and the displaced old §1.1 heading (V16) meet at one line |
| H20 | V12, V16 | the release-condition display (V12) and the §1.2 renumber (V16) are one insertion run |
| H25 | V19, V20 | the anchor observation (V19) ends where the old §4.4 heading (V20) is removed |

[YOURS] **Sharing is lawful; silence about it was not.** C-V5 forbids a hunk with
*no* row and a row with *no* hunk — the two failure modes that let a change escape
the table. A hunk carrying several rows hides nothing: every row is still named,
and the hunk is still assigned. What the check actually requires is that the
partition be **total and surjective**, not injective, because `diff` merges
adjacent edits by proximity and an author cannot choose the boundaries. Every one
of the seven above is exactly that: two or more authorized edits that happen to be
textually adjacent. The defect in V003 was the unearned word *"both"* — claiming a
display it had not made.
...
V004 INSERTIONS                 = 244
V003 DELETIONS                  = 26
LINE ARITHMETIC    = 760 - 26 + 244 = 978   (verified)
```
...
OPERATIVE_CONTENT = byte-identical to V003 (verified by diff scope at §5.5: no
...
CARRIAGE = complete_finite_delta (+17 hunks, all assigned; conventions
...
VERB_AUDIT_SELF = CLEAN (+3 build defects self-caught pre-seal at §5.6, +1 finding
  outside the repair list disclosed at §0, +V003's self-caught miscount carried)
```
