#!/usr/bin/env python3
"""
SEALED GENERATOR — STAGE8_GRAVITY_FORCES_COUPLING_HUNT_DARIO_V001.md   (relay 1093, DARIO lane)
...
ART = WS / "STAGE8_GRAVITY_FORCES_COUPLING_HUNT_DARIO_V001.md"; SIDE = Path(str(ART)+".seal.sha256")
SELF = Path(__file__)
...
def main():
    ...
    # R10 the founding thesis is the maximum-pressure answer: it may seal ONLY with all three
    # parts of the forcing test named. The obstructive answer must carry its testability list and
    # must not overstate itself into a refutation.
    if "GRAVITY_FORK = GRAVITY-FORCES" in t:
        need={"the unique-require":"unique-require:","the fixed trigger":"fixed trigger:",
              "proven independence":"INDEPENDENCE = PROVEN"}
        missing=[k for k,v in need.items() if v not in t]
        if missing:
            return fail(10,"the founding thesis may not seal without all three parts of the "
                           f"forcing test: {missing}")
        print("THESIS_GATE = CLEAN (unique-require, fixed trigger and independence all named)")
    if "GRAVITY_FORK = GRAVITY-INSUFFICIENT" in t:
        need={"the named residual freedom":"RESIDUAL FREEDOM, NAMED",
              "the not-refuted limit":"IS NOT REFUTED HERE",
              "the testability list":"WHAT WOULD MAKE THE THESIS TESTABLE"}
        missing=[k for k,v in need.items() if v not in t]
        if missing:
            return fail(10,f"an insufficiency ruling may not seal without {missing}")
        print("INSUFFICIENCY_GATE = CLEAN (residual named, thesis not overstated as refuted, "
              "testability list present)")
    # R11 a circularity finding must be attributed, not asserted by this lane alone
    if "INDEPENDENCE = CIRCULAR" in t and "the finding is the opposite lane's" not in t:
        return fail(11,"a circularity finding must name whose finding it is")
    print("ATTRIBUTION_GATE = CLEAN")
    # R12 a search that found nothing may not be published as a proof
    if "NOT INDEPENDENTLY CONFIRMED HERE" in t and "not a closure proof" not in t:
        return fail(12,"an unconfirmed fork must state that a failure to find is not a proof")
    print("SEARCH_SILENCE_GATE = CLEAN")
    # R13 carried premises may not be dressed as verified
    if "CARRIED, NOT VERIFIED AT BYTES" not in t:
        return fail(13,"premises carried without byte verification must be labelled as such")
    print("PREMISE_GATE = CLEAN (unverified premises labelled)")
