# STAGE8 TASK 6 / A35–RD-22 — evaluator specification V003

**Artifact:** `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V003.md`  
**Lane:** Codex Lane 2  
**Date:** 2026-08-06  
**Custody:** binding-class repair by Lane 2; cross-family review by Dario; any RD-22 execution authorization remains a principal act  
**Status:** V003 FINITE-DELTA SPECIFICATION ONLY — 38 criteria are bound as carried and 28 are re-rendered; no implementation, check, fixture, physical quantity, fixed point, end test, or authorization is executed here

## 0. Preflight, scope, and authority boundary

### 0.1 Register and no-clobber checks

[PROVABLE] The living questions-settled register was read by entry. Its current
head is `Q-568`; that entry records the scoping V003 quotation repair and does
not modify Q-567's evaluator-specification conviction. The commissioned
live-append state is therefore exact.

[PROVABLE] Before V003 was written, neither its requested filename nor its
protocol sidecar existed in the cleanroom or the archive workspace. V002 and
its sealed sidecar remained intact. V003 is a no-clobber finite delta, not an
overwrite.
...
| `B-V011-SP2-07` | Remove load-bearing `assert`, agree under normal and optimized runs, use a content-addressed isolated runtime, and pass a terminal seal. |
...
| `DAG(G,P)` | Parse nodes and exact parent lists, reject cycles/self-parenting/missing parents, and compare with required parents `P`. |
| `M2(q,S)` | Run three guards over content set `S`: fixed-string; whitespace-normalized; and scope-and-synonym AST/context search. The implementation must neutralize all four registered false-negative modes: regex metacharacters, line wraps, self-reference/scope, and hyphen/space/underscore variants (the last by joint `[-_ ]` normalization); record every hit with its source span. |
...
| `C-B-V008-09` | STRUCTURAL | interval/Maxwell closure schemas; finite candidate/tie/failure grammar and certificate `(G_branch,p_branch)`; expected branch-ID/outcome ledger `E_branch`; dependency graph | `r_enum:=ENUM(G_branch,p_branch)`; `r_ids:=COMPARE(r_enum.ids,E_branch.ids,empty)`; for every `b` in `r_enum.items`, `r_schema_b:=SCHEMA(S_branch,b)` and `r_out_b:=COMPARE(r_schema_b.normalized,E_branch[b],empty)`; `r_dag:=DAG(G_dependencies,P_dependencies)` | `P0 and r_enum.success and r_ids.success and r_dag.success and (for every b in r_enum.items: r_schema_b.success and r_out_b.success)` |
...
| `C-B-V008-11` | STRUCTURAL | holdout registry/commitment/beacon/eligibility schema and manifest `(S_holdout,M_holdout)`; protocol proof `p_holdout`; finite contamination-transition grammar/certificate `(G_contam,p_contam)`; expected transition ledger `E_contam`; outcome-access query `q_outcome` | `r_schema:=SCHEMA(S_holdout,M_holdout)`; `r_access:=M2(q_outcome,preseal_sources)`; `r_protocol:=KERNEL(p_holdout,proof_axioms)`; `r_enum:=ENUM(G_contam,p_contam)`; `r_ids:=COMPARE(r_enum.ids,E_contam.ids,empty)`; for every `m` in `r_enum.items`, `r_m:=EXACT(m(M_holdout))` and `r_cmp_m:=COMPARE(r_m.normal_form,E_contam[m],empty)` | `P0 and r_schema.success and r_access.success and r_access.hits=empty and r_protocol.success and r_enum.success and r_ids.success and (for every m in r_enum.items: r_m.success and r_cmp_m.success)` |
...
| `C-B-V009-02` | STRUCTURAL | equivalence grammar/certificate `(G_equiv,p_equiv)`; unit/sector declarations; expected allowed-ID ledger `E_equiv` including the dimensionful-scale rejection | `r_enum:=ENUM(G_equiv,p_equiv)`; `r_ids:=COMPARE(r_enum.ids,E_equiv.ids,empty)`; for every `e` in `r_enum.items`, `r_units_e:=UNITS(e)` and `r_cmp_e:=COMPARE(r_units_e.classification,E_equiv[e],empty)` | `P0 and r_enum.success and r_ids.success and (for every e in r_enum.items: r_units_e.success and r_cmp_e.success)` |
...
| `C-B-V009-04` | STRUCTURAL | three logarithm definitions/domains; finite domain-partition grammar/certificate `(G_log_domain,p_log_domain)`; expected dispatch ledger `E_log`; boundary fixtures | `r_enum:=ENUM(G_log_domain,p_log_domain)`; `r_ids:=COMPARE(r_enum.ids,E_log.ids,empty)`; for every `x` in `r_enum.items`, `r_formal_x:=DOMAIN(log_formal,D_formal[x])`, `r_principal_x:=DOMAIN(log_principal,D_principal[x])`, `r_taylor_x:=DOMAIN(log_taylor,D_taylor[x])`, and `r_dispatch_x:=COMPARE({r_formal_x.status,r_principal_x.status,r_taylor_x.status},E_log[x],empty)`; `r_bound:=EXACT(taylor_norm_bound)` | `P0 and r_enum.success and r_ids.success and r_bound.success and (for every x in r_enum.items: r_dispatch_x.success)` |
...
| `C-B-V009-06` | STRUCTURAL | machine seal graph and parent schema | `DAG` all nodes from specification through final review | every required parent is literal, the graph is acyclic, and no prose-only dependency is accepted |
| `C-B-V009-07` | STRUCTURAL | response configuration schema/manifest `(S_config,M_config)`; frozen expected manifest `E_config`; finite mutation grammar/certificate `(G_config_mut,p_config_mut)`; expected rejection ledger `E_config_mut` | `r_schema:=SCHEMA(S_config,M_config)`; `r_frozen:=COMPARE(M_config,E_config,empty)`; `r_enum:=ENUM(G_config_mut,p_config_mut)`; `r_ids:=COMPARE(r_enum.ids,E_config_mut.ids,empty)`; for every `m` in `r_enum.items`, `r_m:=EXACT(m(M_config))` and `r_cmp_m:=COMPARE(r_m.normal_form,E_config_mut[m],empty)` | `P0 and r_schema.success and r_frozen.success and r_enum.success and r_ids.success and (for every m in r_enum.items: r_m.success and r_cmp_m.success)` |
...
| `C-B-V009-10` | STRUCTURAL | frozen rank-20 design; finite 20-basis grammar/certificate `(G_basis20,p_basis20)`; finite parity-branch grammar/certificate `(G_parity,p_parity)`; reconstruction graph `G_reconstruct`; expected reconstruction ledger `E_reconstruct`; topological-null fixture | `r_type:=TYPE(G_reconstruct)`; `r_basis:=ENUM(G_basis20,p_basis20)`; `r_parity:=ENUM(G_parity,p_parity)`; `r_basis_ids:=COMPARE(r_basis.ids,E_reconstruct.basis_ids,empty)`; `r_parity_ids:=COMPARE(r_parity.ids,E_reconstruct.parity_ids,empty)`; for every `u` in `r_basis.items`, `r_u:=EXACT(reconstruct(u))` and `r_cmp_u:=COMPARE(r_u.normal_form,E_reconstruct[u],empty)`; for every `p` in `r_parity.items`, `r_p:=EXACT(reconstruct_branch(p))` and `r_cmp_p:=COMPARE(r_p.normal_form,E_reconstruct[p],empty)`; `r_null:=EXACT(reconstruct(T_top)=0)` | `P0 and r_type.success and r_basis.success and r_parity.success and r_basis_ids.success and r_parity_ids.success and r_null.success and (for every u in r_basis.items: r_u.success and r_cmp_u.success) and (for every p in r_parity.items: r_p.success and r_cmp_p.success)` |
...
| `C-B-V009-13` | STRUCTURAL | registry schema/manifest `(S_registry,M_registry)`; eligible-ID grammar/certificate `(G_eligible,p_eligible)`; selection proof `p_select`; finite contamination grammar/certificate `(G_contam,p_contam)`; expected selection/failure ledger `E_holdout` | `r_schema:=SCHEMA(S_registry,M_registry)`; `r_eligible:=ENUM(G_eligible,p_eligible)`; `r_eligible_ids:=COMPARE(r_eligible.ids,M_registry.eligible_ids,empty)`; `r_select:=KERNEL(p_select,proof_axioms)`; `r_select_cmp:=COMPARE(r_select.conclusion,E_holdout.selection,empty)`; `r_contam:=ENUM(G_contam,p_contam)`; `r_contam_ids:=COMPARE(r_contam.ids,E_holdout.contamination_ids,empty)`; for every `m` in `r_contam.items`, `r_m:=EXACT(m(M_registry))` and `r_cmp_m:=COMPARE(r_m.normal_form,E_holdout[m],empty)` | `P0 and r_schema.success and r_eligible.success and r_eligible_ids.success and r_select.success and r_select_cmp.success and r_contam.success and r_contam_ids.success and (for every m in r_contam.items: r_m.success and r_cmp_m.success)` |

## 4. V010 blocker-to-check map

| Check ID | Class | Content-addressed inputs beyond `BASE` | Ordered executable procedure | PASS iff |
|---|---|---|---|---|
| `C-B-V010-01` | **GATED-EXECUTION** | `FX-A35-01`; V010 response formula; declared symbolic-limit domain; finite post-hoc-multiplier mutation grammar/certificate `(G_factor,p_factor)`; expected rejection ledger `E_factor` | after the physical-evaluation gate: `r_response:=SYMBOLIC(V010_canonical_response)`; `r_domain:=DOMAIN(V010_canonical_response,D_limit)`; `r_enum:=ENUM(G_factor,p_factor)`; `r_ids:=COMPARE(r_enum.ids,E_factor.ids,empty)`; for every `m` in `r_enum.items`, `r_m:=SYMBOLIC(m(V010_canonical_response))` and `r_cmp_m:=COMPARE(r_m.classification,E_factor[m],empty)` | `P0 and r_response.success and r_domain.success and r_enum.success and r_ids.success and (for every m in r_enum.items: r_m.success and r_cmp_m.success)` |
...
| `C-B-V010-13` | STRUCTURAL | closed registry/dedupe/commitment/beacon/eligibility schemas and manifests; exhaustive registry grammar/certificate `(G_registry,p_registry)`; typed custodian graph `G_custodian`; selection proof `p_selection`; contamination/drift grammar/certificate `(G_failure,p_failure)`; expected registry/selection/failure ledger `E_holdout`; outcome-access query `q_outcome` | `r_registry:=ENUM(G_registry,p_registry)`; `r_registry_ids:=COMPARE(r_registry.ids,E_holdout.registry_ids,empty)`; `r_schema:=SCHEMA(S_holdout,M_holdout)`; `r_custodian:=TYPE(G_custodian)`; `r_selection:=KERNEL(p_selection,proof_axioms)`; `r_selection_cmp:=COMPARE(r_selection.conclusion,E_holdout.selection,empty)`; `r_access:=M2(q_outcome,preseal_sources)`; `r_failure:=ENUM(G_failure,p_failure)`; `r_failure_ids:=COMPARE(r_failure.ids,E_holdout.failure_ids,empty)`; for every `m` in `r_failure.items`, `r_m:=EXACT(m(M_holdout))` and `r_cmp_m:=COMPARE(r_m.normal_form,E_holdout[m],empty)` | `P0 and r_registry.success and r_registry_ids.success and r_schema.success and r_custodian.success and r_selection.success and r_selection_cmp.success and r_access.success and r_access.hits=empty and r_failure.success and r_failure_ids.success and (for every m in r_failure.items: r_m.success and r_cmp_m.success)` |
| `C-B-V010-14` | STRUCTURAL | claim graph; amplitude/action provenance; FS relation; pre-seal conversion mutations | `DAG; M2; EXACT` Hessian check; inject factor-two/factor-four/later coefficient mutations | FS appears only downstream as a check on the physical amplitude’s Hessian; no `A^c`, multiplicity, or later conversion is selected or silently inserted |
...
| `C-B-V011-MR-02` | STRUCTURAL | statistics grammar/certificate `(G_stats,p_stats)`; one-record restriction functor; expected restriction ledger `E_stats`; closed selector-status record `(S_stats_status,M_stats_status)`; selector proof `p_stats_selector`; one-record-inference query `q_one_record_selector` | `r_enum:=ENUM(G_stats,p_stats)`; `r_ids:=COMPARE(r_enum.ids,E_stats.ids,empty)`; for every `s` in `r_enum.items`, `r_s:=EXACT(restrict_one_record(s))` and `r_cmp_s:=COMPARE(r_s.normal_form,E_stats[s],empty)`; `r_status:=SCHEMA(S_stats_status,M_stats_status)`; `r_selector:=KERNEL(p_stats_selector,proof_axioms)`; `r_selector_cmp:=COMPARE(r_selector.conclusion,M_stats_status,empty)`; `r_query:=M2(q_one_record_selector,selector_sources)` | `P0 and r_enum.success and r_ids.success and r_status.success and r_selector.success and r_selector_cmp.success and r_query.success and r_query.hits=empty and (for every s in r_enum.items: r_s.success and r_cmp_s.success)` |
...
| `C-B-V011-MR-07` | **GATED-EXECUTION** | pinned zero-free-domain proof `p_zero_free`; pinned record-interval derivation `p_tau_R`; expected conclusion ledger `E_zero_free`; analytic response domain; regulator/cell fixture grammar/certificate `(G_fixture,p_fixture)` | after the interval/response gate: `r_zero:=KERNEL(p_zero_free,proof_axioms)`; `r_tau:=KERNEL(p_tau_R,proof_axioms)`; `r_zero_cmp:=COMPARE(r_zero.conclusion,E_zero_free.domain,empty)`; `r_tau_cmp:=COMPARE(r_tau.conclusion,E_zero_free.tau_R,empty)`; `r_domain:=DOMAIN(response,E_zero_free.domain)`; `r_enum:=ENUM(G_fixture,p_fixture)`; for every `f` in `r_enum.items`, `r_f:=SYMBOLIC(containment(E_zero_free.tau_R,E_zero_free.domain,f))` | `P0 and r_zero.success and r_tau.success and r_zero_cmp.success and r_tau_cmp.success and r_domain.success and r_enum.success and (for every f in r_enum.items: r_f.success)` |
...
| `C-B-V011-MR-09` | **GATED-EXECUTION** | handle and full-star projector graph `G_star`; pinned proof `p_full_star_access` with canonical disjunctive goal `derived_source_access or derived_complete_composite_handle`; expected access ledger `E_star`; interval action | after the charged-access gate: `r_type:=TYPE(G_star)`; `r_proof:=KERNEL(p_full_star_access,proof_axioms)`; `r_proof_cmp:=COMPARE(r_proof.conclusion,E_star.access_conclusion,empty)`; `r_one:=EXACT(one_handle_action)`; `r_full:=EXACT(full_star_action)`; `r_one_cmp:=COMPARE(r_one.normal_form,E_star.one_handle_insufficient,empty)`; `r_full_cmp:=COMPARE(r_full.normal_form,E_star.full_star_action,empty)` | `P0 and r_type.success and r_proof.success and r_proof_cmp.success and r_one.success and r_full.success and r_one_cmp.success and r_full_cmp.success` |
...
| `C-B-V011-SP1-02` | STRUCTURAL | boundary data; Lorentz representation; candidate-map grammar/certificate `(G_map,p_map)`; Lorentz-generator grammar/certificate `(G_Lor,p_Lor)`; selected-map derivation `p_selected_map`; expected map/square ledger `E_map` | `r_maps:=ENUM(G_map,p_map)`; `r_lor:=ENUM(G_Lor,p_Lor)`; `r_map_ids:=COMPARE(r_maps.ids,E_map.candidate_ids,empty)`; `r_lor_ids:=COMPARE(r_lor.ids,E_map.generator_ids,empty)`; `r_selected:=KERNEL(p_selected_map,proof_axioms)`; `r_selected_cmp:=COMPARE(r_selected.conclusion,E_map.selected_map,empty)`; `r_type:=TYPE(E_map.selected_map)`; for every `g` in `r_lor.items`, `r_g:=EXACT(Lorentz_square(E_map.selected_map,g))` and `r_cmp_g:=COMPARE(r_g.normal_form,E_map[g],empty)` | `P0 and r_maps.success and r_lor.success and r_map_ids.success and r_lor_ids.success and r_selected.success and r_selected_cmp.success and r_type.success and (for every g in r_lor.items: r_g.success and r_cmp_g.success)` |
...
| `C-B-V011-SP1-05` | STRUCTURAL | source-incidence grammar: positive metrics, intermediates, refinements, covariant columns | `ENUM` grammar with independent completeness certificate; `TYPE` every candidate | the family is exhaustive in the declared class and selection occurs only after enumeration |
...
| `C-B-V011-SP2-03` | **GATED-EXECUTION** | closed SP04 schema/manifest requiring separate `C`, `P`, `CP`, complete scalar/pseudoscalar grammar, axial map and Dirac domain, regulator, Fujikawa Jacobian, determinant ratio, boundary/eta, spectral-pairing output, zero-index branch, and index-one control; grammar/certificate; proof bundles `p_CP`, `p_pair`, `p_measure_sign`; gated spectral inputs; content-addressed expected-result schema/ledger `(S_expected_SP04,E_SP04)` | after anomaly authorization: `r_schema:=SCHEMA(S_SP04,M_SP04)`; `r_expected:=SCHEMA(S_expected_SP04,E_SP04)`; `r_type:=TYPE(G_C_P_CP)`; `r_enum:=ENUM(G_scalar_ps,p_complete)`; `r_domain:=DOMAIN(axial_map,D_Dirac)`; `r_kernel_CP:=KERNEL(p_CP,proof_axioms)`; `r_compare_CP:=COMPARE(r_kernel_CP.conclusion,E_SP04.expected_CP_relation,empty)`; `r_kernel_pair:=KERNEL(p_pair,proof_axioms)` with goal `DOMAIN(gamma5,D) and {D,gamma5}=0 and D psi=lambda psi => D(gamma5 psi)=-lambda(gamma5 psi)`; `r_kernel_measure_sign:=KERNEL(p_measure_sign,proof_axioms)` with exact zero-index measure/sign-equivalence goal; `r_spectral:=SPECTRAL(E_measure_and_index)`; `r_compare_pair:=COMPARE(r_spectral.pairing,r_kernel_pair.conclusion,empty)`; `r_compare_measure:=COMPARE(r_spectral.zero_index_measure_tuple,E_SP04.expected_zero_index_measure_tuple,empty)`; `r_compare_sign:=COMPARE(r_kernel_measure_sign.conclusion,E_SP04.expected_sign_equivalence,empty)`; `r_compare_control:=COMPARE(r_spectral.control,E_SP04.expected_index_one_control,empty)` | `P0 and r_schema.success and r_expected.success and r_type.success and r_enum.success and r_domain.success and r_kernel_CP.success and r_compare_CP.success and r_kernel_pair.success and r_kernel_measure_sign.success and r_spectral.success and r_compare_pair.success and r_compare_measure.success and r_compare_sign.success and r_compare_control.success` |
...
### 8.3 G2 — binding self-audit, all 66 rows

[YOURS] The sweep extracted the exact 66-ID universe from §§3–8 and applied
the V003 binding rule to every row. `BOUND` means the carried V002 row already
routes each load-bearing atom to a receiving opcode result. `RE-RENDERED` names
the unbound atom removed or replaced. This is criterion-language auditing; no
descriptor was executed.
...
| 09 | `C-B-V008-09` | RE-RENDERED | branch completeness lacked `ENUM(grammar,cert)` and enumerated-ID/result comparisons |
| 10 | `C-B-V008-10` | BOUND | `DAG` and `M2` receive every stage, parent, and forbidden-core atom |
| 11 | `C-B-V008-11` | RE-RENDERED | protocol replay and every contamination transition lacked proof/finite binding |
| 12 | `C-B-V009-01` | BOUND | `TYPE` receives fiber inclusions and `M2` receives alias exclusion |
| 13 | `C-B-V009-02` | RE-RENDERED | allowed-equivalence completeness and comparison were unbound |
| 14 | `C-B-V009-03` | BOUND | global/local typing and identities are received by `TYPE` and `EXACT` |
| 15 | `C-B-V009-04` | RE-RENDERED | the domain partition's completeness and dispatch comparison were unbound |
...
| 18 | `C-B-V009-07` | RE-RENDERED | `each mutation fails` had no finite grammar/certificate or expected-result receiver |
...
| 21 | `C-B-V009-10` | RE-RENDERED | one `EXACT` phrase did not bind all basis, parity, null, and residual conjuncts |
...
| 24 | `C-B-V009-13` | RE-RENDERED | eligible-set completeness, selection proof, and failure-family comparisons were unbound |
| 25 | `C-B-V010-01` | RE-RENDERED | `every` post-hoc multiplier had no finite mutation enumeration/comparison |
...
| 37 | `C-B-V010-13` | RE-RENDERED | exhaustive registry, independent custody, selection proof, and all failure transitions lacked complete receivers |
| 38 | `C-B-V010-14` | BOUND | `DAG`, `M2`, and `EXACT` receive provenance, claim, Hessian, and fixed mutation atoms |
...
| 40 | `C-B-V011-MR-02` | RE-RENDERED | statistics completeness, restriction comparison, and selector independence were unbound |
...
| 43 | `C-B-V011-MR-05` | RE-RENDERED | adopted/derived rule provenance and selection from the enumerated family were unbound |
| 44 | `C-B-V011-MR-06` | RE-RENDERED | target-independent selector proof and `silently selects` exclusion lacked receivers |
| 45 | `C-B-V011-MR-07` | RE-RENDERED | certified zero-free domain and independently derived interval were inputs but not `KERNEL` results |
| 46 | `C-B-V011-MR-08` | BOUND | gated `SYMBOLIC` and `TYPE` results receive zero/rejection/provenance atoms |
| 47 | `C-B-V011-MR-09` | RE-RENDERED | the charged-source/composite-handle derivation lacked a pinned proof replay |
| 48 | `C-B-V011-SP1-01` | BOUND | carrier, inner-product, CAR, and surrogate-rejection atoms are `TYPE`/`EXACT` results |
| 49 | `C-B-V011-SP1-02` | RE-RENDERED | the selected map was not derived and the complete Lorentz-generator family was unbound |
...
| 52 | `C-B-V011-SP1-05` | BOUND | the independent completeness certificate is consumed by `ENUM`; each candidate by `TYPE` |
...
| 59 | `C-B-V011-SP2-03` | BOUND | every proof, grammar, spectral field, and expected record has a named receiver |
...
| 66 | `C-D-A35-03-PHYSICAL-RESIDUE` | RE-RENDERED | `derived`, positivity, covariance, branch completeness, and finite-`z` exclusion were unbound |
...
[YOURS] After the 28 in-place replacements, every PASS atom in all 66 rows is
binding-reducible under §2.2. No class assignment changes.
...
[YOURS] The 28 re-rendered rows add content-addressed proof, grammar,
certificate, schema, domain, and expected-result inputs. Therefore exactly
those 28 descriptor hashes change, and consequently the V003 specification
hash, future `check_map_root`, and every future child/verifier manifest and
receipt that binds that root must change. No ID, class, fixture, runtime pin,
or aggregate conjunct changes.
...
exact descriptor hashes = all 66 V003 descriptors, including the 28 re-renders
exact class partition   = 56/10
exact binding board     = 38 BOUND + 28 RE-RENDERED
```

[PROVABLE] Q-559's census/firewall, Q-561's class/runner/runtime-pin verdicts,
and Q-567's binding-class conviction stand. This V003 implements the conviction
and does not relitigate those protected determinations.
...
checks the exact 63 blocker IDs plus 3 discrepancy IDs, all 66 V003 descriptor
  hashes, the 56/10 class partition, and the 38 BOUND + 28 RE-RENDERED
  specification binding board;
...
| criterion prose must reduce to the closed opcode language | all 66 PASS fields, descriptor schemas, verifier expected census | 38 rows are BOUND; 28 are RE-RENDERED with result-bit conjunctions |
...
| 28 descriptor input/procedure/criterion records change | `R1`, `R8`, `R9`, machine ledger, future manifests/receipts | IDs/counts/classes stay fixed; all future roots must bind the V003 bytes |

[YOURS] A future edit after V003 to any blocker description, check predicate, class,
...
[PROVABLE] The V003 66-row binding audit and C-V5 re-diff compare only sealed
text, descriptor structure, opcode bindings, and byte ranges. They execute no
descriptor and form no physical object.
...
| a positive-half proof is weakened to a favorable presence check | every re-rendered derivation is a content-addressed `KERNEL` replay and every completeness claim is an `ENUM(grammar,cert)` result | CLEAN |
| a prior sample is presumed complete | all 66 rows appear individually in §8.3; 38 are BOUND and 28 are RE-RENDERED with the unbound item named | CLEAN |

### 12.3 Surface anchor

[PROVABLE] The geometry-side predicates name the actual sealed objects and
families in the V003 lineage: the finite `K_L`, actual object-fiber
...
[PROVABLE] V003's binding repair consumes actual manifest-native packet
...
### 12.4 M-2 — three guards and all four false-negative modes

[PROVABLE] The V003 sweep used all three guards and tested each of the four
registered false-negative modes explicitly:

1. **Regex-metacharacter mode** (`d^per`): fixed-string searches were run before
   any regex-shaped query. The exact V003 ordinal headings and A35/SP14 phrases,
...
2. **Line-wrap mode:** whitespace-normalized copies joined wrapped clauses and
   ...
   comparing blocker clauses and executable rows. This caught the SP2-05
...
3. **Self-reference/scope mode:** every hit was classified by source role and
   containing section, so V003's own discussion of a phrase could not satisfy
...
The three guards, with all four modes neutralized, reproduce the 63-row census,
the three A35-only sharpenings, the two unnumbered V003 regression conjuncts,
the runtime-pin discrepancy, and the 28-row V003 re-render set. A zero hit
under guards 1–2 was never used as an absence claim.
...
identifiers have zero final-output hits. That is a bounded no-member result for
this V003 byte subject, not an impossibility claim.
...
| `re-rendered` | 28 rows named individually in §8.3 and replaced with closed result-bit criteria |
| `swept 66/66` | the one-row-per-ID ledger in §8.3, whose counts sum to 66 |
| `opcode-reducible` | criterion grammar in §2.2 plus each self-audit disposition in §8.3 |
| `BOUND` / `RE-RENDERED` | exact 66-ID map/board bijection and the 38/28 mechanical totals in §8.3 |
| `restored` | byte-exact SP2-07 blocker quotation plus the zero-hit `M2` predicate at its row |
| `unchanged` | anchor-bounded runtime, firewall, and aggregate hashes in §12.7A match V002 |
| `carriage verified` | raw named V002→V003 diff and exact hunk assignment in §12.7 |

No `implemented`, `executed`, `authorized`, `computed`, `proved physical`, or
`seal attained` verb is used. “PASS iff” states a future machine criterion; it
is not a present verdict.
...
ESCAPE_HATCH = removed (+SP1-07 rebound)
SP2_07 = restored
SP2_06 = carrier_named
BINDING_SWEEP = 66/66 (+38 BOUND / 28 RE-RENDERED: C-B-V008-08,C-B-V008-09,C-B-V008-11,C-B-V009-02,C-B-V009-04,C-B-V009-07,C-B-V009-10,C-B-V009-13,C-B-V010-01,C-B-V010-02,C-B-V010-06,C-B-V010-10,C-B-V010-13,C-B-V011-MR-01,C-B-V011-MR-02,C-B-V011-MR-05,C-B-V011-MR-06,C-B-V011-MR-07,C-B-V011-MR-09,C-B-V011-SP1-02,C-B-V011-SP1-04,C-B-V011-SP1-07,C-B-V011-SP1-08,C-B-V011-SP1-09,C-B-V011-SP2-04,C-B-V011-SP2-06,C-B-V011-SP2-07,C-D-A35-03-PHYSICAL-RESIDUE)
PROTECTED_SECTIONS = unchanged (hashes verify)
CARRIAGE = complete_finite_delta (+21 hunks)
