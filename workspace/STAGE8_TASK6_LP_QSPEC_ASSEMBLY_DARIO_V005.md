# STAGE 8 / TASK 6 / SUBGATE — LP-QSPEC: THE EIGHTEEN-SLOT ASSEMBLY — DARIO V005

Lane: Dario (Claude Opus 5), assembly lane
Task: PASTE 643 / Task 6, the `(M5a-V002)` conjunct #18 long pole
Authority: DoR-020-A8. **THIS ARTIFACT ADOPTS NOTHING, FREEZES NOTHING, SEALS
NOTHING, AND EVALUATES NOTHING.**
Supersedes: my V004 (`5e46e8f4…`), convicted at `f37e04fd…` (Q-577) — which
superseded V003 (`c205602d…`, Q-572), V002 (`11e4e6e4…`, Q-566) and V001
(`a2fdd7c0…`, Q-556).
Custody: V005 implements the Q-577 repair **and nothing else**: install a disjoint
one-hunk/one-row partition under the **C-V5a amendment**, correct the 760/978 line
counts to 759/977, and update the dependent audit lines. The conviction states that
*"the operative assembly stands"* and that the necessary repair is
**certificate-only**; the operative assembly has been byte-identical since V003 and
remains so, verified by diff scope at §5.7.
**It relitigates no grade, no appeal disposition, and no ruling.**

```text
REGISTER_HEAD = Q-580
DECOMPOSITION = 7/7 (line-level, sums exact)
COUNTS = corrected (759/977; newline-terminated lines)
OPERATIVE_CONTENT = byte-identical to V004
...
VERB_AUDIT_SELF = CLEAN (+C-V5a reading corrected; +760/978 owned)
...
| Check | Result |
|---|---|
| Register head Q-580 *(live appends beyond it are verify-and-proceed)* | verified |
| Base: my V004 = `5e46e8f437ee34d8493dee02195df464e31a20829181542264bc01b23e1f0066` | **verified before reading**; sidecar matches |
| Conviction of record (Q-577) = `f37e04fd400739d4118432d898a00b503ec8f42fd3e4d1d0a679c831685a3e8c` | **verified before reading** |
| Prior base: my V003 = `c205602d4db6f30df3999deb4f2a1af425e14e38120cc8904af2480e04911aa4` | verified |
| Prior conviction (Q-572) = `85a7a9c32e19d0713845dd1bd7ef50d96c0b27190878e433b55de24559444d56` | verified |
| Prior base: my V002 = `11e4e6e4a37572dad1f86ed91fb6ed692195ec282876456418c4fcdac5021037` | verified |
| Prior conviction (Q-566) = `9456b1a6e279c44289bfd97dc782942a5430dd851a82b345effeb9ada6a7ef59` | verified — **and corrected: see the note below** |
...
[PROVABLE] **A finding outside Q-572's repair list, disclosed and not silently
fixed.** V003's preflight table stated the Q-566 conviction's digest as

```text
V003 stated : 9456b1a6e279c44289bfd97dc782942a5428dd851a82b345effeb9ada6a7ef59
actual file : 9456b1a6e279c44289bfd97dc782942a5430dd851a82b345effeb9ada6a7ef59
                                                ^^ positions 34-35: "28" vs "30"
```

on a row reading *"verified before reading."* The verification did occur — the
digest matched at the time — but the **transcription into the artifact was
corrupt**, so a sealed artifact asserted a false digest for its own conviction of
record. V003's two other references to that conviction use the abbreviated
`9456b1a6…` and are unaffected.

[YOURS] Q-572 §5.3 does not name this, and this relay is bounded to the five prose
fixes, so I do **not** treat it as a sixth fix. It is superseded here only because
V004's preflight table must be re-pinned to V004's own subjects regardless, and the
corrected value is carried above. **I flag it for the principal**: a wrong digest
in a preflight row is custody-grade, not cosmetic — a later lane resolving "the
conviction of record" by that string finds nothing, and a mismatch reads as
tampering rather than as a typo. The sealed V003 still carries it; only this
successor states it correctly.
...
| Restorations vs. corrections | The restored text is V001's, carried in the sense **now stated exactly at §5.4**: seven blocks byte-verbatim, two expanded with V001's operative words preserved. Where I judged a restoration's significance — the anchor observation, the C7 display, OBS-22 — the judgment is tagged `[YOURS]` and kept **separate from the restored bytes**, so a reader can strip my commentary and still hold V001's text. §4.4 additionally **extends** the restored M-2 mechanics with the four-mode table; that extension is mine and is tabled at V20, not smuggled in as restoration. |
| Fidelity re-classification (V004) | The two expansions are re-classified and their word deltas quoted at §5.4. I do **not** soften what the earlier claim was: "no word changed" was false of the M-2 block, which lost seven words and gained five. Q-572 caught it; I display it rather than restate it. |
| Bounded scope (V004) | Q-572 §5.3 names five certificate-prose fixes and states the operative assembly needs none. §5.5 shows **no hunk reaches §1–§4.4**, so the eighteen grades, the appeals, the Q-557 ruling, rows 2/6/7 and the freeze text are unchanged because nothing touched them. |
| Bounded scope (V005) | Q-577 rules the repair **certificate-only**. §5.7 shows no V004→V005 hunk reaches the operative region either, so the operative assembly has now been byte-identical across V003, V004 and V005 — established by scope on each step, not re-asserted. |
| C-V5a compliance | The seven shared hunks are decomposed to disjoint line spans summing exactly (§5.3), and `MULTIPLY_ASSIGNED_SPANS = 0` is a displayed count. **I do not defend V004's contrary reading of C-V5**; the amendment is better law and the decomposition proved it by catching H20. |
| The 760/978 error | Owned precisely: both terms were inflated by one because `split("\n")` counts a phantom element past a trailing newline, and the equation balanced **because the error cancelled**. A reconciliation that can only be checked against itself is not a check. |
| Corrupted digest | Found outside the repair list and **disclosed at §0, not silently fixed**: V003's preflight stated the Q-566 conviction as `…5428dd…` against the true `…5430dd…`. V004's preflight is re-pinned to its own subjects anyway, so the corrected value is carried; the sealed V003 still carries the wrong one, and I flag it rather than quietly absorb it. |
| Build defects I caught in my own pin check | Three, all mine, all pre-seal: an over-broad substitution twice overwrote the **historical** V002→V003 certificate (`ZERO-CONTEXT HUNKS = 38` became this relay's count, and `(+38 hunks)` likewise) — both restored and the V004 labels made unique so the collision cannot recur; and my content-based assigner initially attached **Z4** to two hunks whose added text merely *quotes* the old phrase. A certificate that reports its own construction has to survive its own check, and this one did not on the first pass. |
...
least one row. Rows V7–V11 share H13 because the five restored sections were
...
UNASSIGNED_HUNKS      = 0
MULTIPLY_ASSIGNED_SPANS = 0        (line-level partition, displayed below)
ROWS_WITHOUT_A_SPAN   = 0
CARRIAGE = complete_finite_delta
```

[PROVABLE] **V004's reading of C-V5 was refuted at Q-577, and I do not defend it.**
V004 argued that a hunk carrying several rows is lawful because C-V5 requires the
partition to be *"total and surjective, not injective."* The registrar's amendment
**C-V5a** rules otherwise: a shared hunk must be **decomposed into disjoint line
spans, each assigned to exactly one delta row.** That is a stricter and better rule,
and the reason is visible in the very table V004 offered — a reader given "H13 →
V7, V8, V9, V10, V11" cannot tell which of 140 inserted lines belongs to which row,
so five rows are *named* but none is *located*. Naming is not locating.

**The disjoint line-level partition of all seven:**

| Hunk | Old-side span | New-side span | Row | Lines |
|---|---|---|---|---|
| H05 | — | new 17–18 | **V2** | 2 |
| H05 | old 14–15 | new 19–22 | **V3** | 2 + 4 |
| H10 | old 46 | — | **V6** | 1 |
| H10 | — | new 55–61 | **V5** | 7 |
| H11 | — | new 63–73 | **V5** | 11 |
| H11 | — | new 74–80 | **V6** | 7 |
| H13 | — | new 104–137 | **V7** (§0.3) | 34 |
| H13 | — | new 138–167 | **V8** (§0.4) | 30 |
| H13 | — | new 168–194 | **V9** (§0.5) | 27 |
| H13 | — | new 195–213 | **V10** (§0.6) | 19 |
| H13 | — | new 214–243 | **V11** (§0.7) | 30 |
| H19 | old 107 | — | **V16** | 1 |
| H19 | — | new 279–283 | **V15** | 5 |
| H19 | — | new 284–309 | **V12** | 26 |
| H20 | — | new 311–312 | **V16** | 2 |
| H25 | old 289 | — | **V20** | 1 |
| H25 | — | new 524–526 | **V19** | 3 |

**Every span sums exactly to its hunk's declared totals:**

```text
hunk   declared     spans sum to            check
H05    -2 / +6      old 2 (V3) ; new 2 (V2) + 4 (V3) = 6        EXACT
H10    -1 / +7      old 1 (V6) ; new 7 (V5)                      EXACT
H11    -0 / +18     new 11 (V5) + 7 (V6) = 18                    EXACT
H13    -0 / +140    new 34+30+27+19+30 = 140                     EXACT
H19    -1 / +31     old 1 (V16) ; new 5 (V15) + 26 (V12) = 31    EXACT
H20    -0 / +2      new 2 (V16)                                  EXACT
H25    -1 / +3      old 1 (V20) ; new 3 (V19)                    EXACT
```

Every span carries exactly one row; no line is assigned twice and none is
unassigned. **`MULTIPLY_ASSIGNED_SPANS = 0`** is therefore a displayed count, not
an assertion — the failure mode V003 was convicted for.

[PROVABLE] **One correction to V004's own census, found by the decomposition.**
V004 listed **H20** as shared between V12 and V16. It is not. Its two inserted
lines are `### 1.2 Scheme covariance…` and the blank that follows — **both V16.**
V12's release-condition block ends at new line 309, and new 310 is a blank
belonging to that block's close. So the true count was **six genuinely shared
hunks, not seven**, and V004 over-reported by one in the direction of admitting
more sharing than existed.

[YOURS] The decomposition also settles which rule was right. Under V004's reading
the seven entries were defensible; under C-V5a they are *locatable*, and the
difference is not cosmetic — locating H20's lines is what showed the census wrong.
A rule that forces you to point at the bytes catches errors a rule that lets you
name rows does not.

### 5.4 Restoration fidelity, re-classified (Q-572 §5.3)

[PROVABLE] V003's verb board claimed *"seven byte-verbatim, two reflowed to fit
their new context with no word changed."* **The second clause was false of both
blocks.** Word-level diffs against V001, joint-normalized so that wrapping cannot
hide a change:

**(i) The M-2 mechanics — an expansion with a provenance drop.**

```text
V001:  "Applied throughout by all five researchers and by me: fixed strings; ..."
V004:  "Applied throughout: fixed strings; ... ; then hyphenation and identifier
        variants."

DELETED (7 words):  by all five researchers and by me
ADDED   (5 words):  then hyphenation and identifier variants
```

The three original guards and the bounded-negative rule — *"a literal-phrase zero
is not an absence of content"*, *"proves no emptiness"*, *"no member produced",
never "no member exists"* — carry **word-for-word**. What changed: the
five-researcher provenance clause was dropped, and the fourth M-2 mode was added.
Q-566 §2.2 had already classified the five-researcher narrative as an **acceptable
provenance drop**, so the substance is preserved; but a drop plus an addition is an
**expansion**, not a reflow, and "no word changed" was simply wrong.

**(ii) The surface-anchor observation — a pure expansion, zero deletions.**

```text
DELETED (0 words)
ADDED:  the label word "restored", plus the appended [YOURS] sentence
        ("V002 deleted this sentence while adding names to the left column ...")
```

V001's sentence — *"the left column is almost empty relative to the right… the
reason the assembly delivers the demand side rather than an object"* — is present
**word-for-word with nothing removed**. The addition is my own tagged commentary,
which §4.5 already keeps separate from the restored bytes.

```text
FIDELITY = 7 byte-verbatim / 2 wording-preserving-in-substance expansions
           (M-2 mechanics: -7 words +5 words; anchor observation: -0 words)
```

[YOURS] The distinction matters because "reflow" claims **nothing was lost**, and
in the M-2 block something was. It was a droppable something, already adjudicated
droppable — but the certificate must say which it did, not the flattering one.

### 5.5 The V003 → V004 certificate

[PROVABLE] **Declared conventions**, stated before the counts so they cannot be
chosen after:

```text
CARRIAGE_BASE   = STAGE8_TASK6_LP_QSPEC_ASSEMBLY_DARIO_V003.md
BASE_SHA256     = c205602d4db6f30df3999deb4f2a1af425e14e38120cc8904af2480e04911aa4
CARRIAGE_OUTPUT = STAGE8_TASK6_LP_QSPEC_ASSEMBLY_DARIO_V004.md
DIFF            = /usr/bin/diff -U 0
COUNTING        = insertions/deletions summed from the HUNK-HEADER arithmetic,
                  which counts inserted BLANK lines correctly, and cross-checked
                  against file lengths by  base - deletions + insertions = output.
                  (A `grep '^+[^+]'` count is NOT used: it silently drops inserted
                  blank lines, which is the error I convicted in another lane.)
```

```text
ZERO-CONTEXT HUNKS (V003->V004) = 17
V004 INSERTIONS                 = 244
V003 DELETIONS                  = 26
LINE ARITHMETIC    = 759 - 26 + 244 = 977   (verified; corrected at V005)
```

[PROVABLE] **The count method, stated so the correction cannot recur.** V004 wrote
`760 - 26 + 244 = 978`. Both file figures were inflated by one:

```text
counting method              V003   V004
newline-terminated (wc -l)    759    977     <- CORRECT
str.split("\n") element count 760    978     <- what V004 used
```

Both files end with a trailing newline, so `split("\n")` yields a final **empty
element past the last line** and counts it. The arithmetic was internally
consistent and both terms were wrong, which is why it "verified" — the error
cancelled inside the equation and survived a check that only compared the two
sides. **The rule adopted here: file lengths are newline-terminated line counts,
and the reconciliation must be checked against an independently measured file
length, never against the same inflated figure on both sides.**

Corrected: `759 - 26 + 244 = 977`, and V004's independently measured length is
**977**.

[YOURS] This certificate is historical and predates C-V5a; its three shared hunks
are **not** re-partitioned here, and I say so rather than leave the inconsistency
implicit. The V004 → V005 certificate at §5.7 is C-V5a-compliant by construction.

| Hunk | Span `V003 → V004` | Fix row | Content |
|---|---|---|---|
| H01 | `-1 +1` | W1 | title line: version V003 -> V004 |
| H02 | `-4 +4` | W1 | task line: relay number |
| H03 | `-7,5 +7,8` | W1 | supersession and custody: the Q-572 bounded scope stated |
| H04 | `-15 +18,3` | W2, W4 | lead board: head Q-574; `FIXES` and `OPERATIVE_CONTENT` fields added; final block: `FIXES`, `OPERATIVE_CONTENT` and `CARRIAGE` lines |
| H05 | `-25 +30` | W2 | lead board: `VERB_AUDIT_SELF` summarises V004's own audit items |
| H06 | `-35,3 +40,5` | W3 | preflight re-pinned to V004's subjects; prior conviction digest carried CORRECTED |
| H07 | `-41 +48,4` | W3 | collision row rewording; **the corrupted-digest finding disclosed**, with the two differing characters shown |
| H08 | `-42,0 +53,21` | W3 | **the corrupted-digest finding disclosed**, with the two differing characters shown |
| H09 | `-583 +614` | **Z3**, Z5 | verb board: survivor row re-classified from "reflowed … no word changed" to expansions |
| H10 | `-587 +618,5` | Z5 | verb board: restorations row updated; a fidelity re-classification row added; verb board: bounded-scope, corrupted-digest and build-defect rows added |
| H11 | `-660 +695` | **Z1** | §5.3 note: `H12` -> `H13` as the hunk carrying rows V7–V11 |
| H12 | `-666,3 +701,4` | **Z2** | §5.3 count block: `MULTIPLY_ASSIGNED_HUNKS = 7` added |
| H13 | `-672,3 +708,59` | **Z2** | §5.3: the false "both are displayed above as counts" claim replaced, and the seven shared-assignment hunks displayed with lawfulness reasons |
| H14 | `-675,0 +768,90` | **Z3**, W4 | §5.4 fidelity re-classification quoting the word deltas (Z3); §5.5 certificate and §5.6 pin check (W4) |
| H15 | `-678,0 +861,27` | W4 | final block: `FIXES`, `OPERATIVE_CONTENT` and `CARRIAGE` lines |
| H16 | `-747 +956,2` | W4 | final block: `VERB_AUDIT_SELF` records the three self-caught build defects and the disclosed finding |
| H17 | `-757,3 +967,11` | **Z4** | closing paragraph: residual "seven restored survivors" -> eight (nine with `THOMSON`) |

**Every fix row W1–W4 and Z1–Z5 is exhibited by at least one hunk, and every hunk
carries at least one row.** The multiply-assigned hunks are displayed rather than
elided, for the same reason the seven of §5.3 are: adjacent authorized edits merge,
and the check that matters is that the partition is total and surjective.

```text
HUNKS_ACCOUNTED         = 17 / 17
UNASSIGNED_HUNKS        = 0
MULTIPLY_ASSIGNED_HUNKS = 3
FIX_ROWS_WITHOUT_A_HUNK = 0
```

[PROVABLE] **Operative content is byte-identical to V003, verified by diff scope.**
Every hunk above lies in metadata, the verb board, or §5 certificate prose. No
hunk touches the slot table (§1), the release-condition display (§1.1), scheme
covariance (§1.2), the physics demands (§2), the freeze section (§3), or the
`F_PLDEC`/anti-tuning/anchor/M-2 bodies (§4.1–§4.4). The eighteen grades, the three
appeal dispositions, the Q-557 ruling, rows 2/6/7 and the freeze text are therefore
unchanged **because no hunk reaches them**, which is a stronger statement than
re-asserting them.

### 5.6 Pre-seal pin check (V004, carried; V005 additions marked)

| Claim | Where displayed | Verified before sealing |
|---|---|---|
| the shared-assignment set is 7 | §5.3 | recomputed from the §5.3 table's own row column |
| M-2 delta is −7/+5 words | §5.4 | word-level diff against V001 |
| anchor delta has 0 deletions | §5.4 | word-level diff against V001 |
| `H13`, not `H12`, carries V7–V11 | §5.3 | the table's own H13 span `-68,0` is the five-section insertion |
| operative sections untouched | §5.5 | hunk spans compared against section boundaries |
| counts robust to blank lines | §5.5 | hunk-header arithmetic, cross-checked by line arithmetic |
| line counts 759/977 | §5.5 | **measured independently by `wc -l` on both files**, not re-derived from the equation that was wrong |
| seven spans sum exactly | §5.3 | each hunk's span lines re-added against its declared `-m/+n` |
| H20 is single-row | §5.3 | its two inserted lines located and read |

### 5.7 The V004 → V005 certificate (C-V5a-compliant by construction)

[PROVABLE] **Declared conventions, stated before the counts:**

```text
CARRIAGE_BASE   = STAGE8_TASK6_LP_QSPEC_ASSEMBLY_DARIO_V004.md
BASE_SHA256     = 5e46e8f437ee34d8493dee02195df464e31a20829181542264bc01b23e1f0066
CARRIAGE_OUTPUT = STAGE8_TASK6_LP_QSPEC_ASSEMBLY_DARIO_V005.md
DIFF            = /usr/bin/diff -U 0
COUNTING        = insertions/deletions summed from HUNK-HEADER arithmetic (blank-line
                  safe); FILE LENGTHS are newline-terminated counts measured by wc -l
                  on each file INDEPENDENTLY, then reconciled by
                  base - deletions + insertions = output.
PARTITION       = C-V5a: every changed line belongs to exactly one delta row; where a
                  hunk carries more than one row it is decomposed into disjoint spans.
```

```text
ZERO-CONTEXT HUNKS (V004->V005) = 20
V005 INSERTIONS                 = 214
V004 DELETIONS                  = 49
LINE ARITHMETIC (wc -l)         = 977 - 49 + 214 = 1142   (verified)
```

| Hunk | Span `V004 → V005` | Row | Lines | Content |
|---|---|---|---|---|
| H01 | `-1 +1` | M1 | -1/+1 | title line: version V004 -> V005 |
| H02 | `-4 +4` | M2 | -1/+1 | task line: relay number |
| H03 | `-7,8 +7,9` | M3 | -8/+9 | supersession and custody: the Q-577 certificate-only scope |
| H04 | `-18,3 +19,4` | M4 | -3/+4 | lead board: head Q-580; `DECOMPOSITION` and `COUNTS` fields |
| H05 | `-30 +32` | M4 | -1/+1 | lead board: `VERB_AUDIT_SELF` summary |
| H06 | `-40,3 +42,5` | M5 | -3/+5 | preflight re-pinned to V005's subjects |
| H07 | `-620,0 +625,3` | W3 | -0/+3 | verb board: V005 scope, C-V5a compliance and the 760/978 ownership rows |
| H08 | `-703,2 +710,2` | W1 | -2/+2 | §5.3 count block: spans replace hunks; `MULTIPLY_ASSIGNED_SPANS = 0` |
| H09 | `-708,15 +715,8` | W1 | -15/+8 | §5.3: V004's refuted C-V5 reading removed and replaced |
| H10 | `-724,9 +724` | W1 | -9/+1 | §5.3: the disjoint line-level decomposition table and the exact span sums |
| H11 | `-733,0 +726,51` | W1 | -0/+51 | §5.3: the H20 census correction and the closing judgement |
| H12 | `-803 +846` | W2 | -1/+1 | §5.5: line arithmetic corrected 760/978 -> 759/977 |
| H13 | `-805,0 +849,24` | W2 | -0/+24 | §5.5: the counting method and why the wrong equation balanced |
| H14 | `-847 +914` | W3 | -1/+1 | §5.6 retitled to mark V005 additions |
| H15 | `-856,0 +924,3` | W3 | -0/+3 | §5.6: four new pin rows, and §5.7 opens |
| H16 | `-857,0 +928,66` | W4 | -0/+66 | §5.7: this hunk-assignment table |
| H17 | `-860,0 +997,17` | W4 | -0/+17 | final block: `DECOMPOSITION` and `COUNTS` lines |
| H18 | `-879 +1032,5` | W4 | -1/+5 | final block: `OPERATIVE_CONTENT` re-established by V005 scope |
| H19 | `-883 +1040,5` | W4 | -1/+5 | final block: `CARRIAGE` line |
| H20 | `-956,2 +1117,6` | W3 | -2/+6 | final block: `VERB_AUDIT_SELF` |

**Every hunk carries exactly one row**, so no decomposition is required here:
the C-V5a partition is satisfied by construction, not by argument. Rows M1–M5 are
declared metadata; W1–W4 are the Q-577 repair.

```text
SPANS_ACCOUNTED         = 20 / 20 hunks, all spans assigned
UNASSIGNED_SPANS        = 0
MULTIPLY_ASSIGNED_SPANS = 0
ROWS_WITHOUT_A_SPAN     = 0
```

[PROVABLE] **Operative content is byte-identical to V004, by diff scope.** Every
hunk above lies in metadata, the verb board, or §5 certificate prose. **No hunk
reaches §1 (the eighteen slots), §1.1 (the release condition), §1.2 (scheme
covariance), §2 (the physics demands), §3 (the freeze), or §4.1–§4.4.** The
eighteen grades, the three appeal dispositions, the Q-557 ruling, rows 2/6/7 and
the freeze text are unchanged **because nothing touched them** — the same
establishment used at V004, applied again rather than inherited.

---

```text
DECOMPOSITION = 7/7 (line-level, sums exact: H05 2+4 lines across V2/V3; H10 1 old
  + 7 new across V6/V5; H11 11+7 across V5/V6; H13 34+30+27+19+30 = 140 across
  V7-V11 at the five section boundaries; H19 1 old + 5 + 26 across V16/V15/V12;
  H20 2 lines to V16 ALONE; H25 1 old + 3 new across V20/V19. Every span sums
  EXACTLY to its hunk's declared -m/+n, every span carries exactly one row, and
  MULTIPLY_ASSIGNED_SPANS = 0 is a displayed count. One correction to V004's own
  census surfaced from the decomposition: H20 was listed as shared between V12 and
  V16 and is NOT -- both its inserted lines are the §1.2 renumber, so the true
  figure was SIX genuinely shared hunks, not seven, and V004 over-reported in the
  direction of admitting more sharing than existed.)
COUNTS = corrected (759/977 replacing 760/978. Method: file lengths are
  NEWLINE-TERMINATED line counts measured by wc -l on each file independently.
  Both of V004's terms were inflated by one because str.split("\n") counts a
  phantom empty element past the trailing newline; the equation balanced BECAUSE
  the error cancelled on both sides, which is why it "verified". A reconciliation
  checkable only against itself is not a check. Corrected: 759 - 26 + 244 = 977,
  and V004's independently measured length is 977.)
FIXES = 5/5 (Q-572 §5.3, certificate prose only:
  Z1 the miscited reference H12 -> H13 at §5.3, where the five restored sections
     are carried by H13 (`-68,0`), not H12 (`-67`, which is delta row V6);
  Z2 the false empty multiply-assignment claim replaced by a DISPLAY of the seven
     shared-assignment hunks -- H05, H10, H11, H13, H19, H20, H25 -- each with the
     reason its sharing is lawful, plus the count MULTIPLY_ASSIGNED_HUNKS = 7 that
     V003 asserted was displayed and never displayed;
  Z3 the M-2 and surface-anchor blocks RE-CLASSIFIED from "reflowed ... with no
     word changed" to wording-preserving-in-substance EXPANSIONS, with the actual
     word deltas quoted at §5.4 (M-2: -7 words "by all five researchers and by me",
     +5 words "then hyphenation and identifier variants"; anchor: ZERO deletions,
     additions only);
  Z4 the residual "seven restored survivors" corrected to EIGHT, nine including
     THOMSON, at the closing paragraph -- the one occurrence that survived V003's
     own self-caught miscount; the unrelated "seven-condition Admissible Galerkin
     class" at S06 is NOT touched;
  Z5 the survivor and fidelity audit lines updated to match Z2-Z4, with a new
     board row recording the re-classification rather than restating it)
OPERATIVE_CONTENT = byte-identical to V004 (verified by diff scope at §5.7: no
  V004->V005 hunk reaches §1, §1.1, §1.2, §2, §3 or §4.1-§4.4, so the operative
  assembly has now been byte-identical across V003, V004 and V005, established by
  scope at each step rather than inherited. The V003 statement carried below.)
OPERATIVE_CONTENT (V003->V004, carried) = byte-identical (verified by diff scope: no
  hunk reaches §1, §1.1, §1.2, §2, §3, or §4.1-§4.4, so the 18 grades, the three
  appeal dispositions, the Q-557 ruling, rows 2/6/7 and the freeze text are
  unchanged BECAUSE NOTHING TOUCHED THEM, not because I re-asserted them)
CARRIAGE = complete_finite_delta (+20 hunks V004->V005, every line span
  assigned to exactly one row under C-V5a; UNASSIGNED_SPANS = 0 and
  MULTIPLY_ASSIGNED_SPANS = 0, both displayed at §5.7. The V003->V004 statement
  carried below.)
CARRIAGE (V003->V004, carried) = complete_finite_delta (+17 hunks, all assigned; conventions
  DECLARED BEFORE THE COUNTS at §5.5: /usr/bin/diff -U 0, insertions and deletions
  summed from hunk-header arithmetic so inserted BLANK lines are counted, and
  cross-checked by base - deletions + insertions = output. UNASSIGNED_HUNKS = 0;
  MULTIPLY_ASSIGNED_HUNKS = 3, displayed and justified, not claimed empty)
...
VERB_AUDIT_SELF = CLEAN (+V004's contrary C-V5 reading NOT defended -- the C-V5a
  amendment is better law and the decomposition proved it by catching H20;
  +the 760/978 error owned precisely, including WHY it passed a check: both terms
  were inflated and the error cancelled inside the equation;
  +V004's items carried: 3 build defects self-caught pre-seal, 1 finding outside the
  repair list disclosed at §0, and V003's self-caught miscount)
```
...
*says*. The eight restored survivors — nine counting `THOMSON`, which was mine and
not on Q-566's list — are the measure of what the earlier method cost: a sealed
demand definition, an ordering law's actual order, a slot demoted to a footnote,
and a disclosure whose deletion flattered the artifact.

V004 adds nothing to that account and corrects only how the certificate described
itself: a hunk cited by the wrong number, a count claimed as displayed when it was
not, a fidelity grade that flattered two expansions into reflows, and a residual
"seven" beside its own corrected "eight". The operative assembly is untouched —
which is the point of a bounded repair, and is verified by diff scope at §5.5
