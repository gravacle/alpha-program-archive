# STAGE8 TASK 6 / A35–RD-22 — evaluator specification V005

**Artifact:** `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V005.md`  
**Lane:** Codex Lane 2  
**Date:** 2026-08-07  
**Custody:** BR-1/A regime installation and ten-row closed-list repair by Lane 2; adjudication and closed scope by Dario; any RD-22 execution authorization remains a principal act  
**Status:** V005 FINITE-DELTA SPECIFICATION ONLY — BR-1/A is ADJUDICATED, BR-2 is MECHANICAL, ten descriptors are repaired, and the six adjudicated-lawful descriptors carry byte-unchanged; no implementation, physical check, fixture, physical quantity, fixed point, end test, or authorization is executed here

```text
BR1A = installed (ADJUDICATED regime)
TEN = 10/10 repaired
BR1A_RECORD = candidates displayed, all adjudicated (+12 rows / 29 atoms / 54 zero-candidate rows)
BR2_SWEEP = 0 failures (per-row coverage displayed)
PROTECTED_SECTIONS = unchanged
CARRIAGE = complete_finite_delta (+26 hunks)
...
[PART-PROVABLE] The commissioned live preflight identifies register head `Q-580`;
that entry records the BR-1 adjudication and commissions its ten-row V005 repair.
The readable local snapshot does not yet expose that append, so Q-580 is accepted
only under the stated live-append tolerance; no conflicting local entry was found.

[PROVABLE] Before V005 was written, neither its requested filename nor its
protocol sidecar existed in the cleanroom or the archive workspace. V004 and
its sealed sidecar remained intact. V005 is a byte-copy-plus-named-replacements
