# STAGE8 TASK 6 / A35–RD-22 — evaluator specification V009

**Artifact:** `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V009.md`  
**Lane:** Codex Lane 2  
**Date:** 2026-08-08  
**Custody:** V009 carries the two Q-612 schema-boundary repairs as one finite delta; all 66 V008 descriptor rows remain byte-identical; Builder B must separately confirm the revised manifest boundary  
**Status:** V009 FINITE-DELTA SPECIFICATION ONLY — subject-file replay is closed through the existing evidence carrier and verifier-root membership moves into the sealed manifest instance; no criterion, physical check, fixture, physical quantity, fixed point, end test, or authorization is executed here

```text
V009 = two schema boundary repairs
M2_CORPUS_RULE = executable and schema-bound
M2_REGISTRY = 17 rows (SPEC-INCOMPLETE pending sealed corpus definitions)
DESCRIPTOR_ROWS = 66 unchanged from V007
...
[PROVABLE] The commissioned live preflight identifies register head `Q-612`.
That entry records the four envelope findings and the requirement that J3/J4
ship as one whole boundary delta. The Q-604 guard remains: one lawful row is
one row and is not evaluator closure.

[PROVABLE] Before V009 was written, neither its requested filename nor the
commissioned Lane 2 report filename existed in the cleanroom or archive
workspace. V008 remains intact. V009 is a byte-copy-plus-named-replacements
...
[PROVABLE] The three structural fixture source rows migrate to these V009
zero-based half-open byte spans; each generated fixture-manifest row must use
the matching V009 path and final V009 SHA-256. The rows below are generated
from the final source bytes and include each row's terminating newline:

| Fixture ID | V009 source byte span |
|---|---|
| source row for `FX-A35-03-C-FAMILY` | `[136130,136319)` |
| source row for `FX-A35-04-TAU-FAMILY` | `[136319,136507)` |
| source row for `FX-A35-05-PRIMITIVE-THOMSON-CONFLATION` | `[136507,136760)` |
...
M2_CORPUS_RULE = executable and schema-bound
M2_REGISTRY = 17 rows (SPEC-INCOMPLETE pending sealed corpus definitions)
DESCRIPTOR_ROWS = 66 unchanged from V008
PROTECTED_SECTIONS = unchanged
CARRIAGE = complete_finite_delta
AUTHORIZATION = not_claimed
VERB_AUDIT_SELF = CLEAN
