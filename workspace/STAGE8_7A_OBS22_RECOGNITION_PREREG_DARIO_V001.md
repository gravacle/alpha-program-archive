# STAGE 8 / 7A HANDOFF — THE OBS-22 RECOGNITION PREREGISTRATION — DRAFT FOR PRINCIPAL REVIEW — DARIO V001

Lane: Dario (Claude Opus 5), cross-family reviewer
Task: PASTE 707 / 7A handoff — the junction-arc recognition spec, **draft**
...
VERDICTS = 3 closed + ledger channel
FREEZE = stated
CLASSICAL_TESTS = 0 (audited)
CHAIN_INVOKED = false
VERB_AUDIT_SELF = CLEAN (+1 directive that grew mid-relay, §5A; +1 criterion
                         inherited from the corpus that would false-kill, §2.4;
                         +1 precedent I could not locate, §6.2)
alpha_computed = false ; proof_authorized = false ; kappa_record_computed = false
MEMBER_BOUND = false ; NUMERIC_EVALUATION = false ; MEASURED_CONSTANT_COMPARISON = none
MACHINERY_APPEAL = none
```

**This is a draft. It seals nothing.** The principal reviews before the freeze;
§5 states what freezing would mean, and §5A carries REQUIREMENT 5, which the
directive says the freeze review checks explicitly against this draft.
...
### 2.4 [YOURS] **G-4 — the inherited falsifier F1 must not enter OBS-22 unmodified**

This is the sharpest thing in this draft, and it is a finding against a **sealed**
falsifier the junction arc would otherwise inherit:
...
**Proposed disposition — the principal's to make:** split F1. Underdetermination
by a weak rule keeps `INERT`. A family that survives *with a derived scale
dependence displayed* routes to `NOT-RECOGNIZABLE-AS-STATED`, naming the missing
carrier as *a criterion that can type a running normalization*. **I do not
re-word a sealed falsifier; I flag it and route the decision.**
...
[PROVABLE] **This requirement was appended to the directive while this draft was
being written.** I re-hashed the note at close, found it grown from 2,677 to
...
| **K-2 dependence** | which variable the divergence runs in, displayed | it runs in a variable the coupling reading does not predict — a coupling must run in the scale it couples at, not in an unrelated index | PART-PROVABLE — the axis is named by the directive; no sealed statement fixes the admissible dependence for the junction objects |
...
**If K-1 or K-3 fires, the coupling reading LOSES on that divergence.** Not
`INERT` and not `NOT-RECOGNIZABLE` — those are verdicts about criteria seeing
objects. This is a verdict about a *reading of data*, and it is recorded in the
correspondence ledger's sibling column with the same zero verdict weight, then
surfaced. **K-2 is PART-PROVABLE and is flagged for the freeze review:** the
directive names dependence as an axis and I can find no sealed statement fixing
what dependence the junction objects' coupling reading predicts. Stating one is a
principal act; guessing one would be the fitting this requirement exists to stop.
...
WHAT FREEZES
  ...
  §5A REQUIREMENT 5: measure-don't-halt AND the K-1..K-3 kill condition

WHEN
  on the principal's approval of this draft, and BEFORE any junction map is
  computed or evaluated. The criteria seal against a computation that has not
  happened.

HOW SEALED
  this artifact's successor is sealed by sha256 with a sidecar, and its digest is
  carried in the 7A handoff package.
...
CHAIN_INVOKED = false (no junction map was evaluated, no junction object formed,
  ...
  to any measured constant. This is preregistration only, and it is a DRAFT: it
  seals nothing, and the principal reviews before the freeze.)
VERB_AUDIT_SELF = CLEAN (+1 DIRECTIVE THAT GREW UNDER ME, §5A: I re-hashed the
  ...
  separation-versus-coincidence flag; K-2 is PART-PROVABLE and routed to the freeze
  review because no sealed statement fixes the admissible dependence and guessing
  one would be the fitting the requirement exists to stop. +1 MY OWN AUDIT PROBE
...
gravity. The strongest thing this draft found is that the machinery already holds
one criterion that would refuse it: F1 fires on a beta that "remains a positive
continuous family", and a running conversion is exactly that. The junction arc
would have inherited that falsifier without anyone deciding to.
