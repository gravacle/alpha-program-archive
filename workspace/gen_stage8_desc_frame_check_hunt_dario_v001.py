#!/usr/bin/env python3
"""
SEALED GENERATOR — STAGE8_DESC_FRAME_CHECK_HUNT_DARIO_V001.md   (relay 1087, DARIO lane)
...
ART = WS / "STAGE8_DESC_FRAME_CHECK_HUNT_DARIO_V001.md"; SIDE = Path(str(ART)+".seal.sha256")
SELF = Path(__file__)
...
def main():
    ...
    # R9 a TYPE read must certify that it evaluated nothing, and must say where it stopped
    if "EVALUATED_NOTHING = CERTIFIED" not in t:
        return fail(9,"a type-read hunt must certify that it evaluated nothing")
    if "WHERE I STOPPED READING" not in t:
        return fail(9,"a type read into an evaluator-adjacent spec must record where it stopped")
    print("TYPE_READ_GATE = CLEAN (evaluated-nothing certified; the stopping point is recorded)")

    # R10 the frame ruling must carry its limit, and must not sweep away what it does not dissolve
    if "COUPLING_FRAME = NON-EXPECTATION" in t:
        need={"the not-survived limit":"NOT that the wash-out has been survived",
              "what stays standing":"WHAT THE REFRAME DISSOLVES, AND WHAT IT LEAVES STANDING",
              "the moved gap":"MOVES the gap"}
        missing=[k for k,v in need.items() if v not in t]
        if missing:
            return fail(10,"the reframe is the ruling that dissolves this lane's recent work and "
                           f"may not seal without {missing}")
        print("REFRAME_GATE = CLEAN (limit stated, standing items preserved, gap relocated)")
    if "WASHOUT_STATUS = NOT-FATAL" in t and "is an evaluation" not in t:
        return fail(10,"a NOT-FATAL ruling must name the evaluation it is not making")
    print("WASHOUT_GATE = CLEAN")
