# STAGE8 TASK 6 / A35–RD-22 — evaluator specification V004

**Artifact:** `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V004.md`  
**Lane:** Codex Lane 2  
**Date:** 2026-08-07  
**Custody:** BR-1/BR-2 law installation and closed-list repair by Lane 2; correction and closed scope by Dario; any RD-22 execution authorization remains a principal act  
**Status:** V004 FINITE-DELTA SPECIFICATION ONLY — BR-1/BR-2 are installed as law; 58 descriptors carry without V004 row edits and 8 closed-list descriptors are repaired; no implementation, check, fixture, physical quantity, fixed point, end test, or authorization is executed here

```text
BR_LAWS = installed (+BR1_DIRECTION_CONFORMANCE_V001, BR2_REDUCTION_TRUTH_CONFORMANCE_V001)
CLOSED_LIST = 11/11 repaired
BR1_SWEEP = 0 failures
BR2_SWEEP = 0 failures
PROTECTED_SECTIONS = unchanged
CARRIAGE = complete_finite_delta (+26 hunks, conventions declared)
...
head is `Q-574`; that entry records the corrected closed V004 work list and the
two proposed binding rules. The commissioned live-append state is therefore
exact.

[PROVABLE] Before V004 was written, neither its requested filename nor its
protocol sidecar existed in the cleanroom or the archive workspace. V003 and
its sealed sidecar remained intact. V004 is a byte-copy-plus-named-replacements
...
The spec-fixed BR values used below are the canonical tokens `REJECTED`, `FAILS`,
`CONTAINED`, and `INTERTWINER_IDENTITY`. They are constants in this specification,
not producer fields. `ASSERTION_FREE_FORMATION(r)` is descriptor metadata, not an
opcode or Boolean: it is lawful only when `r.success` asserts formation/reduction
and no criterion atom consumes `r`'s value.

The two conformance procedures are named and closed:

```text
BR1_DIRECTION_CONFORMANCE_V001(spec_bytes):
  rows := strict_parse_the_66_descriptor_rows(spec_bytes)
  for each row in stable-ID order:
    atoms := backward_slices_from_each_PASS_conjunct_to_its_requirement_atom
    for each atom whose slice consumes CONTENT from an E_* or M_* value:
      accept only if the same slice contains exactly one BR-1(a), BR-1(b),
      or BR-1(c) forcer; a computed-to-producer COMPARE is not a forcer
      unless the KERNEL conclusion in BR-1(b) supplies the direction
    emit {check_id,atom,producer_object,slice,missing_forcer} for every failure
  return failures sorted by {check_id,atom,producer_object}
...
Both procedures consume the whole final file, not a repair-row allowlist. A
self-consistency comparison may remain as evidence, but it is never counted as
the directional receiver. A row outside the closed V004 list returned by either
procedure is a finding and may not be silently edited.
...
### 8.3 G2 — V004 closed-list binding board, all 66 rows

[YOURS] The board retains the exact 66-ID universe from §§3–8 and applies BR-1
and BR-2 to the final file. `BOUND` means the V003 row carries unchanged into
V004; `RE-RENDERED` means the V003 re-render was among the correction's 22
confirmed rows and carries unchanged; `V004-REPAIRED` is one of the eight
descriptor rows in the closed V004 list. This is criterion-language auditing;
no descriptor was executed. It does not renew the withdrawn claim that all 28
V003 re-renders received row-by-row semantic blocker verification.
...
```text
CARRIED BOUND = 36
CARRIED RE-RENDERED = 22
V004-REPAIRED = 8
TOTAL = 66
```

[YOURS] `BR1_DIRECTION_CONFORMANCE_V001` and
`BR2_REDUCTION_TRUTH_CONFORMANCE_V001` each scan all 66 final descriptor rows and
return zero failures (§8.6). That is a mechanical BR-conformance verdict at the
scope stated here, not a renewed 66-row semantic blocker review. No class
assignment changes.
...
[YOURS] Exactly eight V003 descriptor rows change in V004: `C-B-V009-04`,
`C-B-V010-01`, `C-B-V010-13`, `C-B-V010-14`, `C-B-V011-MR-05`,
`C-B-V011-MR-07`, `C-B-V011-SP1-02`, and `C-B-V011-SP1-05`. Consequently the
V004 specification hash, future `check_map_root`, and every future
child/verifier manifest and receipt that binds that root must change. No ID,
class, fixture, runtime pin, or aggregate conjunct changes.
...
exact descriptor hashes = all 66 V004 descriptors, including the 8 V004 repairs
exact class partition   = 56/10
exact binding board     = 36 carried BOUND + 22 carried RE-RENDERED + 8 V004-REPAIRED
```

[PROVABLE] Q-559's census/firewall, Q-561's class/runner/runtime-pin verdicts,
Q-567's binding-class conviction, and Q-574's corrected closed scope stand. This
V004 installs the two class laws and renders only the closed repairs; it does
not relitigate the protected determinations.

### 8.5 Closed V004 work-list disposition

| # | Subject | Installed repair | Mechanical verification | Status |
|---:|---|---|---|---|
| 1 | `C-B-V009-04` | conjoin all three `DOMAIN.success` bits plus spec-forced `r_distinct_x` and `r_one_x` | fixed receiver names in procedure and PASS; `E_log` absent | REPAIRED |
| 2 | `C-B-V011-MR-07` | use replayed proof conclusions in `DOMAIN`/containment; pair `r_f` with `COMPARE(r_f.result,CONTAINED,empty)` | paired reduction and comparison both conjoined | REPAIRED |
| 3 | `C-B-V011-SP1-05` | bind completeness to `r_enum.success`; bind ordering to `r_order.hits=empty`; type every enumerated candidate | `ENUM`, `M2`, `TYPE`, and all result conjuncts present | REPAIRED |
| 4 | `C-B-V010-01` | compare every enumerated factor classification to spec-fixed `REJECTED` | `E_factor` absent; `REJECTED` comparison conjoined | REPAIRED |
| 5 | `C-B-V010-13` independence | add a pinned `r_indep:=KERNEL(p_independence,proof_axioms)` with the principal-inequality goal | proof result conjoined | REPAIRED |
| 6 | `C-B-V010-13` contamination/drift | compare every enumerated result to spec-fixed `FAILS` | `E_holdout[m]` absent; `FAILS` comparison conjoined | REPAIRED |
| 7 | `C-B-V011-MR-05` | add `r_distinct:=EXACT(pairwise_distinct(...))` | result exists in procedure and PASS | REPAIRED |
| 8 | `C-B-V011-SP1-02` | compare every Lorentz square to spec-fixed `INTERTWINER_IDENTITY` | `E_map[g]` absent; fixed identity comparison conjoined | REPAIRED |
| 9 | `C-B-V010-14` | replace bare “silently inserted” with named `r_m2.success and r_m2.hits=empty` | fixed-string receiver present in PASS cell | REPAIRED |
| 10 | historical V003 certificate | correct `FINAL_INSERTIONS` from 248 to 261 | blank-line-safe diff count and `1392 - 163 + 261 = 1490` | REPAIRED |
| 11 | `B-V011-SP2-07` requirement-of-record | restore “Every load-bearing audit must be free of Python `assert`” | fixed-string equality with census bytes 435–437 | REPAIRED |

[PROVABLE] The optional V010-05 fixture-coverage suggestion was not charged and
is not implemented. Descriptor-row diffing returns exactly the eight row IDs in
items 1–9 above (item 5 and item 6 share one row); no row outside the closed list
changes and neither BR procedure returns an outside-list finding.

### 8.6 BR conformance transcript over final V004

Declared search convention: descriptor tables are parsed as one logical Markdown
row per stable ID; fixed strings precede regex; a producer expectation used only
for self-consistency is not credited as a directional forcer; result dependency
is traced from the `PASS iff` cell backward through the ordered procedure.

```text
BR1_DIRECTION_CONFORMANCE_V001
  descriptor_rows_scanned = 66
  class_partition         = 56 STRUCTURAL / 10 GATED-EXECUTION
  repaired_directional_slices =
    C-B-V009-04       -> EXACT(pairwise_disjoint), EXACT(exactly_one_in_domain)
    C-B-V010-01       -> COMPARE(classification,REJECTED,empty)
    C-B-V010-13       -> KERNEL(principal inequality), COMPARE(result,FAILS,empty)
    C-B-V011-MR-05    -> EXACT(pairwise_distinct)
    C-B-V011-MR-07    -> KERNEL conclusions + COMPARE(result,CONTAINED,empty)
    C-B-V011-SP1-02   -> COMPARE(normal_form,INTERTWINER_IDENTITY,empty)
  outside_closed_list_findings = []
  failures = []
  BR1_SWEEP = 0 failures

BR2_REDUCTION_TRUTH_CONFORMANCE_V001
  descriptor_rows_scanned = 66
  reduction_assignments_scanned = 8 across 7 rows
  assertion_free_bare_success = [C-B-V010-01:r_response]
  repaired_relation_pair      = [C-B-V011-MR-07:r_f->r_cmp_f]
  other_received_reductions   = [C-B-V010-01:r_m,
                                 C-B-V011-SP1-04:r_spectral_p,
                                 C-B-V011-SP1-08:r_spectral,
                                 C-B-V011-SP2-03:r_spectral,
                                 C-B-V011-SP2-06:r_spectral,
                                 C-D-A35-03-PHYSICAL-RESIDUE:r_spectral]
  unreceived_relation_success = []
  outside_closed_list_findings = []
  failures = []
  BR2_SWEEP = 0 failures
```

[PROVABLE] These are mechanical source-language results. They do not execute a
descriptor and do not assert the truth of a physical relation. The verdict lines
claim BR conformance only; they do not reinstate the withdrawn V003 semantic
`28/28 CONFIRMED` headline.
...
checks the exact 63 blocker IDs plus 3 discrepancy IDs, all 66 V004 descriptor
  hashes, the 56/10 class partition, and the 36 carried BOUND + 22 carried
  RE-RENDERED + 8 V004-REPAIRED specification binding board;
...
| criterion prose must reduce to the closed opcode language | all 66 PASS fields, descriptor schemas, verifier expected census | 36 rows carry BOUND; 22 V003 re-renders carry; 8 closed-list rows are V004-REPAIRED |
| producer expectations cannot carry direction and reduction success is not truth | BR-1/BR-2 law text, both named full-file procedures, all 66 descriptor rows | both final conformance sweeps return zero failures; no outside-list descriptor edit |
...
| 8 V004 descriptor input/procedure/criterion records change | `R1`, `R8`, `R9`, machine ledger, future manifests/receipts | IDs/counts/classes stay fixed; all future roots must bind the V004 bytes |

[YOURS] A future edit after V004 to any blocker description, check predicate, class,
...
[PROVABLE] The V004 closed-list/BR audit and C-V5 re-diff compare only sealed
...
| a producer expectation fabricates the favorable direction | BR-1 requires a value-fixing schema, proof conclusion, or spec-fixed constant; the final 66-row procedure returns zero failures | CLEAN |
...
| a prior sample is presumed complete | all 66 rows appear individually in §8.3; the verdict is scoped to 58 carried rows plus 8 closed-list repairs and two full-file BR tests | CLEAN |

### 12.3 Surface anchor

[PROVABLE] The geometry-side predicates name the actual sealed objects and
families in the V004 lineage: the finite `K_L`, actual object-fiber
...
[PROVABLE] V004 carries V003's binding repair over actual manifest-native packet
...
response inhabitants. V004 adds only the two binding laws and the eleven
closed-list repairs recorded in §8.5.
...
[PROVABLE] The V004 closed-list and BR-conformance sweep used all three guards
and tested each of the four registered false-negative modes explicitly:

1. **Regex-metacharacter mode** (`d^per`): fixed-string searches were run before
   any regex-shaped query. The exact V004 ordinal headings and A35/SP14 phrases,
...
3. **Self-reference/scope mode:** every hit was classified by source role and
   containing section, so V004's own discussion of a phrase could not satisfy
...
The three guards, with all four modes neutralized, preserve the 63-row census,
the three A35-only sharpenings, the two unnumbered V003 regression conjuncts,
the runtime-pin discrepancy, and the 66-row V004 board. They also identify the
eight changed descriptor rows and distinguish them from the eleven closed-list
items. A zero hit under guards 1–2 was never used as an absence claim.
...
V004-specific fixed strings `REJECTED`, `FAILS`, `CONTAINED`,
...
`producer-declared`, `spec-fixed`, and `assertion-free`. That is a bounded
source-structure result for this V004 byte subject, not an impossibility claim.
...
| `re-rendered` | eight V004 descriptor rows named individually in §8.3 and §8.5; the other 58 retain their V003 descriptor bytes |
| `swept 66/66` | the one-row-per-ID ledger in §8.3 plus the two named mechanical conformance procedures and transcript in §8.6 |
| `opcode-reducible` | criterion grammar in §2.2 plus each self-audit disposition in §8.3 |
| `BOUND` / `RE-RENDERED` / `V004-REPAIRED` | exact 66-ID map/board bijection and the scoped 36/22/8 mechanical totals in §8.3 |
| `restored` | byte-exact SP2-07 blocker quotation plus the zero-hit `M2` predicate at its row |
| `unchanged` | anchor-bounded runtime, firewall, and aggregate hashes in §12.7B match V003 |
| `carriage verified` | raw named V003→V004 diff, declared conventions, and exact hunk assignment in §12.7B |
| `zero failures` | source-structure results of the named BR-1 and BR-2 procedures over all 66 final rows in §8.6; not a physical execution verdict |
