#!/usr/bin/env python3
"""
SEALED GENERATOR — STAGE8_CONTINUUM_TARGET_INDEPENDENCE_GATE_DARIO_V001.md   (relay 1091, DARIO lane)
...
ART = WS / "STAGE8_CONTINUUM_TARGET_INDEPENDENCE_GATE_DARIO_V001.md"; SIDE = Path(str(ART)+".seal.sha256")
SELF = Path(__file__)
...
def main():
    ...
    # R10 an adverse verdict must be surfaced raw, must name its knobs, and must carry remedies;
    # a clearing verdict is held to a HIGHER bar, since clearing authorises an alpha computation.
    if "VERDICT = HAS-ALPHA-KNOB" in t:
        need={"the raw surfacing":"SURFACED RAW",
              "the named knobs":"ALPHA_REACHING_KNOBS =",
              "the remedies":"WHAT WOULD CHANGE THE VERDICT",
              "the reservation to the principal":"I DO NOT RULE ON WHETHER ROUTE"}
        missing=[k for k,v in need.items() if v not in t]
        if missing: return fail(10,f"an adverse gate verdict may not seal without {missing}")
        print("ADVERSE_VERDICT_GATE = CLEAN (raw, knobs named, remedies given, ruling reserved)")
    if "VERDICT = TARGET-INDEPENDENT" in t:
        if "ALPHA_REACHING_KNOBS = NONE" not in t:
            return fail(10,"a clearing verdict must state that no ingredient reaches alpha")
        if "UNIQUE-REQUIRE" not in t:
            return fail(10,"a clearing verdict must show every alpha-reaching ingredient pinned")
        print("CLEARING_GATE = CLEAN")

    # R11 the allow/require table must exist and must not be empty of ALLOWED-vs-REQUIRE rulings
    if "ALLOW_REQUIRE_TABLE =" not in t:
        return fail(11,"the per-ingredient allow/require table must be published")
    if "PACKAGE_ENUMERATED =" not in t:
        return fail(11,"the package enumeration must be published with spans")
    print("TABLE_GATE = CLEAN (package enumerated; allow/require table published)")

    # R12 a carried premise may not be dressed as verified
    if "CARRIED, NOT VERIFIED AT BYTES" not in t and "CARRIED-UNVERIFIED" not in t:
        return fail(12,"premises carried without byte verification must be labelled as such")
    print("PREMISE_GATE = CLEAN (unverified premise labelled)")
