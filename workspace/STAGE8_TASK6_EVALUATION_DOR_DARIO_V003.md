# STAGE 8 / TASK 6 / STEP 2 — THE EVALUATION DoR: THE SCOPE DECLARATION — V003

Lane: Dario (Claude Opus 5), drafting lane
Task: PASTE 609 / Task 6, Step 2
...
Supersedes: my V002 (`1f4834e6…`), confirmed DEFECTIVE (+3 bounded) at `93b516c5…`.
Base: my V001 (`9704f273…`) for all content the confirm found lost.
Custody: three bounded fixes plus carriage restoration; Codex 3 re-confirms bounded.
...
## 0. Preflight, the delta board, and a correction to my own carriage

| Check | Result |
|---|---|
| Register head Q-544 (live-append tolerant) | verified — verify-and-proceed |
| Confirm `STAGE8_TASK6_DOR_V002_CONFIRM_LANE3_V001.md` = `93b516c57bd18ded9a52455c6272b5f5e9aebdea0b6d39e716b2add681fed192` | **verified before reading** |
| My V002 = `1f4834e6…` ; my V001 = `9704f273…` | verified |
| Output name collision | none — clear to write |

### 0.1 THE CARRIAGE CORRECTION — owned plainly

[PROVABLE] My V002 asserted `DELTA = bounded` with *"UNCHANGED = every other
clause of V001, carried verbatim."* **That assertion is false, and the confirm
exhibits five witnesses.** I compressed load-bearing V001 content — witness
*conditions*, an independent limitation, a permanent regression row, a dependency
table, and guard qualifications — while claiming verbatim carriage.

This is precisely the failure mode I charged against another lane's carriage claim
at Q-517 (a "verbatim outside declared classes" claim falsified by an instance).
Mine is worse in kind: that instance moved no mathematics, whereas **U3 and U5
deleted guard content** — an armed regression row and four not-computed
qualifications. A shortened guard is a weakened guard.

**The rule I adopt for this draft, and state so it can be checked:**

```text
CARRIAGE RULE V003.
  Clause BLOCKS from V001 are reproduced in full.
  Every change is enumerated individually in the delta board below.
  No summary sentence asserts carriage over a class of clauses.     (C-V3)
```

### 0.2 The delta board — exact, and enumerated per change

| # | Item | Action | Where |
|---|---|---|---|
| D1 | row 8 bypassed the restored gate-7 inheritance | `(K2-R)` **applied verbatim**; re-audit now reads **rows 3–8** | (V2-2) row 8; (V5-4) |
| D2 | period-route one-branch-exit disposition not displayed | `(K3-R)` **applied verbatim** as a displayed row | (V4-3) |
| D3-U1 | modulus witness *conditions* + exact failure statement deleted | `(U1-V1)` **restored in full** | (V1-2) |
| D3-U2 | independent preselection-bridge limitation deleted | `(U2-V1)` **restored in full** | (V1-8) |
| D3-U3 | permanent anti-tuning regression row + cross-sector provenance sentence deleted | `(U3-V1)` **restored**; provenance sentence restored | (V5-2) |
| D3-U4 | prior dependency audit replaced rather than extended | `(U4-V1)` **restored and the K1–K4 table APPENDED to it** | (V5-4) |
| D3-U5 | four not-computed guard qualifications deleted | `(U5-V1)` **restored in full** | (V3) |

```text
DELTA = D1 + D2 + five restorations (U1-U5).
CARRIED FROM V002 = the four K1-K4 renderings, unchanged.
NO CELL MATHEMATICS CHANGES. NO GATE IS OPENED. NO VALUE IS PRODUCED.
```

[YOURS] **On D1, I record the reason and not merely the rendering.** The confirm's
countermodel is exact:
...
Under `(F-1)` my V002 row 8's prerequisite column admitted the numeric act while
`(V1-10a)` separately forbade it — **an internal inconsistency inside my own
declaration**, a table permitting what a clause prohibits. It is the same class of
error as K2 at the other end of the ladder: A32 seal ancestry is **custody**; gate
7 is the **symbolic assembly**; a numeric alpha needs both, and I bound row 8 to
custody alone.
...
[PROVABLE] The ratification's limit is unchanged: this DoR *"may DRAFT in parallel
...
### (V1-2) Step 8–9: the modulus — **U1 RESTORED IN FULL**
...
### (V1-3) Step 10: the case-lattice cell determination — K4, unchanged from V002
...
### (V1-5) The one-sided-trial rider, attached to `chi_K`'s reading
...
[PROVABLE] `chi_K^Mx` carries `q_T,RL` in a denominator, and **the licensed case
lattice has no infinite-`chi_K` cell** — a vanishing seed exits the lattice rather
than refuting anything. On seed zero the payload records `OUT_OF_LATTICE` or
noncomputable, **and no value is invented.**

### (V1-6) Contact on the local route — K3, unchanged from V002
...
### (V1-8) The sensitivity ladder's place — **U2 RESTORED IN FULL**
...
All finite-`L` objects in **exact rational or exact symbolic arithmetic**, with
the implementation **named and hashed**, and any enclosure reported as exact
rational strings. [YOURS] A float-path result is **not a value of this declaration
at all**.
...
[PROVABLE] `(V1-10a)`'s symbolic-first requirement is now **also enforced in the
gate table** at row 8, closing the `(F-1)` inconsistency.
...
### (V2-2) The gate map — **D1 APPLIED at row 8**
...
[PROVABLE] **Cumulative inheritance now runs the whole ladder.** Row 2 carries the
chain's mathematical entrance; rows 3–8 inherit it transitively. Row 8 additionally
names gate 7, so no numeric act can issue on custody alone.
...
### (V2-3) Where `(M5a-V002)`'s conjuncts gate which step
...
the fail-closed rule **no gate above 0 is presently open**.

---

## V3. WHAT IS NOT COMPUTED, AND WHY — **U5 RESTORED IN FULL**
...
quietly supplies. It does not: the local route is a **shadow** route.
...
| Cell 4, `\|chi_K\| ≥ A_loop^{-1}` | **not** strict by this threshold. Step 11 does **not** proceed. Record `NOT_CONTRACTIVE_BY_THRESHOLD`; **no search for an alternative metric to rescue it.** |
...
[YOURS] These must not be conflated. A report rendering both as "chi is zero"
destroys the one-sided rider.

### (V4-3) Branch dispositions — **D2 APPLIED: the period one-branch-exit row is displayed**
...
[YOURS] The display standard is the point: a prose assertion that a disposition is
"retained" is not a disposition. I applied that standard to the `A7_BRANCH` field
in V002 and failed to apply it to this row; it is now displayed.
...
### (V5-2) Anti-tuning ledger — **U3 RESTORED, +K2 and K4 rows retained**
...
### (V5-4) Dependency re-audit — **U4 RESTORED, with the K1–K4 table APPENDED**

**Table A — the confirmed V001 re-walk, restored `(U4-V1)`:**
...
**Table B — the K1–K4 re-walk, appended not substituted:**

| Rendering | Direct consumers re-walked | Result |
|---|---|---|
| K1 `(W1-3)` | the lead block; later references to "the ratified type block" | four fields attributed to the ratification; the rest re-tagged |
| K2 `(W1-7)` | gate row 2; **rows 3–8** which inherit it; (V5-2) | cumulative Steps 0–7 enter at row 2 and propagate transitively |
| K3 `(W2-5)`/`(W2-6)` | (V1-6); the lead field; (V1-8); (V4-3); (V5-3) | fork withdrawn; value proven; A7 preserved for the period machinery |
| K4 `(W3-1)`/`(W3-2)` | (V1-3); gate row 4; (V4-1); (V5-5) | five surfaces re-keyed; count-stop deleted |

**Table C — this relay's three fixes:**

| Fix | Direct consumers re-walked | Result |
|---|---|---|
| D1 `(K2-R)` | gate row 8; the inheritance statement; (V5-2)'s new row; `(V1-10a)` | row 8 now names gate 7; inheritance reads **rows 3–8**; `(F-1)` closed |
| D2 `(K3-R)` | (V4-3) | the period one-branch-exit disposition is displayed |
| D3 U1–U5 | (V1-2), (V1-8), (V5-2), (V5-4), (V3) | all five restorations landed; no other clause altered |
...
| My verb | Check |
|---|---|
| `DELTA` claim | **Corrected.** V003 asserts no class-wide carriage. Every change is enumerated in §0.2 under the `(C-V3)` rule, which exists because my V002 claim was falsified by five witnesses. |
| "restored in full" (U1–U5) | Means the V001 text is reproduced, not summarized. U1's conditions, U2's second sentence, U3's row and provenance, U4's table, U5's four qualifications are each present above. |
| row 8 | Now names gate 7. I record that my V002 table permitted what my own `(V1-10a)` forbade — an internal inconsistency, not merely an omission. |
| (V4-3) period one-branch-exit row | **Displayed**, not asserted as retained. The display standard I applied to `A7_BRANCH` and failed to apply here. |
...
| `DRAFTED` | A draft for later ruling. `RULING_READY_PENDING_SUBGATE` is **not** asserted here — that is Codex 3's call, and `(M5a-V002)` remains `false_of_record`. |
| "is computed" throughout V1 | Means **would be computed when licensed**. `NUMERIC_EVALUATION = false`. |
| Sources | Every clause cites a sealed source or is tagged `[YOURS]`. No physics law, candidate, comparator, number, branch selector, threshold, or seal is authored. |

---

```text
DOR_V003 = DRAFTED (+3/3 fixed, +carriage restored:
  D1 (K2-R) applied verbatim — row 8 may not begin until gate 7
     + ALPHA-RESULT-SEAL with its four G6 parents transitively certified
     + the separate licensed act; the inheritance statement now reads rows 3-8;
     the (F-1) countermodel is closed and recorded as an internal inconsistency
     between my own table and my own (V1-10a);
  D2 (K3-R) applied verbatim — the period-route ONE-BRANCH-EXITS disposition is
     DISPLAYED: that branch records OUT_OF_LATTICE, the other reports normally,
     the pair is still reported as a pair, neither is selected, averaged, merged
     or dropped;
  D3 carriage restored — U1 the two modulus witness CONDITIONS and the exact
     failure statement; U2 the independent preselection-bridge limitation;
     U3 the permanent "infer nonvanishing from cycle presence" regression and
     the cross-sector-unit sealed provenance; U4 the prior dependency table,
     with the K1-K4 table APPENDED not substituted; U5 the four not-computed
     guard qualifications)
VERB_AUDIT_SELF = CLEAN
```

The three findings are applied with their displays, the five lost witnesses are
restored in full rather than summarized, and the delta is enumerated per change
under a stated carriage rule. I make no class-wide carriage assertion in this
draft, because that is the assertion that failed in V002 — and it failed in the
direction that matters, deleting guard content while claiming to preserve it.
