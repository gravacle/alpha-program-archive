#!/usr/bin/env python3
"""
SEALED GENERATOR — STAGE8_JOINT_GR_EM_FUNCTION_HUNT_DARIO_V001.md   (relay 1095, DARIO lane)
...
ART = WS / "STAGE8_JOINT_GR_EM_FUNCTION_HUNT_DARIO_V001.md"; SIDE = Path(str(ART)+".seal.sha256")
SELF = Path(__file__)
...
def main():
    ...
    # R10 the joint form of the thesis is the maximum-pressure answer: FUNCTION-FORCES may seal
    # only with all three parts. Any weaker verdict must report the three tests SEPARATELY, so a
    # pass on one cannot carry a fail on another, and must not overstate itself into a refutation.
    if "VERDICT = FUNCTION-FORCES" in t:
        need={"the forced function":"FUNCTION_FORCED = UNIQUE-REQUIRE",
              "independent legs":"GR_LEG = INDEPENDENT-DERIVED",
              "a proven unique root":"ROOT = UNIQUE-FORCED"}
        missing=[k for k,v in need.items() if v not in t]
        if missing:
            return fail(10,"the joint thesis may not seal without all three parts: "+str(missing))
        print("THESIS_GATE = CLEAN (forced function, independent legs, unique root all present)")
    else:
        need={"the function ruling":"FUNCTION_FORCED =","the GR leg":"GR_LEG =",
              "the EM leg":"EM_LEG =","the root":"ROOT =",
              "the not-refuted limit":"NOT REFUTED IN JOINT FORM"}
        missing=[k for k,v in need.items() if v not in t]
        if missing:
            return fail(10,f"a weaker verdict must report each test separately and must not "
                           f"overstate itself: {missing}")
        print("SEPARATE_TESTS_GATE = CLEAN (function, both legs and root reported separately; "
              "thesis not overstated as refuted)")
    # R11 the named flattening is the assignment's spine and must be carried explicitly
    if "DO NOT FLATTEN" not in t.upper():
        return fail(11,"the assignment's named flattening must be carried explicitly")
    print("FLATTENING_GATE = CLEAN (the named error is carried)")
    # R12 the evaluator-facing boundary must be declared where the root is discussed
    if "ROOT =" in t and "evaluator-facing" not in t:
        return fail(12,"a root ruling must declare where the evaluator-facing boundary was held")
    print("BOUNDARY_GATE = CLEAN")
    # R13 carried premises may not be dressed as verified
    if "CARRIED, NOT REOPENED" not in t:
        return fail(13,"the carried range must be declared")
    print("PREMISE_GATE = CLEAN")
