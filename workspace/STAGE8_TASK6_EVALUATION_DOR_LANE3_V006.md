# STAGE 8 / TASK 6 / STEP 2 — THE EVALUATION DoR: THE SCOPE DECLARATION — V006

Lane: Codex Lane 3 (SOL, high effort), drafting lane
Task: PASTE 615 / Task 6, Step 2
Authority to draft: DoR-020-A8 + the ratified A32 instrument + living entry Q-550.
**THIS ARTIFACT IS A
DRAFT. IT ADOPTS NOTHING, RULES NOTHING, LIFTS NO GATE, AND COMPUTES NOTHING.**
Base: Lane-3 V005 (`6b4c96a067bfc7159211909b9fa449cb305bc92fe00005e8ce929cddbccbe773`).
Repair authority: Q-550 + Dario's V005 sweep
(`970e4f4d3a218ef99a6f102e04fab8eba5fd6ad321403a33a6b07387fc5a257d`).
Custody: **DRAFT, two bounded regions only:** G1 is the ordered X1–X9
addendum with its control/audit renderings; G2 restores the clause-8 prohibition.
Dario re-sweeps fresh.
...
## 0. Preflight, standing C-V5, and the bounded V006 delta

| Check | Result |
|---|---|
| Register head Q-550 (live-append tolerant) | verified — Q-550 ratifies C-V5 as standing and orders this bounded addendum plus line restoration |
| Immediate base V005 | `STAGE8_TASK6_EVALUATION_DOR_LANE3_V005.md`, SHA-256 `6b4c96a067bfc7159211909b9fa449cb305bc92fe00005e8ce929cddbccbe773`, verified before reading |
| V005 sweep | `STAGE8_TASK6_DOR_V005_SWEEP_DARIO_V001.md`, SHA-256 `970e4f4d3a218ef99a6f102e04fab8eba5fd6ad321403a33a6b07387fc5a257d`, verified before reading |
| Historical X baseline | `STAGE8_TASK6_EVALUATION_DOR_DARIO_V003.md`, SHA-256 `da5b03e151bc1b391ba1c666b3c3500f3f474461d4f1e669184ad09ce905b913`, verified before comparison |
| Standing C-V5 historical pair | V004 `1e3e24289b97303bba1e8f57612e09dbace897e482ee9d25c2af35953524b000` → V005 `6b4c96a067bfc7159211909b9fa449cb305bc92fe00005e8ce929cddbccbe773` |
| Original X1–X9 finding | `STAGE8_TASK6_DOR_V004_SWEEP_LANE3_V001.md`, SHA-256 `f651b34befdaa0f2410778e80c970f355e39cab733cdb64f9e37706950bfbedc`, verified before reading |
...
| Clause-8 sources | DoR-020-A2 adoption `5c945cae9ae8afa60080ee08d569324d350d6717bf2a3058460bc00451b86f8e`; sequenced-program decision `eaeffd37982cc0063a42095a6880c34d6a95a95b7240cd02b61e485f4b17482a`; both seals verified |
| Output collision | none — clear to draft |
| Protected operations | none performed |

### 0.1 C-V5 — the standing finite-equality instrument
...
This is the historical V004→V005 instance. Q-550 ratifies `(C-V5)` as the
standing instrument; §0.3 and V6 instantiate it only on their named comparison
domains. `(C-V5)` is a named finite equality for one version pair. It does not
...
### 0.2 Historical V004→V005 delta rows
...
### 0.3 Ordered-remedy addendum — the historical X1–X9 regions

[PROVABLE] Q-550 regularizes the earlier remedy substitution: C-V5 remains the
standing finite-equality instrument, **and** the ordered X1–X9 enumeration from
the 612 sweep executes here. The baseline and subject are:

```text
BASE_X
  := sealed V003, SHA-256
     da5b03e151bc1b391ba1c666b3c3500f3f474461d4f1e669184ad09ce905b913.

SUBJECT_X
  := this V006.

X_REGION_DOMAIN
  := the ordered tuple of nine pairwise-disjoint, anchor-bounded comparisons
     listed X1,...,X9 below.

ADDENDUM_EXCLUSION
  := remove §0.3 from SUBJECT_X before extracting every X body region.

REGION_DIFF_i
  := `/usr/bin/diff -U 0` on anchor-bounded region i from BASE_X to the
     ADDENDUM_EXCLUSION projection of SUBJECT_X.

PROBE_M1
  := flatten newlines and squeeze whitespace on BOTH compared region copies,
     then fixed-string search the full phrase; a long phrase is corroborated
     by two disjoint substrings. ADDENDUM_EXCLUSION governs both the line-level
     region extraction and every body-window probe.

CLAIM_X1_X9
  := (REGION_DIFF_1,...,REGION_DIFF_9)
     = (EDIT_X1,...,EDIT_X9) exactly as displayed below.          (X-EQ)
```

Normalization governs token-presence probes, not the line-level diff domain.
`(X-EQ)` is a finite equality over these nine named historical regions only; it
is not a whole-file V003→V006 carriage claim, does not enlarge the V005→V006
delta domain, and asserts nothing about an unnamed region or version pair.

| ID | Anchor-bounded region | Exact edit retained in the V006 body |
|---|---|---|
| X1 | V003 §0 block from `[YOURS] **On D1` through `custody alone.`; projected V006 body seam after the historical delta display and before the tag legend | delete D1's displayed `(F-1)` valuation and the internal-inconsistency/custody-versus-symbolic-assembly reasoning block |
| X2 | `(V1-10)`, the paragraph immediately following `(V1-10a)` | replace V003's `[PROVABLE]` paragraph by a `[YOURS]` paragraph; add the `LOCAL-SHADOW`-typed assembly sentence; retain the row-8 enforcement assertion while deleting `now` and `closing the (F-1) inconsistency` |
| X3 | `(V2-3)`, the `SPEC-SEAL` root-status sentence only | add `the ladder is unformed at its root, not partway up` |
| X4 | V3 closing paragraph beginning `[YOURS] Items 1–3` | add that the product's shadow type prevents downstream confusion with the period-native object |
| X5 | V4 preamble between the V4 heading and `(V4-1)` | add the before-any-value timing rule: a disposition chosen after seeing a value is not pre-registration |
| X6 | `(V4-1)` row `Cell 4, \|chi_K\| ≥ A_loop^{-1}` | add that alternate-metric rescue is the attack blocked by `MODULUS_COMPATIBILITY_CERT` |
| X7 | `(V4-2)` prose immediately after `(V4-2a)` | expand the finite `chi_K = 0` versus polar-seed/`OUT_OF_LATTICE` distinction |
| X8 | V003 paragraph beginning `[YOURS] The display standard is the point`; V006 body seam between the `(V4-3)` table and `(V4-4)` | delete the D2 self-correction paragraph while retaining the displayed disposition row |
| X9 | Bodies of V5-4 Tables B and C, from each header row through its last data row | remove the `Result` columns and their row-result cells; in Table C shorten `(V5-2)'s new row` to `(V5-2)` and re-label `D3 U1–U5` as `U1–U5`; retain the remaining rendering/fix and direct-consumer cells and retain the later global no-change sentence |

The whitespace-normalized body-window probes return:

```text
X2_X7_ADDITION_ANCHORS_IN_SUBJECT       = present (6/6);
X1_X8_X9_DELETION_ANCHORS_IN_BASE       = present (3/3);
X1_X8_X9_DELETION_ANCHORS_IN_SUBJECT    = absent  (3/3);
X2_RETAG_AND_DELETION_PROBES             = pass (4/4);
X9_TABLE_C_REKEY_PROBES                  = pass (2/2);
X1_X9_ORDER                             = X1,X2,X3,X4,X5,X6,X7,X8,X9;
X1_X9_ENUMERATION                       = COMPLETE_9_OF_9;
X_REGION_UNASSIGNED                     = empty;
X_REGION_MULTIPLY_ASSIGNED              = empty;
C_V5_HISTORICAL_DELTA_DOMAIN            = unchanged.              (X-CLEAN)
```
...
| set a cross-sector unit silently to one | every required factorization displayed in the propagation trace; no conversion silently set to one; undeclared conversion is an armed falsifier. **Sealed provenance: DoR-020-A2's frozen clause 8.** | clean |
...
**Table F — V006's two bounded regions, appended:**

| Region | Direct consumers re-walked |
|---|---|
| G1 — X1–X9 addendum | header and preflight; standing C-V5 statement; §0.3 historical-region equality and probes; this dependency row; (V5-5); operative overlay; V6; final lines |
| G2 — clause-8 prohibition | operative (V5-2) cross-sector-unit row; Table A's clause-8 mapping; this dependency row; (V5-5); V6 |
...
**Rows appended by V002/V003/V004/V005/V006:**
...
| V005 `VERB_AUDIT_SELF = CLEAN` (historical) | Carries the historical V005 audit after F1–F5, the operative overlay, and `(REDIFF-V005)`; it is not inherited from the quoted V001 token and does not certify V006. |
| `X1_X9_ENUMERATION = COMPLETE_9_OF_9` | The finite equality `(X-EQ)` names BASE_X, SUBJECT_X, nine ordered anchor windows, and the exact edit in each; `(X-CLEAN)` displays no missing or duplicate region. It is not a whole-file carriage claim. |
| clause-8 prohibition restored | The operative (V5-2) row contains the exact phrase `no conversion silently set to one` alongside the retained armed falsifier and sealed DoR-020-A2 clause-8 provenance. |
| `SUBSTANTIVE_REGIONS = {G1,G2}` | `(REDIFF-V006)` assigns every raw V005→V006 hunk once. G1 contains the addendum and its required control/audit renderings; G2 contains only the operative clause-8 row. No third substantive region is asserted or found. |
| `DOR_V006 = DRAFTED` | A draft under Q-550's bounded instruction. It adopts nothing, opens no gate, and leaves `(M5a-V002)` `false_of_record`. |
| current `VERB_AUDIT_SELF = CLEAN` | Audits V006 after `(X-EQ)`, the operative G2 row, the dependency re-walk, the raw C-V5 hunk partition, and the whitespace-normalized probes are displayed. |
...
```text
DECLARATION:
  V006 remains DRAFTED and opens no gate.
...
HISTORICAL_VERB_TOKEN:
  ...
  it does not certify V006.                                      (E5-OVERLAY)
```
...
## V6. Historical V005 certificate and V006 pre-seal re-diff

### V6.1 Historical V004→V005 certificate retained
...
### V6.2 V005→V006: the two-region C-V5 instance

```text
V006_DELTA_DOMAIN
  := the ordered hunks emitted by `/usr/bin/diff -U 0`
     on V005 -> V006; H01,H02,... name those hunks in emitted order.

V006_DELTA_REGIONS := {G1,G2}.

G1_X1_X9_ADDENDUM
  := the ordered-remedy addendum plus its necessary version, preflight,
     dependency, verb-audit, overlay, re-diff, and final-status renderings.

G2_CLAUSE8_LINE
  := the one operative (V5-2) row restoration of
     `no conversion silently set to one`, with its sealed provenance retained.

CLAIM_V006
  := V006_DELTA_DOMAIN
       = EXACT_HUNKS(G1) disjoint_union EXACT_HUNKS(G2).           (C-V5-V006)
```

| Region | Exact edit class | Raw pre-seal hunk set |
|---|---|---|
| G1 | X1–X9 addendum and only its control/audit renderings named above | `{H01–H10,H12–H19}` |
| G2 | clause-8 prohibition restored in the operative cross-sector-unit row | `{H11}` |

```text
PRESEAL_DIFF_HUNKS_V006       = 19;
PRESEAL_DIFF_ADDED_V006       = 173;
PRESEAL_DIFF_DELETED_V006     = 20;
SUBSTANTIVE_REGIONS           = {G1,G2};
OTHER_SUBSTANTIVE_REGIONS     = empty;
UNASSIGNED_HUNKS_V006         = empty;
MULTIPLY_ASSIGNED_HUNKS_V006  = empty.                            (REDIFF-V006)

M1_FULL_CLAUSE8_PHRASE        = present_in_operational_row;
M1_DISJOINT_PROHIBITION_ANCHOR = present;
M1_DISJOINT_FALSIFIER_ANCHOR   = present;
M1_SEALED_PROVENANCE_ANCHOR    = present;
X1_X9_NORMALIZED_PROBES        = 9/9 pass.                        (M1-V006)
```

Every raw hunk in `(REDIFF-V006)` is assigned once to G1 or G2. Control and
attestation text is enumerated inside G1 rather than silently discarded. G2 is
the single operative-row replacement. Thus the V005→V006 delta has exactly the
two named substantive regions and no third region.

```text
DOR_V006 = DRAFTED (+addendum, +line restored, +re-diff clean)
alpha_computed = false
proof_authorized = false
kappa_record_computed = false
MEMBER_BOUND = false
