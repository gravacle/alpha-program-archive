...
VERIFIER = ROOT / "scripts" / "verify_bid_source_parent_terminal_children_v005.py"
VERIFIER_BUILDER = (
...
def write_manifest(
    ...
    payload = {
        ...
        "stage": (
            ...
            f"{'optimized' if optimize else 'normal'}-v005"
        ),
...
def main() -> None:
    ...
    if (
        sha256(V003_FAILURE_RECORD) != V003_FAILURE_RECORD_SHA256
        or sha256(V003_FAILURE_SEAL) != V003_FAILURE_SEAL_SHA256
        or V003_FAILURE_SEAL.read_text(encoding="utf-8").strip()
        != f"{V003_FAILURE_RECORD_SHA256}  {V003_FAILURE_RECORD.name}"
    ):
        raise RuntimeError("preserved v003 failure lineage does not match its trust anchor")
    for path in (
        NORMAL_MANIFEST,
        OPTIMIZED_MANIFEST,
        NORMAL_OUTPUT,
