#!/usr/bin/env python3
"""Synthetic fence tests for the Stage-8 T7 bundle comparator (v003).

v003 successor authorized by
STAGE8_T7_ACTUAL_PARENT_CAR_IMPLEMENTATION_REPAIR_BINDING_V002.md (S1 item
4).  Ports every v002 fence test and adds the mandatory new negatives:
...
SCRIPT = (
    ...
    / "compare_stage8_t7_actual_parent_regulated_car_operator_response_v003.py"
)
MODULE_SPEC = importlib.util.spec_from_file_location("stage8_t7_comparator_v003", SCRIPT)
...
class FixtureData:
    ...
    def __init__(self, variant: str = "genuine") -> None:
        ...
        self.pieces = (
            surrogate_pieces() if variant == "surrogate" else synthetic_pieces()
        )
...
class ComparatorFenceTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        implementation_files = (
            ...
            "scripts/compare_stage8_t7_actual_parent_regulated_car_operator_response_v003.py",
            "scripts/test_compare_stage8_t7_actual_parent_regulated_car_operator_response_v003.py",
        )
        implementation_manifest = self.root / "implementation.json"
        implementation_manifest.write_text(
            json.dumps(
                {
                    "schema": "stage8_t7_actual_parent_car_implementation_test_v003",
                    "files": [
                        {
                            "path": relative,
