...
PROTOCOL = (
    ROOT
    / "COMPLETE_QSPEC_ANALYTIC_CONTINUATION_INDEPENDENT_ISOMETRY_STABILIZATION_ADDENDUM_V001.md"
)
PRIMARY = (
    ROOT
...
OUT = (
    ROOT
    / "stage8_execution/work/"
    "QSPEC_periodic_analytic_continuation_verification_v003.json"
)

EXPECTED = {
    PROTOCOL:
        "85d5a138a1c11dbcfcd85428536afd65bad1f9f6603c7a79c9ad489cd3070e37",
    ROOT / "COMPLETE_QSPEC_ANALYTIC_CONTINUATION_INDEPENDENT_ZERO_STABILIZATION_ADDENDUM_V001.md":
        "ef9a85a5cb8da50cd93c534e61846b2bd72944b139facfde3fb0eac355beecbc",
    ROOT / "COMPLETE_QSPEC_ANALYTIC_CONTINUATION_INDEPENDENT_PREFLIGHT_FAILURE_V002.md":
        "afcd55e8f15f0d63a1c470f7d47efa01e47bd977ce8d3a81a4a891812b13c6b4",
    ROOT / "scripts/verify_complete_qspec_periodic_analytic_continuation_v002.py":
        "d8cc0409bccc2f815500ec4700f9197339b245806ebf2a90948ecb1a90326370",
    ROOT / "stage8_execution/work/QSPEC_periodic_analytic_continuation_verification_v002.json":
        "75fe2540d4a74cbbc6f7bea039810df5ac48af94e60c8dc6da7a67ed543edd70",
    ROOT / "COMPLETE_QSPEC_ANALYTIC_CONTINUATION_INDEPENDENT_VERIFIER_PROTOCOL_V001.md":
        "e5630cbc9d5d92607773ebb5fc8f8a90075f437d8131311e7d1ec6f5bb84ce0e",
    ROOT / "scripts/verify_complete_qspec_periodic_analytic_continuation_v001.py":
        "d822621721444d9fab9af32425d8ecc95190933465b974d7883b2740b3c920fe",
    ROOT / "stage8_execution/work/QSPEC_periodic_analytic_continuation_verification_v001.json":
        "32d381c65bce844615c92e5fcd7138e3fb7e772b952fc359f3d947f189c4e744",
    ROOT / "COMPLETE_QSPEC_ANALYTIC_CONTINUATION_INDEPENDENT_PREFLIGHT_FAILURE_V001.md":
        "a3be70250a7f450418c4837271bc1621543586219969b844532fbc8e416011fd",
...
def cell_kraus(
    ...
    record: np.ndarray,
    use_hermitian_spectral_step: bool = False,
) -> tuple[np.ndarray, ...]:
    ...
    if use_hermitian_spectral_step:
        eigenvalues, eigenvectors = np.linalg.eigh(free)
        half_free = (
            eigenvectors
            @ np.diag(np.exp(-0.5j * eigenvalues / STEPS))
...
def retract_stinespring_isometry(
    kraus: tuple[np.ndarray, ...],
) -> tuple[tuple[np.ndarray, ...], dict[str, float | bool]]:
    source_dimension = kraus[0].shape[0]
    record_dimension = len(kraus)
    raw = np.vstack(kraus)
    identity = np.eye(source_dimension, dtype=complex)
    gram = raw.conjugate().T @ raw
    raw_defect = float(np.linalg.norm(gram - identity))
    ...
    positive = bool(float(np.min(eigenvalues)) > 0.0)
    inverse_sqrt = (
        eigenvectors
        @ np.diag(eigenvalues ** -0.5)
        @ eigenvectors.conjugate().T
    )
    repaired = raw @ inverse_sqrt
    correction = float(
        np.linalg.norm(repaired - raw)
        / max(np.linalg.norm(raw), 1e-30)
    )
    repaired_defect = float(
        np.linalg.norm(
            repaired.conjugate().T @ repaired - identity
        )
    )
    passed = bool(
        positive
        and raw_defect < roundoff_budget
        and correction < 1e-11
        and repaired_defect < 1e-12
    )
    repaired_kraus = tuple(
        repaired[
            outcome * source_dimension:
            (outcome + 1) * source_dimension,
            :,
        ]
        for outcome in range(record_dimension)
    )
    return repaired_kraus, {
        "raw_Gram_defect": raw_defect,
        "roundoff_budget": roundoff_budget,
        "minimum_Gram_eigenvalue": float(np.min(eigenvalues)),
        "relative_correction": correction,
        "repaired_Gram_defect": repaired_defect,
        "pass": passed,
    }
...
def main() -> None:
    ...
    prior_verification = json.loads(
        (
            ROOT
            / "stage8_execution/work/"
            "QSPEC_periodic_analytic_continuation_verification_v002.json"
        ).read_text()
    )
    parent = build_parent()
    ...
    zero_hermiticity_error = float(
        np.linalg.norm(
            parent["free_zero"] - parent["free_zero"].conjugate().T
        )
    )
    zero_values, zero_vectors = np.linalg.eigh(parent["free_zero"])
    zero_spectral_half_step = (
        zero_vectors
        @ np.diag(np.exp(-0.5j * zero_values / STEPS))
        @ zero_vectors.conjugate().T
    )
    zero_taylor_half_step = expm_taylor(
        -0.5j * parent["free_zero"] / STEPS
    )
    zero_half_step_relative_difference = float(
        np.linalg.norm(
            zero_spectral_half_step - zero_taylor_half_step
        )
        / max(np.linalg.norm(zero_spectral_half_step), 1e-30)
    )
    zero_stabilization_pass = bool(
        zero_hermiticity_error < 1e-12
        and zero_half_step_relative_difference < 1e-12
    )

    zero_raw_cells = tuple(
        cell_kraus(
            parent["free_zero"],
            interaction,
            parent["record"],
    ...
    zero_retractions = tuple(
        retract_stinespring_isometry(kraus)
        for kraus in zero_raw_cells
    )
    zero_cells = tuple(item[0] for item in zero_retractions)
    zero_isometry_diagnostics = tuple(
        item[1] for item in zero_retractions
    )
    zero_isometry_stabilization_pass = all(
        bool(item["pass"])
        for item in zero_isometry_diagnostics
    )
    ...
    prior_rows = prior_verification["selected_complex_rows"]
    selected_row_changes = []
    for key, row in rows.items():
        prior_row = prior_rows[key]
        selected_row_changes.extend(
            (
                abs(
                    row["maximum_reduced_direct_relative_error"]
                    - prior_row[
                        "maximum_reduced_direct_relative_error"
                    ]
                ),
                abs(
                    row["producer_N64_relative_error"]
                    - prior_row["producer_N64_relative_error"]
                ),
                abs(
                    row["minimum_direct_amplitude_N1_to_N4"]
                    - prior_row["minimum_direct_amplitude_N1_to_N4"]
                ),
                abs(
                    row["spectral"]["N_ge_5_dominance_bound"]
                    - prior_row["spectral"][
                        "N_ge_5_dominance_bound"
                    ]
                ),
                abs(
                    row["spectral"]["remainder_norm_ratio"]
                    - prior_row["spectral"][
                        "remainder_norm_ratio"
                    ]
                ),
            )
        )
    maximum_selected_row_change_from_v002 = max(
        selected_row_changes
    )

    passed = bool(
        analytic_pass
        and zero_stabilization_pass
        and zero_isometry_stabilization_pass
        and support_pass
        and all_rows_pass
        and zero_pass
    )
    result = {
        "schema":
            "complete_qspec_periodic_analytic_continuation_verification_v003",
        "protocol_sha256": sha256(PROTOCOL),
        "primary_sha256": sha256(PRIMARY),
        "independent_exponential":
            "Taylor_for_complex_branches_and_spectral_for_Hermitian_zero",
        "zero_history_stabilization": {
            "hermiticity_error": zero_hermiticity_error,
            "Taylor_spectral_half_step_relative_difference":
                zero_half_step_relative_difference,
            "pass": zero_stabilization_pass,
        },
        "zero_history_isometry_stabilization": {
            "cells": zero_isometry_diagnostics,
            "pass": zero_isometry_stabilization_pass,
        },
        ...
        "maximum_selected_row_change_from_v002":
            maximum_selected_row_change_from_v002,
