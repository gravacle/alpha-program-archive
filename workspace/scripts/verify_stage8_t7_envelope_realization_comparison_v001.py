#!/usr/bin/env python3
"""Independent midpoint verification of the blind ER-A/ER-B comparison."""
...
SPEC = ROOT / "STAGE8_T7_ENVELOPE_REALIZATION_COMPARISON_SPEC_V001.md"
PRIMARY = ROOT / "stage8_execution/work/T07_envelope_realization_comparison.json"
OUT = ROOT / "stage8_execution/work/T07_envelope_realization_comparison_verification.json"
...
def main() -> None:
    ...
    for mode in ("ER-A", "ER-B"):
        midpoint_48 = independent.amplitude(
            histories(mode, 48, alpha, incidence), covariance, occupied
        )
        ...
        strang = tuple(
            read_complex(
                primary_case["branches"][mode]["amplitudes"]["48"][scheme]
            )
            for scheme in ("mixed", "pure")
        )
        mode_checks = {}
        for index, scheme in enumerate(("mixed", "pure")):
            tail = abs(midpoint_48[index] - midpoint_96[index])
            cross = abs(midpoint_96[index] - strang[index])
            mode_checks[scheme] = {
                "midpoint_48_to_96": float(tail),
                "midpoint_96_to_primary_strang_48": float(cross),
            }
            passed = passed and tail < TOLERANCE and cross < TOLERANCE
        checks[mode] = mode_checks

    result = {
        "schema": "stage8_t7_envelope_realization_comparison_verification_v001",
