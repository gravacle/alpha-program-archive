...
v003 (repair binding V002, S1 item 3):
  a. GPG fully removed per the supersession amendment (no signing, no .asc,
     no fingerprint constants or checks); receipts remain sealed immutable
     JSON, externally anchored per artifact (commit+push) by the S3 procedure.
  b. PER-LANE INVOCATION: a required --lane {independent,primary,comparison}
     argument runs exactly one lane per invocation, so each receipt is
     anchored BEFORE the next lane runs. Lane order is enforced through
     verified receipts: primary refuses without the sealed+verified
     independent receipt; comparison refuses without both lane receipts.
  c. PRE-FLIGHT: every fail-closed precondition is verified BEFORE any
     canonical output path is consumed — complete implementation-manifest
     row set + adjacent seal, launcher/target hashes, absence of the
     canonical outputs and receipts appropriate to the lane, and v003
     comparator reachability. Any pre-flight failure blocks before any
     output exists.
  d. The receipt field paths_verified_absent records the EXACT list of
     canonical paths whose absence was verified in pre-flight (replacing
     the v002 receipt's hardcoded absence boolean), so the independent
     receipt attests the absence of the primary outputs as B1 designates.
  e. The comparison lane passes the two lane-receipt digests to the v003
     comparator (--independent-receipt-sha256 / --primary-receipt-sha256)
     and cross-checks the values recorded in the sealed comparison output.
     These recorded fields are RECORDINGS for the externally anchored
     receipt chain, not self-authentication.
"""
...
RUNTIME_LAUNCHER = (
    ROOT / "scripts/launch_stage8_t7_content_addressed_runtime_v003.py"
)
...
def verify_implementation() -> tuple[str, dict[str, str]]:
    ...
    require(
        ...
        "v003 comparator is not reachable",
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run exactly one Stage-8 T7 production lane (v003)."
    )
    ...
    # adjacent seal, launcher/target/comparator hashes, prior-lane receipts,
    # and the lane-appropriate canonical absences.
