...
RUNTIME = PROJECT_ROOT / "provenance" / "primitive_step6_runtime_snapshot_v012.json"
LAUNCHER = PROJECT_ROOT / "scripts" / "launch_content_addressed_runtime_script_v012.py"
PRODUCER_BUILDER = (
    ROOT / "scripts" / "build_bid_source_parent_terminal_producer_manifests_v003.py"
)
VERIFIER_BUILDER = (
    ROOT / "scripts" / "build_bid_source_parent_terminal_verifier_manifest_v003.py"
)
NORMAL_MANIFEST = (
    PROJECT_ROOT
    / "provenance"
    / "bid_source_parent_terminal_producer_normal_v003_execution_manifest.json"
)
OPTIMIZED_MANIFEST = (
    PROJECT_ROOT
    / "provenance"
    / "bid_source_parent_terminal_producer_optimized_v003_execution_manifest.json"
)
VERIFIER_MANIFEST = (
    PROJECT_ROOT
    / "provenance"
    / "bid_source_parent_terminal_children_verifier_v003_execution_manifest.json"
)
TERMINAL = PROJECT_ROOT / "results" / "bid_source_parent_sp14_terminal_v003.json"
SIGNATURE = PROJECT_ROOT / "results" / "bid_source_parent_sp14_terminal_v003.json.asc"
...
def validate_runtime() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    require(runtime.get("schema") == "gravacle.content-addressed-runtime.v012", "runtime schema")
...
def validate_modules(rows: Any, allowed: dict[str, str]) -> None:
    ...
    for row in rows:
        ...
        if kind in {"sealed_file", "sealed_originless_source"}:
            require(isinstance(origin, str) and isinstance(digest, str), "sealed module fields")
            path = Path(origin).resolve()
            require(
        ...
        elif kind == "sealed_parent_export_alias":
            parent = by_name.get(row.get("sealed_parent_module"))
            parent_origin = row.get("sealed_parent_origin")
            parent_digest = row.get("sealed_parent_sha256")
            require(
                origin is None
                ...
                and isinstance(parent_origin, str)
                and allowed.get(str(Path(parent_origin).resolve())) == parent_digest,
                "parent alias classification false",
            )
...
def validate_child(
    ...
    require(
        receipt.get("schema") == "gravacle.sealed-target-child-execution-record.v012"
...
def main() -> None:
    ...
    produced = (
        ...
        TERMINAL,
        SIGNATURE,
        PROJECT_ROOT / "results" / "bid_source_parent_terminal_producer_normal_v003.json",
        PROJECT_ROOT / "results" / "bid_source_parent_terminal_producer_optimized_v003.json",
        PROJECT_ROOT / "results" / "bid_source_parent_terminal_children_verifier_v003.json",
        PROJECT_ROOT / "provenance" / "bid_source_parent_terminal_producer_normal_v003_execution_receipt.json",
        PROJECT_ROOT / "provenance" / "bid_source_parent_terminal_producer_optimized_v003_execution_receipt.json",
        PROJECT_ROOT / "provenance" / "bid_source_parent_terminal_children_verifier_v003_execution_receipt.json",
    )
    require(not any(path.exists() for path in produced), "fresh terminal artifacts required")

    frozen_sources = {
        path: sha256(path)
        for path in (PROGRAM, PRODUCER_BUILDER, VERIFIER_BUILDER, LAUNCHER, RUNTIME)
    }
    ...
    terminal = {
        "schema": "gravacle.bid-source-parent-sp14-terminal.v003",
        ...
        "assurance_scope": (
            ...
            "revalidated mutable macOS trust base; not remote attestation and not "
            "resistance to a malicious kernel, interpreter, or signer."
        ),
        ...
        "originless_object_identity_scope": (
            "Object identity is enforced inside the hash-sealed launcher; the "
            "fresh parent revalidates the exact immutable alias tuple, metadata, "
            "causal row, and launcher bytes but does not reconstruct a dead "
            "child's Python object identity from JSON."
        ),
