#!/usr/bin/env python3
"""Independent CAR/RK4 verification of the finite-Fock record amplitudes."""
...
def fixed_number_lift(
    ...
    states = tuple(
        ...
        if bits.bit_count() == particle_count
    )
    ...
    for column, bits in enumerate(states):
        for annihilated in range(dimension):
            if not bits & (1 << annihilated):
                continue
            sign_a = -1 if (bits & ((1 << annihilated) - 1)).bit_count() % 2 else 1
            reduced = bits ^ (1 << annihilated)
            for created in range(dimension):
                if reduced & (1 << created):
                    continue
                sign_c = -1 if (reduced & ((1 << created) - 1)).bit_count() % 2 else 1
...
def main() -> None:
    ...
    coarse = evolve(initial, free_fock, writes_fock, record_writes, 800)
    fine = evolve(initial, free_fock, writes_fock, record_writes, 1600)
