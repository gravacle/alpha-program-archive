...
SPEC = (
    ROOT
    / "COMPLETE_QSPEC_ANALYTIC_CONTINUATION_REPAIR_SPEC_V001.md"
)
OUT = (
    ROOT
    / "stage8_execution/work/"
    "QSPEC_periodic_analytic_continuation_v003.json"
)
...
def analytic_free_factory():
    """Freeze the theta-zero active basis, then continue only the entries."""
...
def main() -> None:
    ...
    free = parent["free"]
    analytic_free = analytic_free_factory()
    ...
    maximum_repaired_cr_residual = 0.0
    maximum_superseded_cr_residual = 0.0
    cr_decreases = True
    for point in CR_POINTS:
        repaired = tuple(
            cauchy_riemann_residual(analytic_free, point, step)
        ...
        maximum_repaired_cr_residual = max(
            maximum_repaired_cr_residual,
            repaired[-1],
        )
        ...
        cr_decreases = cr_decreases and repaired[-1] < repaired[0]
        cr_rows[point_key(point)] = {
            "steps": list(CR_STEPS),
            "repaired_residuals": list(repaired),
            "superseded_residuals": list(superseded),
        }
    cr_pass = bool(
        maximum_repaired_cr_residual < 1e-8
        and cr_decreases
    ...
    result = {
        "schema": "complete_qspec_periodic_analytic_continuation_v003",
        ...
        "cauchy_riemann_preflight": {
            "points": cr_rows,
            "maximum_repaired_residual": maximum_repaired_cr_residual,
            "maximum_superseded_residual":
                maximum_superseded_cr_residual,
            "repaired_residual_decreases": cr_decreases,
            "pass": cr_pass,
        },
        "disk_radius": RADIUS,
