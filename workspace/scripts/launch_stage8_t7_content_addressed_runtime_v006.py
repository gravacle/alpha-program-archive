...
ALLOWED_TARGETS was REBUILT FRESH at the v005 generation and carries,
at v006, exactly the launchable rows of the current implementation set:
the two byte-frozen v002 derive lanes, comparator v005, controllers v005
and v006 (run_stage8_t7_actual_parent_car_pipeline_v00{5,6}.py — the
controller row whose absence in the v004 launcher was Blocking B1), and
the current test files (test_compare v005, test_stage8_t7_controller
v005 and v006, test_stage8_t7_launcher v005 and v006, and the two
byte-frozen v002 derive-lane tests).  The v006 rows are added under the
sealed supplement STAGE8_T7_CONTROLLER_V006_REPAIR_BINDING_SUPPLEMENT_
V001; the launcher-test row is entailed by that supplement's item 2
(the self-echo fence launches the test through the launcher).

NO controller or comparator row of any earlier generation is present.
Statement of mechanism, checkable per discipline rule 4: the superseded
chains are unlaunchable on the canonical paths because their entry
points were QUARANTINED to scripts/superseded_quarantine/ under Brian's
recorded authorization (quarantine manifest
provenance/stage8_t7_superseded_chain_quarantine_manifest_v001.json,
seven fail-closed invocation attempts recorded, zero artifacts) — not
because of any property of this file alone, and never by operator
discipline.  The launcher self-test remains the --selftest flag, which
requires no allowlist row.
