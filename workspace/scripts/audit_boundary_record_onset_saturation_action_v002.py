#!/usr/bin/env python3
"""Exact algebraic audit of the relative record-onset action gate."""
...
RESULT = ROOT / "results" / "boundary_record_onset_saturation_action_v002.json"
...
def build_result() -> dict[str, object]:
    ...
    relative_endpoint_unitary: Matrix = (
        (ZERO, NEG_ONE),
        (ONE, ZERO),
    )

    require(inner(READY, ORTHOGONAL) == ZERO, "Endpoint states are not orthogonal")
    require(matrix_mul(Y, Y) == I2, "Relative generator is not unit normalized")
    require(
        inner(READY, matrix_vector_mul(Y, READY)) == ZERO,
        "Relative-generator mean is not zero",
    )
    require(
        inner(READY, matrix_vector_mul(matrix_mul(Y, Y), READY)) == ONE,
        "Relative-generator second moment is not one",
    )
    require(
        matrix_vector_mul(relative_endpoint_unitary, READY) == ORTHOGONAL,
        "Relative endpoint unitary does not produce orthogonality",
    )
    require(commutator(Y, P0) != ZERO2, "Relative write direction preserves P0")
    require(commutator(Y, P1) != ZERO2, "Relative write direction preserves P1")
    ...
    require(trace(matrix_mul(Y, contrast)) == ZERO, "Relative write and contrast are not orthogonal")

    out = {
        "status": "PASS_RELATIVE_RECORD_ONSET_ACTION_FIXED_SOURCE_SCALE_OPEN",
        "conditional_record_overlap_object": "U0_dagger_U1",
        "relative_endpoint_overlap": 0,
        "fubini_study_relative_orthogonal_distance": "pi/2",
        "integrated_relative_energy_uncertainty_lower_bound": "pi*hbar/2",
        "allow_require_relative_onset_saturation_adopted_Level_1": True,
        "integrated_relative_write_action": "pi*hbar/2",
        "constant_relative_geodesic_energy": "pi*hbar/(2*tau_star)",
        "relative_generator_second_moment_in_ready_state": 1,
        "relative_write_generator_preserves_endpoint_sectors": False,
        "post_closure_contrast_preserves_endpoint_sectors": True,
        "relative_write_contrast_hilbert_schmidt_inner_product": 0,
        ...
        "scope": "relative_orthogonalization_bound_plus_adopted_onset_saturation_only",
    }
    return out
