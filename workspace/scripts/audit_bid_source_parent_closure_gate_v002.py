#!/usr/bin/env python3
"""V002 fail-closed regression audit for V011 source-parent obligations."""
...
EXPECTED_OBLIGATION_TABLE_SHA256 = (
    "10b17507bfb9b593e39fc40d39e027584078fbe131c15e79be3fcce2faeab750"
)
...
EXPECTED_RUNTIME_WITNESS_HASHES = {
    "manifest": "c2ab4d3677ef6f2eb2d9d058d8069267627c931223697915f4a3e91e76696078",
    "output": "1e6e9b212514692ef6904f58a1807c4bd902256ceef97fabc2356aaa0b8de576",
    "receipt": "34170794ece656ca940dc2fb6538be70188da1f1458ecd41e91d74b00e689a4b",
}
...
def run(script: str, optimized: bool = False) -> str:
    if SEALED_EXECUTION:
        ...
        try:
            with redirect_stdout(stdout), redirect_stderr(stderr):
                exec(
                    compile(
                        ...
                        str(path),
                        "exec",
                        optimize=2 if optimized else 0,
                    ),
                    namespace,
                )
            ...
            require(
                ...
                f"{'optimized' if optimized else 'normal'} execution: "
    ...
    require(
        result.returncode == 0,
        f"{script} failed ({'optimized' if optimized else 'normal'}): "
...
def verify_two_layer_runtime_witness() -> None:
    paths = {
        "manifest": RUNTIME_WITNESS_MANIFEST,
        "output": RUNTIME_WITNESS_OUTPUT,
        "receipt": RUNTIME_WITNESS_RECEIPT,
    }
    for label, path in paths.items():
        require(path.is_file(), f"runtime witness {label} is absent")
        require(
            file_sha256(path) == EXPECTED_RUNTIME_WITNESS_HASHES[label],
            f"runtime witness {label} changed",
        )

    manifest = read_json_strict(RUNTIME_WITNESS_MANIFEST)
    output = read_json_strict(RUNTIME_WITNESS_OUTPUT)
    receipt = read_json_strict(RUNTIME_WITNESS_RECEIPT)
    expected_manifest_relative = str(
        RUNTIME_WITNESS_MANIFEST.resolve().relative_to(PROJECT_ROOT.resolve())
    )
    expected_output_relative = str(
        RUNTIME_WITNESS_OUTPUT.resolve().relative_to(PROJECT_ROOT.resolve())
    )
    expected_receipt_relative = str(
        RUNTIME_WITNESS_RECEIPT.resolve().relative_to(PROJECT_ROOT.resolve())
    )
    expected_target_relative = (
        "alpha_fundamental_record_action_cleanroom_v003/scripts/"
        "verify_bid_source_parent_runtime_witness_v001.py"
    )
    require(
        manifest.get("schema") == "gravacle.sealed-target-execution.v002",
        "runtime witness manifest schema changed",
    )
    require(
        manifest.get("stage")
        == "bid-source-parent-runtime-witness-verification-v001",
        "runtime witness manifest stage changed",
    )
    require(manifest.get("alpha_computed") is False, "runtime witness manifest lost alpha firewall")
    require(
        manifest.get("target_script") == expected_target_relative,
        "runtime witness target changed",
    )
    require(
        manifest.get("expected_output") == expected_output_relative,
        "runtime witness output path changed",
    )
    require(
        manifest.get("execution_receipt") == expected_receipt_relative,
        "runtime witness receipt path changed",
    )

    files = manifest.get("files")
    require(isinstance(files, dict) and bool(files), "runtime witness file inventory is empty")
    require(
        files.get(
            "provenance/bid_source_parent_closure_v001_execution_manifest.json"
        )
        == "0535afcf14eb768166725b2ce4434f089077fd6a1e353a2d129d39553928c324",
        "first-layer manifest is not bound into the second layer",
    )
    require(
        files.get("results/bid_source_parent_closure_v001_sealed.json")
        == "3648b2fbbbf7d051f2a7ddcef72bee2849a97096144117d45124216d8a6e9bf1",
        "first-layer output is not bound into the second layer",
    )
    require(
        files.get(
            "provenance/bid_source_parent_closure_v001_execution_receipt.json"
        )
        == "00f72a54cbe1eadda6eb1f600c912507e0acc0e7c96600db14b767b623b1c0ee",
        "first-layer receipt is not bound into the second layer",
    )
    target_sha = files.get(expected_target_relative)
    require(
        isinstance(target_sha, str) and len(target_sha) == 64,
        "runtime witness target digest is absent",
    )
    ...
    expected_output_keys = {
        ...
        "status",
        "witness_loaded_module_count",
        "witness_loaded_native_image_count",
        "witness_manifest_sha256",
        "witness_output_sha256",
        "witness_receipt_sha256",
    }
    require(set(output) == expected_output_keys, "runtime witness output schema changed")
    require(
        output.get("schema")
        == "gravacle.bid-source-parent-runtime-witness-verification.v001",
        "runtime witness output schema identifier changed",
    )
    require(
        output.get("status")
        == "PASS_CONTENT_ADDRESSED_SOURCE_PARENT_RUNTIME_WITNESS_ALPHA_FALSE",
        "runtime witness did not pass",
    )
    require(
        output.get("witness_loaded_module_count") == 194
        and output.get("witness_loaded_native_image_count") == 353,
        "first-layer module/native-image counts changed",
    )
    require(
        output.get("source_parent_closure") is False
        and output.get("content_addressed_runtime_sealed") is True
        and output.get("proof_authorized") is False
        and output.get("alpha_computed") is False,
        "runtime witness authority/firewall state changed",
    )
    witness_checks = output.get("checks")
    require(
        isinstance(witness_checks, dict)
        and len(witness_checks) == 8
        and all(value is True for value in witness_checks.values()),
        "runtime witness checks are not an exact pass",
    )

    require(
        receipt.get("schema") == "gravacle.sealed-target-child-execution-record.v008",
        "runtime witness receipt schema changed",
    )
    require(
        receipt.get("status") == "CHILD_LOCAL_OUTPUT_CONTRACT_VALIDATED_ALPHA_FALSE",
        "runtime witness receipt status changed",
    )
    require(
        receipt.get("stage") == manifest.get("stage"),
        "runtime witness receipt stage mismatch",
    )
    require(
        receipt.get("target_manifest_path") == expected_manifest_relative,
        "runtime witness receipt manifest path mismatch",
    )
    require(
        receipt.get("target_manifest_sha256_before")
        == receipt.get("target_manifest_sha256_after")
        == EXPECTED_RUNTIME_WITNESS_HASHES["manifest"],
        "runtime witness manifest changed during execution",
    )
    require(
        receipt.get("target_script") == expected_target_relative
        and receipt.get("target_source_sha256")
        == receipt.get("exact_executed_source_buffer_sha256")
        == target_sha,
        "runtime witness executed source is not manifest-bound",
    )
    require(
        receipt.get("target_output") == expected_output_relative
        and receipt.get("target_output_sha256")
        == EXPECTED_RUNTIME_WITNESS_HASHES["output"],
        "runtime witness receipt output binding failed",
    )
    require(
        receipt.get("runtime_snapshot_sha256")
        == manifest.get("runtime_snapshot_sha256"),
        "runtime witness snapshot binding failed",
    )
    require(
        receipt.get("output_contract_sha256")
        == canonical_sha256(manifest.get("output_contract")),
        "runtime witness output contract binding failed",
    )
    required_true = (
    ...
    require(
        ...
        "runtime witness lost a required assurance/disclosure flag",
    )
    required_false = (
        "alpha_computed",
    ...
    require(
        ...
        "runtime witness limitation/firewall flags changed",
    )
    require(
        receipt.get("target_manifest_files_sha256_before")
        == receipt.get("target_manifest_files_sha256_after")
        and receipt.get("runtime_manifest_sha256_before")
        == receipt.get("runtime_manifest_sha256_after"),
        "runtime witness file/runtime inventories changed during execution",
    )
    require(
        isinstance(receipt.get("loaded_modules"), list)
        and bool(receipt.get("loaded_modules"))
        and isinstance(receipt.get("loaded_native_images"), list)
        and bool(receipt.get("loaded_native_images")),
        "runtime witness receipt lacks module/native-image ledgers",
    )
...
def main() -> None:
    documents = {
        "gate": "BID_SOURCE_PARENT_CLOSURE_GATE_V002.md",
    ...
    expect(
        statuses["gate"],
        {
            ...
            "content_addressed_source_parent_runtime_witness_verified": True,
    ...
    verify_two_layer_runtime_witness()
    ...
    # SP14 retains both paths: sealed execution must satisfy the isolated
    # runtime contract, while an unsealed developer run still sanitizes
    # startup inputs and must reproduce the same subordinate verdicts.
    if SEALED_EXECUTION:
        require(
            sys.flags.isolated
    ...
    if SEALED_EXECUTION:
        ...
        payload = {
            ...
            "checks": {
                ...
                "normal_optimized_source_equivalence": True,
                "startup_custom_modules_absent": True,
                "all_known_obligations_retained": True,
                "two_layer_runtime_witness_verified": True,
            },
        }
        ...
        print("two_layer_runtime_witness_verified=PASS")
        print(f"unsealed_content_root_sha256={current_root}")
        print("content_addressed_isolated_runtime_manifest=PASS_TWO_LAYER_WITNESS")
