#!/usr/bin/env python3
"""Synthetic fence tests for the Stage-8 T7 bundle comparator."""
...
SCRIPT = (
    ...
    / "compare_stage8_t7_actual_parent_regulated_car_operator_response_v001.py"
)
MODULE_SPEC = importlib.util.spec_from_file_location("stage8_t7_comparator", SCRIPT)
...
class ComparatorFenceTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        implementation_files = (
            ...
            "scripts/test_stage8_t7_actual_parent_regulated_car_operator_response_primary_v001.py",
            "scripts/derive_stage8_t7_actual_parent_regulated_car_operator_response_independent_v001.py",
            "scripts/test_stage8_t7_actual_parent_regulated_car_operator_response_independent_v001.py",
            "scripts/compare_stage8_t7_actual_parent_regulated_car_operator_response_v001.py",
            "scripts/test_compare_stage8_t7_actual_parent_regulated_car_operator_response_v001.py",
        ...
        implementation_manifest.write_text(
            json.dumps(
                {
                    "schema": "stage8_t7_actual_parent_car_implementation_test_v001",
                    "files": [
                        {
                            "path": relative,
        ...
        self.primary_matrices = self.build_primary_matrices()
    ...
    def build_primary_matrices(self) -> dict[str, dict[str, np.ndarray]]:
        ...
        for ell_index, ell in enumerate(("ell0", "ell1")):
            ...
            for history_index, history in enumerate(self.history_values):
                for record_index, record_value in enumerate(self.record_values):
                    propagator = np.diag(
                        np.exp(
                            -1j
                            * (
                                scale * history
                                + 0.03 * float(record_value)
                            )
                            * diagonal
                        )
                    )
    ...
    def bundle_content(self, lane: str) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
        ...
        for category, values in self.primary_matrices.items():
            for identifier, primary_matrix in values.items():
                key = f"{category}__{identifier.replace('.', '_')}"
                if category == "record_projectors" or lane == "primary":
                    arrays[key] = primary_matrix.copy()
                else:
                    arrays[key] = (
                        self.overlap.conjugate().T
                        @ primary_matrix
                        @ self.overlap
                    )
        ...
        connection = np.diag(np.linspace(0.1, 0.2, 32)).astype(complex)
        for ell in COMPARATOR.EXPECTED_DISCRETE_LABELS["ell"]:
            if lane == "independent":
                transported_connection = (
                    self.overlap.conjugate().T @ connection @ self.overlap
                )
                arrays[f"diag_h0__{ell}"] = np.zeros((32, 32), complex)
                arrays[f"diag_connection_midpoint__{ell}"] = (
                    transported_connection
                )
            else:
                arrays[f"diag_h0__{ell}"] = np.zeros((32, 32), complex)
                arrays[f"diag_connection_midpoint__{ell}"] = connection.copy()
            for pair in ("p1", "p2"):
                for kernel in ("all", "pointer"):
                    target = self.primary_matrices["aggregate_kernels"][
                        f"{ell}.{pair}.{kernel}"
                    ]
                    lane_target = (
                        target
                        if lane == "primary"
                        else self.overlap.conjugate().T @ target @ self.overlap
                    )
                    if lane == "primary":
                        arrays[
        ...
        *,
        ...
        payload = {
            ...
            "runtime_attestation": dict(
                getattr(sys, COMPARATOR.RUNTIME_MARKER)
            ),
