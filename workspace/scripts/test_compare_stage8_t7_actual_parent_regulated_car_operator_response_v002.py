#!/usr/bin/env python3
"""Synthetic fence tests for the Stage-8 T7 bundle comparator (v002).

Ports the v001 fixture tests onto a GENUINE synthetic fixture whose
one-particle propagators are actually midpoint-integrated from small stored
generator pieces (repair binding R2), and adds fences for the v002 repairs:

- a fabricated-propagator bundle (correct pieces, u not integrated from
  them) must BLOCK on the lineage gate;
- the genuine synthetic fixture must PASS the lineage gate;
- the new spectral-Kraus and adjoint-exchange gates block on perturbation;
- E_conn history-pair pinning (execution binding B2) is recorded;
- the finding-7 flag renaming is verified (no protected-flag name true).
...
SCRIPT = (
    ...
    / "compare_stage8_t7_actual_parent_regulated_car_operator_response_v002.py"
)
MODULE_SPEC = importlib.util.spec_from_file_location("stage8_t7_comparator_v002", SCRIPT)
...
# These unit tests may run either under the sealed v002 launcher (which
...
# the comparator requires, bound to the real v002 launcher file hash so the
# fixture manifest row matches.  Under a sealed launcher the real marker is
# used unchanged.
V002_LAUNCHER = COMPARATOR.ROOT / COMPARATOR.RUNTIME_LAUNCHER_PATH
...
def build_time_stacks(steps: int, ell_index: int, pieces: dict[str, Any]) -> dict[str, np.ndarray]:
    times = (np.arange(steps, dtype=np.float64) + 0.5) / steps
    scale = ell_scale(ell_index)
    m_profile = np.sin(np.pi * times) ** 2
    b_profile = np.sin(np.pi * times)
    m_stack = np.stack(
        [scale * profile * pieces["cell_matrix"] for profile in m_profile]
    ).astype(complex)
    b_stack = np.stack(
        [scale * profile * pieces["bump_matrix"] for profile in b_profile]
    ).astype(complex)
    return {"times": times, "m_stack": m_stack, "b_stack": b_stack}
...
class FixtureData:
    """Class-level cache of the genuine synthetic fixture (built once)."""

    _instance: "FixtureData | None" = None

    @classmethod
    def instance(cls) -> "FixtureData":
    ...
    def __init__(self) -> None:
        ...
        for ell_index, ell in enumerate(("ell0", "ell1")):
            p_stack = (self.pieces["base_p_stack"] / ell_scale(ell_index)).astype(
                complex
            )
            ...
            for lane, steps in (("primary", 48), ("independent", 384)):
                stacks = build_time_stacks(steps, ell_index, self.pieces)
...
class ComparatorFenceTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        implementation_files = (
            ...
            "scripts/test_compare_stage8_t7_actual_parent_regulated_car_operator_response_v002.py",
        )
        implementation_manifest = self.root / "implementation.json"
        implementation_manifest.write_text(
            json.dumps(
                {
                    "schema": "stage8_t7_actual_parent_car_implementation_test_v002",
                    "files": [
                        {
                            "path": relative,
    ...
    def bundle_content(self, lane: str) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
        fixture = self.fixture
    ...
    def write_bundle(
        ...
        *,
        ...
        manifest, arrays = self.bundle_content(lane)
    ...
    # ------------------------------------------------------------------
    # Ported v001 fence tests (running on the genuine v002 fixture).
    # ------------------------------------------------------------------
