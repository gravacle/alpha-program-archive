#!/usr/bin/env python3
"""Build the content-addressed manifest for the source-parent regression gate."""
...
OUTPUT = PROJECT_ROOT / "results" / "bid_source_parent_closure_v001_sealed.json"
RECEIPT = (
    PROJECT_ROOT
    / "provenance"
    / "bid_source_parent_closure_v001_execution_receipt.json"
)
TARGET = ROOT / "scripts" / "audit_bid_source_parent_closure_gate_v001.py"
OBLIGATION_ROW = re.compile(
    r"^\| (SP\d{2}) \| ([^|]+?) \| ([^|]+?) \| ([^|]+?) \|$"
)

DOCUMENTS = (
    "BID_SOURCE_PARENT_CLOSURE_GATE_V001.md",
...
def main() -> None:
    for path in (RUNTIME, RUNTIME_BUILDER, RUNTIME_GATE, TARGET):
        if not path.is_file():
    ...
    dependency_relatives = (
        ...
        + ("scripts/audit_bid_source_parent_closure_gate_v001.py",)
    )
    ...
    files = {
        ...
        for path in dependency_paths + (RUNTIME_BUILDER, RUNTIME_GATE)
    }
    checks = [
    ...
    exact = {
        "schema": "gravacle.bid-source-parent-regression.v001",
    ...
    payload = {
        ...
        "stage": "bid-source-parent-regression-gate-v001",
        "alpha_computed": False,
