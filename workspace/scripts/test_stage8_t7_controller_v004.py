#!/usr/bin/env python3
"""Fixture-only integration tests for the v004 actual-parent CAR controller.
...
lane scripts, a stub launcher, a fixture git archive (bare file:// remote),
and BOTH fixture manifests: the controller's own v003-path implementation
manifest and the bridge (v001-path) manifest the frozen derive lanes verify.
The canonical workspace is only ever read (the controller source is copied
out, never executed against the canonical root); the copied controller's
pinned archive-repository path is repointed at the fixture archive.

v004 (manifest-binding bridge amendment V001, FIX 1 + RE-SCOPE): ports the
v003 suite; fixtures build both manifests with consistent derive-executor
rows so the FIX 1 bridge pre-flight passes on the happy path; a
mismatched-rows fixture must block BEFORE any canonical artifact exists; the
push-capability pre-flight is exercised against a fixture git repository
with a file:// remote (happy path, which must remain untouched by the dry
run) and against a nonexistent remote (fail-closed block, zero artifacts).
"""
...
CONTROLLER_SOURCE = ROOT / "scripts/run_stage8_t7_actual_parent_car_pipeline_v004.py"
PINNED_PYTHON = Path(
...
CONTROLLER_RELATIVE = "scripts/run_stage8_t7_actual_parent_car_pipeline_v004.py"
LAUNCHER_RELATIVE = "scripts/launch_stage8_t7_content_addressed_runtime_v004.py"
...
# S1-1d; unchanged by the v004 bridge, which is bundle-side, not CLI-side).
# A controller that omits either flag fails this stub's argparse.
STUB_COMPARATOR_TEMPLATE = '''#!/usr/bin/env python3
"""Fixture stub comparator lane (real v004 CLI contract)."""
{helpers}
import argparse
...
def main():
    ...
    payload = {{
        "schema": "fixture_stub_comparison_v004",
        "overall_verdict":
...
def build_fixture(
    ...
    *,
    ...
    require(
        ...
        "controller v004 must pin the archive repository path",
    )
...
def test_gpg_fully_removed() -> None:
    ...
    for forbidden in (
        ...
        require(
            ...
            f"controller v004 must not reference {forbidden!r}",
        )
...
def test_bridge_row_mismatch_blocks() -> None:
    # FIX 1 amendment A1: the entire bridge condition is pre-flighted BEFORE
    # any lane runs. A v001-path manifest whose derive-executor row disagrees
    # with the controller's own manifest must block with ZERO artifacts (the
    # exact wedge class of the re-audit: no BLOCKED comparison output, no
    # consumed canonical path).
...
def test_push_capability_failure_blocks() -> None:
    # RE-SCOPE item: a push failure discovered after sealing reproduces the
    # GPG wedge, so push capability is pre-flighted fail-closed. The fixture
    # archive's origin points at a nonexistent remote: the dry-run push
    # fails, the controller blocks, and no lane or artifact may exist.
...
def main() -> int:
    ...
    print(f"controller_v004_tests_passed {len(tests)}/{len(tests)}")
    return 0
