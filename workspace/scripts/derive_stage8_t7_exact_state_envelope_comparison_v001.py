#!/usr/bin/env python3
"""Exact-state, higher-resolution ER-A/ER-B Hermite-Galerkin comparison."""
...
OUT = ROOT / "stage8_execution/work/T07_exact_state_envelope_comparison.json"
EXACT = ROOT / "stage8_execution/work/T07_exact_hermite_mixed_covariance.npz"
EXACT_SHA256 = "395141f2d3146d638b5c395b14a06eaa347701aec4f3cd6715befc6f8cda2464"
...
def run_case(
    ...
    for mode in ("ER-A", "ER-B"):
        ...
        for steps in (24, 48, 96):
            ...
            mixed, pure, mixed_terms, pure_terms = base.completed_amplitudes(
            ...
            amplitudes[str(steps)] = {
                ...
                "mixed_history_terms": [
                    base.complex_json(value) for value in mixed_terms
                ],
                "pure_history_terms": [
                    base.complex_json(value) for value in pure_terms
                ],
            }
            if steps == 96:
                ell_label = "1" if abs(ell - 1.0) < 1e-15 else "sqrt2"
                mode_label = mode.replace("-", "_")
                for index, unitary in enumerate(histories):
                    history_store[
...
def main() -> None:
    ...
    require(sha256(EXACT) == EXACT_SHA256, "Exact covariance artifact drift")

    alphas, incidence, clifford_error = base.dirac_matrices()
    history_store: dict[str, np.ndarray] = {}
    cases = [
        run_case(n, ell, alphas, incidence, history_store)
        for n in (2, 4)
    ...
    np.savez_compressed(HISTORIES, **history_store)
    result = {
        "schema": "stage8_t7_exact_state_envelope_comparison_v001",
        "spec_sha256": sha256(SPEC),
        "exact_covariance_sha256": EXACT_SHA256,
        "history_artifact": str(HISTORIES.relative_to(ROOT)),
        "history_artifact_sha256": sha256(HISTORIES),
        "cases": cases,
        "numerics_pass": numerics_pass,
        "overall_verdict": (
            "EXACT_STATE_ENVELOPE_DIAGNOSTICS_COMPUTED"
            if numerics_pass
