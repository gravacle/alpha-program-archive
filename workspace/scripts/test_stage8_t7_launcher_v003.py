...
LAUNCHER = ROOT / "scripts/launch_stage8_t7_content_addressed_runtime_v003.py"
PINNED_PYTHON = Path(
...
def test_attestation_content() -> None:
    ...
    echoed = launcher_subprocess(
        ["scripts/test_stage8_t7_launcher_v003.py"], environment
    )
    require(
        echoed.returncode == 0,
...
def main() -> int:
    ...
    print(f"launcher_v003_tests_passed {len(tests)}/{len(tests)}")
    return 0
