...
TERMINAL = PROJECT_ROOT / "results" / "bid_source_parent_sp14_terminal_v006.json"
SIGNATURE = PROJECT_ROOT / "results" / "bid_source_parent_sp14_terminal_v006.json.asc"
...
def validate_preserved_failure_lineage() -> dict[str, dict[str, str]]:
    output: dict[str, dict[str, str]] = {}
    for version, record, record_hash, seal, seal_hash in (
        (
            "v003",
            V003_FAILURE_RECORD,
            V003_FAILURE_RECORD_SHA256,
        ...
        require(
            record.is_file() and seal.is_file(),
            f"preserved {version} failure lineage is absent",
        )
        require(
            sha256(record) == record_hash
            and sha256(seal) == seal_hash
            and seal.read_text(encoding="utf-8").strip()
            == f"{record_hash}  {record.name}",
            f"preserved {version} failure lineage changed",
        )
        output[version] = {
...
def validate_manifest_envelope(
    ...
    require(required <= set(manifest), "manifest envelope field inventory")
    require(
        manifest.get("preserved_failure_lineage")
        == validate_preserved_failure_lineage(),
        "manifest does not bind the complete preserved failure lineage",
    )
    ...
    require(
        all(isinstance(name, str) and isinstance(digest, str) for name, digest in files.items()),
        "manifest file row malformed",
    )
    target_text = manifest.get("target_script")
    require(isinstance(target_text, str), "manifest target path malformed")
    target = (PROJECT_ROOT / target_text).resolve()
    ...
    require(
        ...
        and files.get(relative(V005_FAILURE_RECORD)) == V005_FAILURE_RECORD_SHA256
        and files.get(relative(V005_FAILURE_SEAL)) == V005_FAILURE_SEAL_SHA256,
        "manifest file inventory omits preserved failure lineage",
    )
    contract = manifest.get("output_contract")
    require(isinstance(contract, dict), "manifest output contract absent")
    exact = contract.get("exact_values")
    require(
        isinstance(exact, dict)
        and exact.get("alpha_computed") is False
        and exact.get("proof_authorized") is False
        and exact.get("source_parent_closure") is False,
        "manifest output authority firewall",
    )
    return files
...
def validate_manifest(manifest_path: Path, runtime: dict[str, Any], optimize: int) -> dict[str, Any]:
    ...
    files = validate_manifest_envelope(manifest, runtime, optimize)
...
def main() -> None:
    ...
    produced = (
        ...
        PROJECT_ROOT / "results" / "bid_source_parent_terminal_children_verifier_v006.json",
        PROJECT_ROOT / "provenance" / "bid_source_parent_terminal_producer_normal_v006_execution_receipt.json",
        PROJECT_ROOT / "provenance" / "bid_source_parent_terminal_producer_optimized_v006_execution_receipt.json",
        PROJECT_ROOT / "provenance" / "bid_source_parent_terminal_children_verifier_v006_execution_receipt.json",
    )
    ...
    manifest_contract_test = run_python(runtime, MANIFEST_CONTRACT_TEST)
    require(
        manifest_contract_test.stdout.splitlines()
        == [
            "V006_MANIFEST_CONTRACT_PREFLIGHT=PASS",
            "positive_envelopes=3",
            "child_contract_inventories=1",
            "hostile_negative_controls=4",
            "preserved_failure_generations=2",
            "alpha_computed=false",
        ],
        "v006 builder-to-production-validator preflight did not pass exactly",
    )
    ...
    require(
        ...
        == V003_FAILURE_SEAL_SHA256
        and verifier.get("preserved_v005_failure_record_sha256")
        == V005_FAILURE_RECORD_SHA256
        and verifier.get("preserved_v005_failure_seal_sha256")
        == V005_FAILURE_SEAL_SHA256
        and verifier.get("checks", {}).get("preserved_failure_lineage_validated")
        is True,
        "independent verifier did not bind the complete preserved failure lineage",
    )
    require(
        failure_lineage == validate_preserved_failure_lineage()
        and all(
    ...
    terminal = {
        "schema": "gravacle.bid-source-parent-sp14-terminal.v006",
        ...
        "preserved_failure_lineage": failure_lineage,
        ...
        "builder_to_production_validator_preflight": True,
        "manifest_contract_preflight_sha256": sha256(MANIFEST_CONTRACT_TEST),
