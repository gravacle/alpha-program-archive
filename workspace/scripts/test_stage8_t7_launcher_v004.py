#!/usr/bin/env python3
"""Fixture-only tests for the v003 content-addressed runtime launcher.
...
v003 adds the READ-ONCE A/B/A test (repair binding V002, S1-4): the
attestation digest must equal the digest of the source that ACTUALLY
executed, including under an adversarial swap attempt after target
resolution.
"""
...
LAUNCHER = ROOT / "scripts/launch_stage8_t7_content_addressed_runtime_v004.py"
PINNED_PYTHON = Path(
...
def load_launcher_module():
    spec = importlib.util.spec_from_file_location("launcher_v003_under_test", LAUNCHER)
...
def test_allowlist_escape_blocked() -> None:
    ...
    expect_block(
        ...
        "not allowlisted",
        "v001 target must not be allowlisted by the v003 launcher",
    )
    expect_block(
        lambda: module.resolve_target(
        ...
        "not allowlisted",
        "v002 comparator must not be allowlisted by the v003 launcher",
    )
    expect_block(
        lambda: module.resolve_target(
        ...
        "not allowlisted",
        "v002 controller must not be allowlisted by the v003 launcher",
    )
    expect_block(
    ...
    expect_block(
        lambda: module.resolve_target("scripts/nonexistent_target_v003.py"),
        "not allowlisted",
        "unknown script name",
    )
...
def test_symlink_handling() -> None:
    ...
    alias_link = fixture / "scripts/unlisted_alias_v003.py"
    alias_link.symlink_to(canonical)
    plain_unlisted = fixture / "scripts/plain_unlisted_v003.py"
...
def test_read_once_execution_and_swap_resistance() -> None:
    ...
    (fixture / "scripts").mkdir(parents=True)
    target = fixture / "scripts/run_stage8_t7_actual_parent_car_pipeline_v003.py"
...
def test_attestation_content() -> None:
    ...
    echoed = launcher_subprocess(
        ["scripts/test_stage8_t7_launcher_v004.py"], environment
    )
    require(
        echoed.returncode == 0,
...
def main() -> int:
    ...
    print(f"launcher_v004_tests_passed {len(tests)}/{len(tests)}")
    return 0
