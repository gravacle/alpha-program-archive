#!/usr/bin/env python3
"""Fail-closed consistency audit for CURRENT_AUTHORITY_LEDGER_V006.

This script checks bookkeeping only. It does not certify any physical premise.
"""
...
LEDGER = ROOT / "CURRENT_AUTHORITY_LEDGER_V006.json"
...
def main() -> None:
    ...
    assert state["ordinary_continuum_absolute_selector_exists"] is False
    assert state["known_finite_gauge_route_fixes_absolute_stiffness"] is False
    ...
    assert state["complete_parameter_free_Q_spec_frozen"] is False
    ...
    out = {
        "status": "PASS_AUTHORITY_V006_FINITE_CELL_PROTOCOL_FROZEN_ALPHA_FALSE",
        "missing_files": missing,
        "alpha_computed": state["alpha_computed"],
        "proof_authorized": state["proof_authorized"],
        "scope": "bookkeeping_consistency_only",
    }
    result_path = ROOT / "results" / "current_authority_v006.json"
