...
RUNTIME_LAUNCHER = (
    ROOT / "scripts/launch_stage8_t7_content_addressed_runtime_v002.py"
)
...
def sign_receipt(receipt: Path) -> str:
    signature = Path(f"{receipt}.asc")
    require(GPG.is_file(), "GPG binary is absent; receipt signing fails closed")
    require(
        not signature.exists(),
        f"immutable receipt signature already exists: {signature}",
    )
    ...
    require(
        signing.returncode == 0 and signature.is_file(),
        f"receipt signing failed (fail closed): {receipt}",
    )
    ...
    require(
        ...
        f"receipt signature verification failed: {receipt}",
    )
    require(
        f"[GNUPG:] VALIDSIG {SIGNING_FINGERPRINT}" in verification.stdout,
        "receipt signature fingerprint mismatch",
    )
    signature.chmod(stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH)
...
def run_lane(
    *,
    ...
    payload = {
        "schema": "stage8_t7_local_sealed_execution_receipt_v002",
        "lane": lane,
        "status": (
...
def main() -> int:
    ...
    summary = {
        "schema": "stage8_t7_actual_parent_car_pipeline_summary_v002",
