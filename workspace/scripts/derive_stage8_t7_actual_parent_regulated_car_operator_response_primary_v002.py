...
def spatial_momenta(n: int, ell: float) -> tuple[np.ndarray, ...]:
...
def build_time_data(
    ...
    spatial_cells: list[np.ndarray] = []
    spatial_bumps: list[np.ndarray] = []
...
def execute_resolution(
    ...
    return {
        ...
        "spatial_bumps": spatial_bumps,
        "midpoint_times": midpoint_times,
        "spatial_momenta": momenta,
        "unitaries": unitaries,
...
def isolated_route1_rerun() -> dict[str, object]:
    ...
    canonical_payload = json.loads(canonical_result.read_text(encoding="utf-8"))
    canonical_route1 = canonical_payload.get(
        "route1_special_case_consistency_falsifier", {}
    )
    canonical_rows = canonical_route1.get("rows")
    rerun_rows = route1.get("rows")
    require(
        isinstance(canonical_rows, list)
        and isinstance(rerun_rows, list)
        and len(canonical_rows) == len(rerun_rows)
        and len(canonical_rows) > 0,
        "Route-1 canonical value-comparison row inventory mismatch",
    )
    canonical_value_fields = (
        "route1_completed_component_real",
        "route1_completed_component_imag",
        "route2_generic_all_kernel_real",
        "route2_generic_all_kernel_imag",
    )
    maximum_canonical_value_difference = 0.0
    for canonical_row, rerun_row in zip(canonical_rows, rerun_rows):
        require(
            float(canonical_row["theta_plus"]) == float(rerun_row["theta_plus"])
            and float(canonical_row["theta_minus"])
            == float(rerun_row["theta_minus"]),
            "Route-1 canonical comparator history pair mismatch",
        )
        for field in canonical_value_fields:
            difference = abs(
                float(rerun_row[field]) - float(canonical_row[field])
            )
            maximum_canonical_value_difference = max(
                maximum_canonical_value_difference, difference
            )
            require(
                difference <= 1.0e-12,
                f"Route-1 canonical value comparison failed: {field}",
            )
    return {
        ...
        "canonical_value_comparison": {
            "canonical_sha256": expected_canonical,
            "threshold_absolute": 1.0e-12,
            "value_fields": list(canonical_value_fields),
            "row_count": len(canonical_rows),
            "maximum_absolute_difference": maximum_canonical_value_difference,
            "passed": True,
        },
        "passed": True,
    }
...
def execute() -> dict[str, object]:
    ...
    arrays["genpiece_alpha_stack"] = np.stack(alphas).astype(np.complex128)
    arrays["genpiece_Sn"] = source_incidence.astype(np.complex128)
    reports: dict[str, object] = {}
    ...
    for ell_index, ell in enumerate(ELLS):
        ...
        arrays[f"genpiece_midpoint_times__{tag}"] = np.asarray(
            production["midpoint_times"], dtype=np.float64
        )
        arrays[f"genpiece_M_stack__{tag}"] = np.stack(
            production["spatial_cells"]
        ).astype(np.complex128)
        arrays[f"genpiece_B_stack__{tag}"] = np.stack(
            production["spatial_bumps"]
        ).astype(np.complex128)
        arrays[f"genpiece_h0__{tag}"] = production["h0"].astype(np.complex128)
        arrays[f"genpiece_p_stack__{tag}"] = np.stack(
            production["spatial_momenta"]
        ).astype(np.complex128)
        ...
        same_history_identity_checks: dict[str, dict[str, float]] = {}
        for history_index in range(len(HISTORIES)):
            same_all, same_pointer, _ = gaussian_responses(
                production["unitaries"][history_index],
                production["unitaries"][history_index],
                probabilities,
                pointer_weights,
            )
            identity = np.eye(same_all.shape[0], dtype=complex)
            all_identity_residual = matrix_norm(same_all - identity)
            pointer_eigenvalues = np.linalg.eigvalsh(
                0.5 * (same_pointer + same_pointer.conjugate().T)
            )
            pointer_bound_violation = max(
                0.0,
                -float(pointer_eigenvalues[0]),
                float(pointer_eigenvalues[-1]) - 1.0,
            )
            require(
                all_identity_residual <= INTERNAL_TOL,
                f"Same-history all-kernel identity failed: {tag} a{history_index}",
            )
            require(
                pointer_bound_violation <= INTERNAL_TOL,
                f"Same-history pointer bound failed: {tag} a{history_index}",
            )
            same_history_identity_checks[f"a{history_index}"] = {
                "history_value": float(HISTORIES[history_index]),
                "all_identity_residual": all_identity_residual,
                "pointer_bound_violation": pointer_bound_violation,
            }
        ...
        reports[tag] = {
            ...
            "same_history_identity_checks_n48": same_history_identity_checks,
        }
    ...
    result = {
        ...
        "generator_midpoint_count": {
            tag_ell(index): RESOLUTIONS[-1] for index in range(len(ELLS))
        },
        "lane": "primary",
        "spec_sha256": SPEC_SHA256,
