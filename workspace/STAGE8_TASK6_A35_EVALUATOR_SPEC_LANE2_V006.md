# STAGE8 TASK 6 / A35–RD-22 — evaluator specification V006

**Artifact:** `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V006.md`  
**Lane:** Codex Lane 2  
**Date:** 2026-08-07  
**Custody:** V006 corpus-definition rule and affected-row registry by Lane 2; all 66 V005 criteria carry byte-unchanged; any RD-22 execution authorization remains a principal act  
**Status:** V006 FINITE-DELTA SPECIFICATION ONLY — the `M2(q,S)` corpus-definition precondition is installed and the 17 affected rows are registered as `SPEC-INCOMPLETE` pending sealed corpus definitions; no criterion, implementation, physical check, fixture, physical quantity, fixed point, end test, or authorization is executed here

```text
M2_CORPUS_RULE = installed
M2_REGISTRY = 17 rows (SPEC-INCOMPLETE pending sealed corpus definitions)
DESCRIPTOR_ROWS = unchanged (66/66 byte-identical to V005)
...
[PART-PROVABLE] The commissioned live preflight identifies register head `Q-607`;
that entry records C77 and commissions the V009-06 envelope plus this minimal
V006 corpus-definition rule and affected-row registry. The Q-604 guard remains:
one lawful row is one row and is not evaluator closure.
The live questions register exposes Q-607 as its final entry.

[PROVABLE] Before V006 was written, neither its requested filename nor the
commissioned Lane 2 report filename existed in the cleanroom or archive
workspace. V005 remains intact. V006 is a byte-copy-plus-named-replacements
...
#### V006 sealed-corpus law for `M2(q,S)`

[YOURS] For every invocation `M2(q,S)`, the operand `S` **MUST name a
`SEALED_CORPUS_DEFINITION`**: a content-addressed member list of record whose
sealed source supplies, at minimum, each member's declared relative path, byte
length, and SHA-256, together with the list's content root. The implementation
must verify the definition and every member before `M2` starts. A directory
listing, an inline ad hoc set, a generic source-set label, or a set inferred by
the evaluator is not a sealed corpus definition.
...
The V006 sweep of all 66 descriptor rows finds the following complete affected
...
| Check ID | Current `S` operand or shorthand corpus | V006 status |
|---|---|---|
