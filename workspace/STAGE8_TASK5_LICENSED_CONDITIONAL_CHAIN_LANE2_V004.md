# STAGE 8 TASK 5 - LICENSED CONDITIONAL CHAIN V004

[EQ6] Date: 2026-08-04
[EQ6] Lane: Codex Lane 2
[EQ6] Task: PASTE 540
[EQ6] Parent: `STAGE8_TASK5_LICENSED_CONDITIONAL_CHAIN_LANE2_V003`
[EQ6] Parent SHA-256: `f2b0b786ca20df1b96714d870811e14effa414724c1cfe08b35d63e31e301b7b`
...
[EQ6] IS_THE_VERSION_CURRENT = yes_through_Q-462
[EQ6] ARE_ITS_INPUTS_PRESENT = yes
[EQ6] PRECHECK = PASS (artifact and register seals verified)
[EQ6] RELAY_REGISTER_HEAD = Q-461
[EQ6] LIVE_REGISTER_HEAD = Q-462
[EQ6] Q462_EFFECT = chain V003 cross-check (X3/X4) completed;
[EQ6]               full finite certificate remains under construction;
[EQ6]               this bounded V004 repair is now current.
...
[EQ6] CHAIN_V004 = READY_FOR_CROSS_CHECK
[EQ6] CASE_LATTICE = COMPLETE
[EQ6] MODULUS_CERT = ADDED
...
[EQ6]     OR the separately typed edge cases below.
[EQ6]     Consumer: threshold theorem / Gate: `C_RET_SCOPE_w` + `q_loop` formula.

[EQ6]     Full case lattice:
[EQ6]     1. `A_loop = 0`, `chi_K = 0`: `q_loop = 0`, strict contraction.
[EQ6]     2. `A_loop = 0`, `chi_K ≠ 0`: `q_loop = 0`, strict contraction.
[EQ6]     3. `0 < A_loop < ∞`, `chi_K = 0`: `q_loop = 0`, strict contraction.
[EQ6]     4. `0 < A_loop < ∞`, `chi_K ≠ 0`: strict iff `|chi_K| < A_loop^{-1}`.
[EQ6]     5. `A_loop = ∞`, `chi_K = 0`: `q_loop = 0`, strict contraction (pointwise finite branch).
[EQ6]     6. `A_loop = ∞`, `chi_K ≠ 0`: excluded, `q_loop = ∞`, not contractive.

[EQ6] ## P1. Degenerate-threshold branch extension

[EQ6] Step 10 now includes the explicit edge branch `A_loop = ∞`, `chi_K = 0` with the value of `q_loop` declared by the separate branch rule, not by undefined `0·∞` arithmetic.
[EQ6] All eight lattice cells are either licensed as strict contraction (`q_loop<1`) or explicitly excluded as non-contractive (`q_loop=∞`).

[EQ6] ## P2. Modulus-compatibility certificate

[EQ6] Add `MODULUS_COMPATIBILITY_CERT` as a separate falsifiable gate at the Step 8/9 seam:

[EQ6] `MODULUS_COMPATIBILITY_CERT[w]` holds if and only if one of the following is proved:

[EQ6] 1) `DIFF_TO_METRIC` witness:
[EQ6]    - `d_w` is equivalent to the coordinate metric used in Step 8 on `D_w` by a certified chart, and
[EQ6]    - the chain rule transports the Step 8 differential bound to `d_w` exactly.
[EQ6] 2) `DIRECT_MODULUS` witness:
[EQ6]    - Step 9 defines `A_loop` by `d_w`-difference quotients directly, and
[EQ6]    - the Step 8 derivative is used only as a consistency witness, not as an identification.

[EQ6] Exact failure mode blocked: a complete `d_w` metric that lacks (1) and (2) can make
[EQ6] `sup_Dw|dot B_w|` disagree with the true `sup_Dw d_w` modulus (the alternate-complete-metric attack).
...
[EQ6] | V002 location | V004 bounded change | Reason |
[EQ6] |---|---|---|
[EQ6] | preflight/head | rechecked through Q-462; Q-458/Q-460 did not discharge full entrance | send-time currentness |
