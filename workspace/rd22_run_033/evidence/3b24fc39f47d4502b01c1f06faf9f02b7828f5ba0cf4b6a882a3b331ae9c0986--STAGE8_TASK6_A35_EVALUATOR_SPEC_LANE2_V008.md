# STAGE8 TASK 6 / A35–RD-22 — evaluator specification V008

**Artifact:** `STAGE8_TASK6_A35_EVALUATOR_SPEC_LANE2_V008.md`  
**Lane:** Codex Lane 2  
**Date:** 2026-08-08  
**Custody:** V008 installs the three Q-611 R9 replay-interface statements; all 66 V007 descriptor rows remain byte-identical; Builder A implements and dry-checks its side while Builder B's real manifest integration remains the next independent custody act  
**Status:** V008 FINITE-DELTA SPECIFICATION ONLY — independent P0 computation, the closed two-manifest launch interface, and the distinct precondition-refusal value are installed; no criterion, physical check, fixture, physical quantity, fixed point, end test, or authorization is executed here

```text
V008 = three R9 statements
...
[PROVABLE] The commissioned live preflight identifies register head `Q-611`.
That entry records the run-030 replay adjudication and the two whole repairs
commissioned for this relay. The Q-604 guard remains: one lawful row is one
row and is not evaluator closure.

[PROVABLE] Before V008 was written, neither its requested filename nor the
commissioned Lane 2 report filename existed in the cleanroom or archive
workspace. V007 remains intact. V008 is a byte-copy-plus-named-replacements
...
The verifier-manifest instance remains an exact closed eleven-field object,
...
```json
{
  ...
  "required":["argv","entry_point","exit_contract","input_roots","optimize","output_path","receipt_authoritative","receipt_path","schema","stdout_discipline","verifier_root_sha256"],
  "properties":{
...
[PROVABLE] The three structural fixture source rows migrate to these V008
zero-based half-open byte spans; each generated fixture-manifest row must use
the matching V008 path and final V008 SHA-256:

| Fixture ID | V008 source byte span |
|---|---|
| source row for `FX-A35-03-C-FAMILY` | `[133496,133684)` |
| source row for `FX-A35-04-TAU-FAMILY` | `[133685,133872)` |
| source row for `FX-A35-05-PRIMITIVE-THOMSON-CONFLATION` | `[133873,134125)` |
