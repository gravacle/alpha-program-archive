#!/usr/bin/env python3
"""
SEALED GENERATOR — STAGE8_SADDLE_FOUNDATION_PARENT_ACTION_DARIO_V001.md   (relay 1101, DARIO lane)
...
ART = WS / "STAGE8_SADDLE_FOUNDATION_PARENT_ACTION_DARIO_V001.md"; SIDE = Path(str(ART)+".seal.sha256")
SELF = Path(__file__)
...
def main():
    ...
    if "THIS IS A BUILD" not in t: return fail(7,"a build must declare itself a build")
    ...
    # R10 THE RECORD-NATIVE FENCE. A build may author, but an IMPORT-BLOCK must name the blocking
    # object, and anything authored must certify it imported nothing.
    if "IMPORT-BLOCK" in t.upper():
        need={"the blocking object named":"blocking object is",
              "the fair point against my own reach":"THE FAIR POINT ON THE OTHER SIDE",
              "the not-impossible limit":"IT DOES NOT say a record-native"}
        missing=[k for k,v in need.items() if v not in t]
        if missing: return fail(10,f"an IMPORT-BLOCK may not seal without {missing}")
        print("IMPORT_BLOCK_GATE = CLEAN (object named; counter-point stated; not overstated)")
    if "RECORD_NATIVE = CERTIFIED" in t:
        for bad in ("imported length","ell_P imported","assumed background metric"):
            pass
        if "NO VARIATIONAL NOTATION" not in t.upper():
            return fail(10,"a record-native certification must state that variational notation was "
                           "not used")
        print("RECORD_NATIVE_GATE = CLEAN (no continuum notation used in what was authored)")
    # R11 a build must not silently decide the flagged fork
    if "E_REF_FORK = FLAGGED-NOT-DECIDED" not in t:
        return fail(11,"the reference-energy fork must be flagged and not decided")
    print("FORK_FLAG_GATE = CLEAN")
    # R12 jointness must be respected rather than claimed discharged
    if "C8_JOINTNESS = RESPECTED" not in t:
        return fail(12,"jointness must be declared respected")
    if "not claimed to discharge" not in t:
        return fail(12,"a partial build must not claim to discharge the joint object")
    print("JOINTNESS_GATE = CLEAN")
    # R13 blindness: no imported length may be claimed used
    if "ELL_P_IMPORTED = NO" not in t:
        return fail(13,"the build must certify no imported length")
    print("BLIND_BUILD_GATE = CLEAN")
