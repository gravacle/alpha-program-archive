# THE ROAD — one list, here → signature

**STANDING FORMAT:** one flat list of remaining steps only; completed steps DELETED
(history in the register). ▶ in progress · ○ up next.

**STANDING STATE (Q-1034):** steps 2–5 executed to their sealed maximum. The read of
record: NO unconditional verdict — FREE true at both existing carriers (mirror),
FORCED live on nine named debts, RUNS an unforced lean; even a surviving cross-block
forces the CHANNEL, not |n|. Every remaining debt is named and assigned. The list
below IS the upgrade map, grouped by holder.

▶ 5b-remainder. THE ONE LIVE ANALYTIC WALL, RESTRUCTURED Q-1055: n-uniform
      enclosure of FOUR definite-signed quantities per (n, cell) — tr K_H, tr H_A,
      ‖K_H‖₂², ‖H_A‖₂² — the odd (weight-kill-orphaned) sector the new half; needs
      a single-composite kill or majorant at the stay strata, on no sealed artifact
      (the of-record divergence provably does NOT attach). Also open: the half-plane
      localization (closing it makes R-L0 unneeded for the n=1 leg); R-L4's r-1
      regularization-naming act; C-L2/G_cm; OBL-D.
○ 5c. PRINCIPAL ENTRIES (yours, via the mandate process): the cycle-selector /
      b₁-domain extension at the Attach entry; the Attach premise's discharge or
      continuation; the μ-chain custody (DoR-013/014); the G2-F1 authored-physics
      supply (mandate-pre-assigned).
○ 5d. RE-READ STEP 5 unconditionally when 5a–5c land (each act names its re-read).
      → verdict: FORCED / FREE / RUNS of record.
      ── if FREE: seal Finish B → 11. ── if RUNS: flow+anchor charter → 11.
      ── if FORCED: continue ↓ (plus the |n| comparand question, Q-1012/Q-1034 rider)
○ 6. CLOSE THE COMPUTE PRE-GATE — Q_spec → Z_Q → the κ→K* bridge; C_record(K).
○ 7. PRINCIPAL FLIPS FENCE 1 (kappa_record_computed).
○ 8. RATIONALITY-FROM-FORM CHECK → compute κ_record → K*.
○ 9. PRINCIPAL FLIPS FENCE 2 → assemble α = 1/(4π K*).
○ 10. PRINCIPAL FLIPS FENCE 3 → the preregistered END TEST, once.
○ 11. GRAVITY CLOSE — the frozen recognition spec scored against the campaign's
      results; the declaration gate (parallel track).
○ 12. THE SIGNATURE — the claim in the record's own voice, sealed, scored, pushed.

Fences: alpha_computed=false · proof_authorized=false · kappa_record_computed=false.
History: Q-1012 → Q-1052 in QUESTIONS_SETTLED_REGISTER_V001.md.
Narrative: THE_SHAPE_OF_THE_RECORD_V001.md · Framework: THE_LENS_OF_RECORD_V001.md.
