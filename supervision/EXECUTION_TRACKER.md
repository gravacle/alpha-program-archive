...
| 1081 | Dario | [PLAN:DESC-38] SURFACE-NECESSITY HUNT | DONE-CLAIMED (**READOFF_CONSUMES = JOINT-STATE**: F_N=P_0+Z_N·P_ch is in neither marginal, the correlation term isn't marginal-determined → SM-6 is **SURFACE, not machinery** — but a condition on the PRODUCER, not on a measure. **SUPPLY+SM-6 = CONTRADICTION**: an authored/supplied object can't carry from-trace provenance. Self-corrected 1077 (rho_pre 'built from' → 'lies in', against its own interest; probe byte-untouched). Six relays = 'a well-executed attempt on a door that opens the other way'. FORK left to principal; check = 1082) |
| 1082 | Codex 2 | [PLAN:DESC-38] SURFACE-NECESSITY CHECK | QUEUED (needs keystroke; attacks the two hinges — can the Y-marginal suffice after all (demotes 6 relays)? and does the port's provenance field mean FROM-TRACE or DISCLOSED-OK — the single reading that decides the fork) |
