...
| 1103 | Dario | [PLAN:DESC-49/ASSEMBLE] SURFACE-GUIDED COUPLING ASSEMBLY | STAGED-READY (revised again per principal: surface is the guide; NO imposed blueprint stack). ASSEMBLE the coupling from the record's OWN derived native terms following the record's OWN structure. If it assembles easily → RIGHT PATH (Finish-A live). If not → classify EVERY obstruction SURFACE-REAL vs MACHINERY-INDUCED; all-machinery-induced → WRONG PATH (drop the machinery, reassemble); a surface-real gap → Finish-B-SHARP (named native ingredient). Diagnostic of the APPROACH, not just the physics) |
