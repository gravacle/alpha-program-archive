...
| 1103 | Dario | [PLAN:DESC-49/ASSEMBLE] COUPLING ASSEMBLY FROM DERIVED TERMS | STAGED-READY (revised per principal: don't retrace, don't import). ASSEMBLE the scalar coupling using ONLY already-derived sealed terms; take everything ratified as GIVEN (no re-deriving); the answer is the FIRST ingredient the assembly needs that is NOT already derived — NAME it, do not build it (building = import). Scale assembles from derived terms → Finish A; first missing ingredient (likely a dimensionful scale) → Finish-B-SHARP. Lean, record-native, no log re-promotion) |
