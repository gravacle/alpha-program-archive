...
| 1103 | Dario | [PLAN:DESC-49/BUILD] COMPLETION BUILD-DOWN | STAGED-READY (re-purposed from the voided log-hunt). BUILD the completion stack DOWN record-natively (scalarization→state→effects→domains→provenance→codomain→metric-dynamics→log); build the TECHNICAL (dimensionless) items, STOP and NAME the first item that requires a DIMENSIONFUL scale (Q-1004: can't build dimensionful from dimensionless). Localizes the dimensional wall precisely → Finish A (scale emerges) or Finish-B-SHARP (record-native up to the wall item). Record-native fenced, no log re-promotion) |
