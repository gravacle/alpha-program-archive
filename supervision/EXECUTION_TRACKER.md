...
| 1081 | Dario | [PLAN:DESC-38] SURFACE-NECESSITY HUNT (re-aimed on the principal's machinery challenge) | QUEUED (needs keystroke; GATES the descent instead of presupposing it — does the read-off consume the JOINT state or only a state on Y? → is SM-6 SURFACE or MACHINERY? If Y-marginal, six descent relays resolve to 'supply/derive a lawful measure, no SM-6' and the question closes to the mandate decision) |
